...
const VALIDADOR_COLECCIONES = {
    $jsonSchema: {
        ...
        properties: {
            ...
            cdu_manual:     { bsonType: ['bool', 'null'], description: 'CDU fijada a mano: el Conformador no la recalcula.' },
            descripcion:    { bsonType: ['string', 'null'], description: 'Descripción/presentación general de la cabecera/serie.' },
            fecha_inicio:   { bsonType: ['int', 'null'], description: 'Año de inicio de publicación (p. ej. 1920).' },
            fecha_fin:      { bsonType: ['int', 'null'], description: 'Año de fin (vacío/null = hasta la actualidad).' },
...
const VALIDADOR_OBRAS = {
    $jsonSchema: {
        ...
        properties: {
            ...
            fecha_inicio:    { bsonType: ['int', 'null'], description: 'Año de inicio de publicación.' },
            fecha_fin:       { bsonType: ['int', 'null'], description: 'Año de fin (vacío/null = hasta la actualidad).' },
...
async function main() {
    ...
    try {
        ...
        if (ref) {
            const faltan = ['cbr', 'cbz', 'cb7', 'djvu', 'mobi', 'audio', 'video'].filter(v => !ref.enum.includes(v));
            if (faltan.length) {
