...
export const CATALOGO_SCRIPTS = [
    ...
    S({ id: 'inspeccionar-estructura', cat: 'Inbox', escribe: true, aplica: '--escribir',
        resumen: 'Agente de estructura: interpreta un árbol de carpetas y escribe las guías (_guia.json) para aprovecharlo',
        proposito: 'Dice qué es cada carpeta —colección, serie, editorial, materia con su CDU, obra o cajón— y lo convierte en _guia.json, que el vigilante obedece: las editoriales y materias dejan de convertirse en colecciones, las series toman su nombre canónico, y cada libro hereda la colección de su serie y la CDU de su materia (que rellena o precisa la suya; si la contradice, avisa). Señales locales gratis + IA en UNA llamada por cada 50 carpetas. Sin aplicar solo enseña el plan. NUNCA pisa una guía tuya; las carpetas dudosas solo con «incluir dudosas».',
        params: [
            { nombre: 'ruta', tipo: 'texto', requerido: true, etiqueta: 'Ruta del árbol', ejemplo: '/app/Inbox/University Press Collection' },
            { nombre: 'dudosas', flag: '--incluir-dudosas', tipo: 'switch', etiqueta: 'Escribir también las carpetas dudosas (confianza baja)' },
    ...
    S({ id: 'reparar-cdu-contaminada', cat: 'Metadatos y clasificación', escribe: true, aplica: '--ejecutar', peligroso: true,
        ...
        params: [] }),
    S({ id: 'reunir-numeros-revista', cat: 'Metadatos y clasificación', escribe: true, aplica: '--ejecutar', peligroso: true,
        resumen: 'Reúne en UNA cabecera los números de una revista repartidos en cabeceras de un solo número',
        proposito: 'Para los números cuyo FICHERO contiene el texto indicado: los cuelga de la cabecera (la crea si no existe) y borra las que se queden vacías; cambia el título que sea solo un resto del nombre del fichero por «Cabecera nº 73 (enero 2012)»; y pasa la CDU genérica (000) a la indicada o, si no se indica, a la de la cabecera. Caso que lo motivó: 2DArtist 2012, cuya guía se recicló tras el primer número. Si cambia la CDU MUEVE carpetas: ejecutar en el NAS.',
        params: [
            { nombre: 'cabecera', flag: '--cabecera', tipo: 'texto', etiqueta: 'Cabecera (nombre de la revista)', ejemplo: '2DArtist' },
            { nombre: 'fichero', flag: '--fichero', tipo: 'texto', etiqueta: 'Texto que llevan sus nombres de fichero', ejemplo: '2DAIssue' },
            { nombre: 'cdu', flag: '--cdu', tipo: 'texto', etiqueta: 'CDU para los números con 000 (opcional)', ejemplo: '741' },
        ] }),
    S({ id: 'sanear-numeros-revista', cat: 'Metadatos y clasificación', escribe: true, aplica: '--ejecutar', peligroso: true,
        resumen: 'Quita a los números de revista los datos de un LIBRO homónimo (autor, sinopsis, editorial, Dewey) y afina su fecha',
        proposito: 'Hasta el 15-sep-2026 cada número de revista se buscaba por su TÍTULO en OpenLibrary/Google Books: «11.pdf» de L\'Histoire se llevó la ficha de «11/22/63» de Stephen King (sinopsis, autor, editorial, año, Dewey → CDU). Sin cabecera: todos los números afectados — quita autores (una revista no tiene), la sinopsis que NO habla de la revista, la editorial que no es la de la revista, las categorías de libro, Dewey/LCC, y corrige el año por el del nombre del fichero; conserva lo bueno (las sinopsis de la IA que vio el número, la editorial común) y NO toca lo que parece un libro catalogado como revista (lo lista). Con cabecera: además renombra, fija CDU/editorial/idioma/descripción y rehace fecha, nº y títulos con el periodo y la numeración de la carpeta. Con CDU MUEVE carpetas: ejecutar en el NAS. Después, «limpiar-huerfanos».',
        params: [
            { nombre: 'contaminadas', flag: '--contaminadas', tipo: 'switch', etiqueta: 'Todos los números afectados (sin cabecera)' },
            { nombre: 'cabecera', flag: '--cabecera', tipo: 'texto', etiqueta: 'O solo esta cabecera', ejemplo: "l'historie" },
            { nombre: 'nombre', flag: '--nombre', tipo: 'texto', etiqueta: 'Nuevo nombre de la cabecera', ejemplo: "L'Histoire" },
            { nombre: 'cdu', flag: '--cdu', tipo: 'texto', etiqueta: 'CDU de la publicación', ejemplo: '94' },
            { nombre: 'editorial', flag: '--editorial', tipo: 'texto', etiqueta: 'Editorial de la publicación', ejemplo: 'Sophia Publications' },
            { nombre: 'idioma', flag: '--idioma', tipo: 'texto', etiqueta: 'Idioma de los números (ISO)', ejemplo: 'fr' },
            { nombre: 'descripcion', flag: '--descripcion', tipo: 'texto', etiqueta: 'Descripción de la cabecera', ejemplo: 'Revista francesa de divulgación histórica.' },
            { nombre: 'periodo', flag: '--periodo', tipo: 'texto', etiqueta: 'Años de la tirada', ejemplo: '2016 o 2009-2016' },
            { nombre: 'meses', flag: '--meses-del-nombre', tipo: 'switch', etiqueta: 'Los ficheros se llaman por el MES (1.pdf … 12.pdf, 7-8.pdf)' },
            { nombre: 'muestra', flag: '--muestra', tipo: 'texto', etiqueta: 'Nº y fecha de un número conocido', ejemplo: '419@2016-01' },
            { nombre: 'periodicidad', flag: '--periodicidad', tipo: 'texto', etiqueta: 'Periodicidad', ejemplo: 'mensual' },
        ] }),
    S({ id: 'auditar-equivalencias-cdu', cat: 'Metadatos y clasificación', escribe: true, aplica: '--reparar',
        resumen: 'Detecta equivalencias CDU aprendidas que contradicen a la tabla',
        proposito: 'La caché aprendida se consulta ANTES que la tabla, y el LCC se busca por CLASE: una decisión de la IA sobre UN libro («qa → inteligencia artificial») se convertía en regla para TODA la clase (242 libros de matemáticas). Distingue la contaminación de clase (se repara) de las equivalencias específicas que difieren de la tabla (NO se tocan: suelen ser más finas que ella). --reparar solo corrige la CACHÉ; los documentos afectados van aparte porque MUEVEN carpetas.',
        params: [] }),
    S({ id: 'completar-sinopsis', cat: 'Metadatos y clasificación', escribe: true, aplica: '--ejecutar',
        resumen: 'Recupera la sinopsis por ISBN de los documentos que no la tienen',
        proposito: 'El 37% del catálogo no tenía sinopsis: la única tarea que la rellenaba (re-enriquecer-degradados) solo entra en documentos DEGRADADOS, y una ingesta sin IA deja documentos limpios pero vacíos. Busca por ISBN en el Fichero local (offline) y, si no está, en OpenLibrary y Google Books. SIN IA. OJO: el Fichero casi no tiene sinopsis (OpenLibrary la guarda en la OBRA y el volcado solo cargó EDICIONES: de 16.556, «solo fichero» recuperó 1), así que la vía útil son las APIs, con pausa y en tandas con «límite». Reanudable: sin «forzar» solo toca los que siguen sin sinopsis.',
        params: [
            { nombre: 'solofichero', flag: '--solo-fichero', tipo: 'switch', etiqueta: 'Solo Fichero local (sin red, sin riesgo de bloqueo)' },
            { nombre: 'limite', flag: '--limite', tipo: 'numero', etiqueta: 'Máximo de documentos', ejemplo: '500' },
            { nombre: 'pausa', flag: '--pausa', tipo: 'numero', etiqueta: 'Pausa entre documentos en ms (freno anti-bloqueo)', ejemplo: '400' },
            { nombre: 'forzar', flag: '--forzar', tipo: 'switch', etiqueta: 'Reemplazar la sinopsis existente' },
            { nombre: 'id', flag: '--id', tipo: 'texto', etiqueta: 'Un solo documento', ejemplo: '6a5e2b291d9c072868af9ff2' },
        ] }),
    ...
    S({ id: 'limpiar-titulos', cat: 'Metadatos y clasificación', escribe: true, aplica: '--ejecutar',
        ...
        proposito: 'Muchos títulos del volcado BNE (MARC 245) arrastran un «:» colgante al final o un «::» que separa TÍTULO :: SUBTÍTULO. Los quita: divide en «::» (el resto pasa al subtítulo solo si no tenía uno) y suelta los dos-puntos finales. NO toca un «:» interior legítimo («Sapiens: de animales a dioses»). Es la misma limpieza que ya aplica la ingesta al leer del Fichero; aquí se propaga a lo ya catalogado. Solo texto (no mueve carpetas). DRY-RUN por defecto.',
        params: [{ nombre: 'limite', flag: '--limite', tipo: 'numero', etiqueta: 'Límite (para probar)', ejemplo: '200' }] }),
    ...
    S({ id: 'reidentificar-sin-isbn', cat: 'Metadatos y clasificación', escribe: true, aplica: '--ejecutar',
        resumen: 'Recupera el ISBN del propio fichero (miembros de colección) + pivote al Fichero/APIs',
        proposito: 'Los miembros de colección (p. ej. TXtras) se catalogaron por el nombre SIN abrir el fichero → 0 ISBN. Abre EPUB/PDF/MOBI, extrae el ISBN (SIN IA) y, con él, rellena título/autor/editorial/sinopsis/idioma desde el Fichero y las APIs gratuitas. Solo huecos; reanudable (solo mira los sin ISBN). Por defecto: miembros de colección. Debe correr donde estén los ficheros.',
        params: [
            ...
            { nombre: 'cdu', flag: '--cdu', tipo: 'switch', etiqueta: 'Investigar/forzar CDU del Dewey/LCC (crosswalk; mueve carpeta)' },
