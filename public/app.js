...
const consultaCopia=d=>{let s=d.titulo||(d.archivos&&d.archivos[0])||d.nombre||'';
...
async function loadCuar(){try{
  ...
  const listosIds=new Set();cats.forEach(k=>c[k].forEach(d=>{if(d.listo)listosIds.add(d.id);}));
  sanSel=new Set([...sanSel].filter(id=>listosIds.has(id)));
  $('#cuarCount').textContent=cats.reduce((s,k)=>s+c[k].length,0)+' depósito(s)';
  $('#cuarBody').innerHTML=cats.length?cats.map(cat=>{const dup=cat==='duplicados',col=fold.has(cat);
    const listas=c[cat].filter(d=>d.listo);
    const master=listas.length?`<label class="masterchk admin-only" title="Seleccionar/quitar todas las listas"><input type="checkbox" data-selall="${esc(cat)}" ${listas.every(d=>sanSel.has(d.id))?'checked':''}> todas</label>`:'';
    ...
    return `<div class="card" style="margin-bottom:14px">
      ...
      <table><tr><th></th><th>Título / archivo</th><th>Estado</th><th></th></tr>${c[cat].map(d=>{const q=consultaCopia(d);
        const links=(!dup&&FUENTES.length)?`<div class="buscar">🔎 copia: ${FUENTES.map(f=>`<a href="${esc(f.url.replace('{q}',encodeURIComponent(q)))}" target="_blank" rel="noopener">${esc(f.nombre)}</a>`).join(' · ')}</div>`:'';
        const chk=d.listo?`<input type="checkbox" class="admin-only" data-sel="${esc(d.id)}" ${sanSel.has(d.id)?'checked':''}>`:'';
        const estado=dup?`<span class="muted">${esc(d.error||'—')}</span>`
          :d.listo?`<span class="tag ok">✓ lista</span> <span class="muted mono" style="font-size:11px">${esc(d.reemplazo||'')}</span>${d.error_proceso?`<div class="tag bad" style="margin-top:4px">⛔ ${esc(d.error_proceso)}</div>`:''}`
          :`<span class="muted">${esc(d.error||'—')}</span>`;
        let acc=dup?`<button class="btn" data-cmp="${esc(d.id)}">⚖ Comparar</button> <button class="btn admin-only" data-re="${esc(d.id)}">↻ Reingestar</button>`
          :d.listo?`<button class="btn pri admin-only" data-proc="${esc(d.id)}" title="Catalogar esta copia ya">▶ Procesar</button> <button class="btn admin-only" data-rep="${esc(d.id)}" title="Subir otra copia">⤓ Cambiar</button>`
        ...
        <td>${estado}</td>
        <td style="text-align:right;white-space:nowrap">${acc}</td></tr>`;}).join('')}</table></div></div>`;}).join(''):'<div class="empty">Cuarentena vacía 🎉</div>';
  $$('#cuarBody .cuar-head').forEach(h=>h.onclick=e=>{if(e.target.closest('input,button,label,a'))return;
    const cat=h.dataset.fold,f=foldGet();f.has(cat)?f.delete(cat):f.add(cat);foldSet(f);
    h.classList.toggle('col');const body=h.nextElementSibling;if(body)body.style.display=h.classList.contains('col')?'none':'';});
  $$('#cuarBody [data-re]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{const r=await api('/cuarentena/reingestar',{method:'POST',body:JSON.stringify({id:b.dataset.re})});
    if(r.ok){toast(`Reingestados ${r.movidos}/${r.total} → Inbox`);loadCuar();}else{toast(r.motivo,'warn');b.disabled=false;}}catch(e){toast(e.message,'bad');b.disabled=false;}});
  $$('#cuarBody [data-cmp]').forEach(b=>b.onclick=()=>abrirComparador(b.dataset.cmp));
  $$('#cuarBody [data-rep]').forEach(b=>b.onclick=()=>{sanCtx={id:b.dataset.rep};const inp=$('#sanFile');inp.value='';inp.click();});
  $$('#cuarBody [data-proc]').forEach(b=>b.onclick=()=>procesarSaneados([b.dataset.proc]));
  $$('#cuarBody [data-desc]').forEach(b=>b.onclick=async()=>{
    if(!confirm(`¿Descartar «${b.dataset.tit||'este depósito'}»?\n\nSe mueve a la Papelera (carpeta COMPLETA, recuperable) y sale de Cuarentena.`))return;
    b.disabled=true;try{const r=await api('/cuarentena/descartar',{method:'POST',body:JSON.stringify({id:b.dataset.desc})});
      if(r.ok){toast('Descartado → Papelera','warn');loadCuar();}else{toast(r.motivo,'warn');b.disabled=false;}}catch(e){toast(e.message,'bad');b.disabled=false;}});
  $$('#cuarBody [data-desccat]').forEach(b=>b.onclick=async()=>{const cat=b.dataset.desccat;
    const pwd=await modalPassword({titulo:`🗑 Eliminar categoría «${esc(cat)}»`,aviso:`Se moverá TODA la categoría «${esc(cat)}» a la Papelera (recuperable, con las carpetas intactas). Confirma con tu contraseña de administrador.`});
    if(pwd==null)return;b.disabled=true;
    try{const r=await api('/cuarentena/categoria/descartar',{method:'POST',body:JSON.stringify({cat,password:pwd})});
      if(r.ok){toast(`Categoría «${cat}» → Papelera (${r.movidos}/${r.total})`,'warn');loadCuar();}else{toast(r.motivo,'warn');b.disabled=false;}}
    catch(e){toast(e.message,'bad');b.disabled=false;}});
  $$('#cuarBody [data-sel]').forEach(b=>b.onchange=()=>{b.checked?sanSel.add(b.dataset.sel):sanSel.delete(b.dataset.sel);sincMaster(b.closest('.card'));actualizarBotonLote();});
  $$('#cuarBody [data-selall]').forEach(m=>{sincMaster(m.closest('.card'));m.onchange=()=>{const card=m.closest('.card');
    card.querySelectorAll('[data-sel]').forEach(cb=>{cb.checked=m.checked;m.checked?sanSel.add(cb.dataset.sel):sanSel.delete(cb.dataset.sel);});
    m.indeterminate=false;actualizarBotonLote();};});
  actualizarBotonLote();
  if($('#reproAllDup'))$('#reproAllDup').onclick=async()=>{
    if(!confirm('Devuelve TODOS los duplicados al Inbox para re-evaluarlos con la lógica actual (los idénticos se borran, los de otro formato se separan, solo los conflictos reales vuelven). ¿Seguir?'))return;
    const b=$('#reproAllDup');b.disabled=true;
    try{const r=await api('/cuarentena/duplicados/reprocesar-todos',{method:'POST'});toast(`Reprocesando ${r.movidos} fichero(s) de ${r.depositos} depósito(s) → Inbox`);loadCuar();}
    catch(e){toast(e.message,'bad');b.disabled=false;}};
...
async function procesarSaneados(ids){
  ...
  try{const r=await api('/saneamiento/procesar',{method:'POST',body:JSON.stringify({ids})});
    if(!r.ok){toast(r.motivo||'no se pudo iniciar','warn');return;}
    toast(`Procesando ${ids.length} copia(s) en segundo plano…`);seguirSaneamiento();
  }catch(e){toast(e.message,'bad');}
}
function seguirSaneamiento(){if(sanPoll)clearInterval(sanPoll);
  const pintar=s=>{const p=$('#sanProgreso');if(!p)return;
    if(s.enCurso){p.style.display='inline';p.textContent=`⏳ Procesando ${s.hechos+s.fallos+1}/${s.total}…`;}
    else p.style.display='none';};
  const tick=async()=>{try{const s=await api('/saneamiento/estado');pintar(s);
    if(!s.enCurso){clearInterval(sanPoll);sanPoll=null;
      if(s.total)toast(`Saneamiento: ${s.hechos} catalogado(s)${s.fallos?` · ${s.fallos} fallo(s)`:''}`,s.fallos?'warn':'ok');
      loadCuar();}
  }catch{}};
  sanPoll=setInterval(tick,2000);tick();
}
// Selector de fichero para "Reemplazar/Cambiar" → PREPARA la copia (no cataloga; eso es el lote).
$('#sanFile').onchange=async e=>{const file=e.target.files&&e.target.files[0];const ctx=sanCtx;sanCtx=null;if(!file||!ctx)return;
  if(file.size<1024){toast(`Ese fichero parece vacío o dañado (${fmtBytes(file.size)}); elige otra copia`,'warn');return;}
  toast(`Subiendo «${file.name}»…`);
  try{const r=await prepararCopia(ctx.id,file);
    if(r.ok){toast(`✓ Copia lista (${fmtBytes(r.bytes||file.size)}). Pulsa «Procesar» cuando quieras.`);loadCuar();}
...
function cerrarCmp(){$('#cmpScrim').style.display='none';$('#cmpModal').style.display='none';$('#cmpModal').innerHTML='';}
async function abrirComparador(id){
  try{
    const d=await api('/cuarentena/duplicado?id='+encodeURIComponent(id));
    if(!d.ok){toast(d.motivo||'no se pudo comparar','warn');return;}
    const col=(lado,o,win)=>{
      ...
      ].filter(Boolean).join('<br>'):`<span style="color:var(--bad)">${esc(o.motivo||'no disponible')}</span>`;
    ...
    $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
    $('#cmpClose').onclick=cerrarCmp;$('#cmpScrim').onclick=cerrarCmp;
    const resolver=async(quedarse)=>{
      const avisos={
        entrante:'Se retirará el documento catalogado (a la Papelera) y el entrante volverá al Inbox para recatalogarse. ¿Seguir?',
        existente:'El entrante se descartará a la Papelera (recuperable). ¿Seguir?',
        ambos:'Se conservarán LOS DOS: el catalogado queda igual y el entrante volverá al Inbox para catalogarse como documento DISTINTO. ¿Seguir?'};
      if(!confirm(avisos[quedarse]))return;
      try{const r=await api('/cuarentena/duplicado/resolver',{method:'POST',body:JSON.stringify({id,quedarse})});
        if(r.ok){const msg={entrante:`Catalogado retirado · entrante → Inbox (${r.movidos}/${r.total})`,existente:'Entrante descartado a la Papelera',ambos:`Conservados ambos · entrante → Inbox como distinto (${r.movidos}/${r.total})`};toast(msg[quedarse]);cerrarCmp();loadCuar();}
        else toast(r.motivo,'warn');}catch(e){toast(e.message,'bad');}
    };
...
// ── papelera ──
async function loadPap(){try{const p=await api('/papelera');
  $('#papStats').innerHTML=`<div class="stat acc"><div class="ic">♻</div><div class="v">${fmtBytes(p.bytes)}</div><div class="k">Tamaño total</div></div>
  ...
  $('#papBody').innerHTML=p.subcarpetas.length?`<table><tr><th>Subcarpeta</th><th>Ficheros</th><th>Tamaño</th><th></th></tr>
    ${p.subcarpetas.map(s=>`<tr><td class="mono">${esc(s.nombre)}</td><td>${s.ficheros}</td><td>${fmtBytes(s.bytes)}</td>
      <td style="text-align:right"><button class="btn" data-ver="${esc(s.nombre)}">ver</button> <button class="btn bad admin-only" data-del="${esc(s.nombre)}">vaciar</button></td></tr>`).join('')}</table>`:'<div class="empty">Papelera vacía</div>';
  $$('#papBody [data-del]').forEach(b=>b.onclick=async()=>{if(!confirm('¿Vaciar '+b.dataset.del+'? (irreversible)'))return;
    try{await api('/papelera/vaciar',{method:'POST',body:JSON.stringify({sub:b.dataset.del})});toast('Subcarpeta vaciada','warn');loadPap();}catch(e){toast(e.message,'bad');}});
  $$('#papBody [data-ver]').forEach(b=>b.onclick=async()=>{try{const r=await api('/papelera/contenido?sub='+encodeURIComponent(b.dataset.ver));
    alert(b.dataset.ver+'\\n\\n'+(r.ficheros.map(f=>`• ${f.nombre} (${fmtBytes(f.bytes)})`).join('\\n')||'(vacía)'));}catch(e){toast(e.message,'bad');}});
}catch(e){toast(e.message,'bad');}}
$('#papVaciarAll').onclick=async()=>{if(!confirm('¿Vaciar TODA la papelera? (irreversible)'))return;
  try{await api('/papelera/vaciar',{method:'POST',body:JSON.stringify({})});toast('Papelera vaciada','warn');loadPap();}catch(e){toast(e.message,'bad');}};

// ── obras ──
async function purga(ejecutar){const clave=$('#purgaClave').value.trim();if(!clave)return toast('Indica isbn_obra o título','warn');
  if(ejecutar&&!confirm('¿Purgar "'+clave+'"? Elimina la obra y sus tomos (ficheros → Papelera).'))return;
  try{const r=await api('/obras/purgar',{method:'POST',body:JSON.stringify({clave,ejecutar})});
    if(!r.ok)return $('#purgaOut').innerHTML=`<span class="tag bad">${esc(r.motivo)}</span>`;
    if(r.simulacion)$('#purgaOut').innerHTML=`<div class="muted">Obra «${esc(r.obra.titulo)}» — se eliminarían ${r.tomos.length} tomo(s):</div>
      <table>${r.tomos.map(t=>`<tr><td>vol ${t.vol??'?'}</td><td>${esc(t.titulo)}</td><td class="mono muted">${esc(t.isbn||'—')}</td></tr>`).join('')}</table>`;
    else{$('#purgaOut').innerHTML=`<span class="tag ok">Eliminados ${r.eliminados} tomo(s) + la obra. Ya puedes re-soltarla.</span>`;toast('Obra purgada');loadObras();}
  }catch(e){toast(e.message,'bad');}}
$('#purgaSim').onclick=()=>purga(false);$('#purgaExec').onclick=()=>purga(true);
...
async function loadObras(){try{shelf.obra.items=await api('/obras');shelf.obra.sel.clear();pintarShelf('obra');}catch(e){toast(e.message,'bad');}}
async function loadColecciones(){try{const tipo=$('#colsTipo')?$('#colsTipo').value:'';shelf.coleccion.items=await api('/colecciones'+(tipo?('?tipo='+encodeURIComponent(tipo)):''));shelf.coleccion.sel.clear();pintarShelf('coleccion');}catch(e){toast(e.message,'bad');}}
function stackCover(portadas,ph){
  const ps=(portadas||[]).filter(Boolean).slice(0,3);
  if(!ps.length)return `<div class="ph">${ph}</div>`;
  if(ps.length===1)return `<img src="${esc(encUrl(ps[0]))}" loading="lazy" onerror="this.parentNode.innerHTML='<div class=ph>${ph}</div>'">`;
  return `<div class="stack">${ps.slice().reverse().map(p=>`<img src="${esc(encUrl(p))}" loading="lazy">`).join('')}</div>`; // frontal = ps[0] (último)
}
function shelfCard(kind,x){
  const esObra=kind==='obra',nombre=esObra?x.titulo:x.nombre,ph=esObra?'📚':(x.tipo==='revista'?'📰':'📚');
  ...
  const req=(esObra&&x.isbn_obra)?` <button class="rbtn shreq admin-only" data-req="${esc(x._id)}" title="Re-consultar título/sinopsis por ISBN">↻</button>`:'';
...
function pintarShelf(kind){
  const cont=kind==='obra'?$('#obrasBody'):$('#colsBody'); if(!cont)return;
  const st=shelf[kind];
  ...
  const fi=$('#shf_'+kind);if(fi)fi.oninput=()=>{const q=fi.value.toLowerCase();$$(`#${cont.id} .vol[data-${kind}]`).forEach(c=>{c.style.display=(c.dataset.nombre||'').includes(q)?'':'none';});};
  $$(`#${cont.id} .vol[data-${kind}]`).forEach(el=>el.onclick=()=>{kind==='obra'?verObra(el.dataset.obra):verColeccion(el.dataset.coleccion);});
  $$(`#${cont.id} .shelfchk`).forEach(cb=>{cb.onclick=e=>e.stopPropagation();cb.onchange=()=>{cb.checked?st.sel.add(cb.dataset.id):st.sel.delete(cb.dataset.id);const c=cb.closest('.vol');if(c)c.classList.toggle('sel',cb.checked);renderShelfBulk(kind);};});
  $$(`#${cont.id} .shreq`).forEach(b=>{b.onclick=async e=>{e.stopPropagation();b.disabled=true;b.textContent='…';try{const r=await api('/obras/requery',{method:'POST',body:JSON.stringify({id:b.dataset.req})});if(r.ok){toast('Obra actualizada → '+r.titulo);loadObras();}else{toast(r.motivo,'warn');b.disabled=false;b.textContent='↻';}}catch(err){toast(err.message,'bad');b.disabled=false;b.textContent='↻';}};});
  attachRating('#'+cont.id);renderShelfBulk(kind);
}
function renderShelfBulk(kind){
  const el=$('#shbulk_'+kind);if(!el)return;
  const cont=kind==='obra'?$('#obrasBody'):$('#colsBody'),st=shelf[kind];
  if(ROL!=='admin'){el.innerHTML='';return;}
  const ids=st.items.map(x=>x._id),enSel=ids.filter(id=>st.sel.has(id)).length;
  const acc=st.sel.size?`<span style="margin-left:auto"></span><b>${st.sel.size}</b> sel.
  ...
  const all=$('#shAll');all.checked=enSel>0&&enSel===ids.length;all.indeterminate=enSel>0&&enSel<ids.length;
  all.onchange=()=>{const on=all.checked;ids.forEach(id=>on?st.sel.add(id):st.sel.delete(id));$$(`#${cont.id} .shelfchk`).forEach(cb=>cb.checked=on);$$(`#${cont.id} .vol`).forEach(c=>c.classList.toggle('sel',on));renderShelfBulk(kind);};
  if(st.sel.size){const mg=$('#shMerge');if(mg)mg.onclick=()=>shelfFusionar(kind);
    $('#shExpl').onclick=()=>shelfExplotar(kind);$('#shDel').onclick=()=>shelfEliminar(kind);
    $('#shClr').onclick=()=>{st.sel.clear();$$(`#${cont.id} .shelfchk`).forEach(cb=>cb.checked=false);$$(`#${cont.id} .vol.sel`).forEach(c=>c.classList.remove('sel'));renderShelfBulk(kind);};}
}
function shelfSel(kind){const st=shelf[kind];return st.items.filter(x=>st.sel.has(x._id));}
async function shelfFusionar(kind){
  const sel=shelfSel(kind);if(sel.length<2)return;const esObra=kind==='obra',nom=x=>esObra?x.titulo:x.nombre;
  const opts=sel.map((x,i)=>`<label class="pkitem" style="cursor:pointer;gap:8px"><input type="radio" name="mgd" value="${esc(x._id)}"${i===0?' checked':''}> ${esc(nom(x)||'—')}</label>`).join('');
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:480px"><h3 style="margin-top:0">⛙ Fusionar ${sel.length} ${esObra?'obras':'colecciones'}</h3>
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';$('#mgX').onclick=cerrarCmp;$('#cmpScrim').onclick=cerrarCmp;
  $('#mgOk').onclick=async()=>{const destino=($('input[name=mgd]:checked')||{}).value;if(!destino)return;
    try{const r=await api(esObra?'/obras/fusionar':'/colecciones/fusionar',{method:'POST',body:JSON.stringify({ids:sel.map(x=>x._id),destino})});
      if(!r.ok){toast(r.motivo,'bad');return;}cerrarCmp();toast(`${r.movidos} miembro(s) → «${esObra?r.destino.titulo:r.destino.nombre}»; ${r.fusionadas} fusionada(s)`);esObra?loadObras():loadColecciones();
    }catch(e){toast(e.message,'bad');}};
}
async function shelfExplotar(kind){
  const sel=shelfSel(kind);if(!sel.length)return;const esObra=kind==='obra';
  if(!confirm(`Explotar ${sel.length} ${esObra?'obra(s)':'colección(es)'}: sus miembros quedan como documentos sueltos y el grupo desaparece. Los ficheros NO se tocan. ¿Seguir?`))return;
  let lib=0;for(const x of sel){try{const r=await api(esObra?'/obras/explotar':'/colecciones/explotar',{method:'POST',body:JSON.stringify({id:x._id})});if(r.ok)lib+=r.liberados||0;}catch{}}
  toast(`Explotado: ${lib} documento(s) liberado(s)`);esObra?loadObras():loadColecciones();
}
async function shelfEliminar(kind){
  const sel=shelfSel(kind);if(!sel.length)return;const esObra=kind==='obra';
  if(!confirm(`Eliminar las ${esObra?'obras':'colecciones'} VACÍAS de la selección (las que tengan miembros se omiten). ¿Seguir?`))return;
  let del=0,om=0;for(const x of sel){try{const r=await api(esObra?'/obras/eliminar':'/colecciones/eliminar',{method:'POST',body:JSON.stringify({id:x._id})});r.ok?del++:om++;}catch{om++;}}
  toast(`${del} eliminada(s)${om?`, ${om} omitida(s) (no vacías)`:''}`);esObra?loadObras():loadColecciones();
}

// ── detalle: obra → tomos → ficha (drill-down clicable, mobile-first) ──
const recortar=(s,n)=>{s=String(s||'');return s.length>n?s.slice(0,n-1)+'…':s;};
// Distintivo 📶 (etiqueta NFC vinculada) para superponer en cualquier miniatura/portada de un documento.
function nfcBadge(d){return (d&&d.nfc&&(d.nfc.fecha_vinculacion||d.nfc.uid))?'<span class="nfctag" title="Etiqueta NFC vinculada">📶</span>':'';}
function mostrarDetalle(){$$('.page').forEach(s=>s.classList.remove('on'));$('#p-detalle').classList.add('on');
  $('#sidebar').classList.remove('open');$('#scrim').classList.remove('open');window.scrollTo(0,0);}

async function verObra(id){detalle={tipo:'obra',id};mostrarDetalle();apilarVista({v:'det',tipo:'obra',id});$('#title').textContent='Obra';
  $('#p-detalle').innerHTML='<div class="card"><div class="muted">Cargando obra…</div></div>';
  try{const r=await api('/obras/'+encodeURIComponent(id));pintarObra(r);}
  catch(e){$('#p-detalle').innerHTML=`<div class="crumb"><a onclick="go('obras')">← Obras</a></div><div class="empty">${esc(e.message)}</div>`;}}

function tomoCard(d,numero,falta){
  if(falta)return `<div class="vol falta"><div class="cov"><div class="ph">∅</div></div><div class="meta"><div class="n">Tomo ${numero}</div><div class="t">falta</div></div></div>`;
  const cov=d.portada?`<img src="${esc(encUrl(d.portada))}" loading="lazy" onerror="this.parentNode.innerHTML='<div class=ph>📕</div>'">`:'<div class="ph">📕</div>';
  const fmt=(d.formatos||[]).slice(0,3).map(f=>`<span class="fmt">${esc(f)}</span>`).join('');
...
function pintarObra(r){
  const o=r.obra,desc=o.cdu_desc;
  const sub=[o.isbn_obra?'ISBN obra '+o.isbn_obra:'',o.editorial,o.coleccion].filter(Boolean).map(esc).join(' · ')||'—';
  ...
  const vols=r.volumenes.length?r.volumenes.map(v=>tomoCard(v.doc,v.numero,!v.presente)).join(''):'<div class="empty">Sin tomos registrados</div>';
  const sin=r.sin_numero&&r.sin_numero.length?`<div class="card" style="margin-top:14px"><h3 style="color:var(--warn)">Tomos sin número (${r.sin_numero.length})</h3><div class="vol-grid">${r.sin_numero.map(d=>tomoCard(d,'?',false)).join('')}</div></div>`:'';
  $('#p-detalle').innerHTML=head+`<div class="card"><h3>Tomos</h3><div class="vol-grid">${vols}</div></div>`+sin;
  $$('#p-detalle .vol[data-doc]').forEach(el=>el.onclick=()=>verDoc(el.dataset.doc,{obra:{_id:o._id,titulo:o.titulo}}));
  attachRating('#p-detalle');
}

// ── colecciones: cabeceras de revista (números) + series de libros ──

async function verColeccion(id){detalle={tipo:'coleccion',id};mostrarDetalle();apilarVista({v:'det',tipo:'coleccion',id});$('#title').textContent='Colección';
  $('#p-detalle').innerHTML='<div class="card"><div class="muted">Cargando colección…</div></div>';
  try{const r=await api('/colecciones/'+encodeURIComponent(id));pintarColeccion(r);}
  catch(e){$('#p-detalle').innerHTML=`<div class="crumb"><a onclick="go('colecciones')">← Colecciones</a></div><div class="empty">${esc(e.message)}</div>`;}}
...
function pintarColeccion(r){
  const c=r.coleccion,desc=c.cdu_desc,esRev=c.tipo==='revista';
  ...
  const etiq=d=>esRev?(d.clave_numero||(d.año_edicion?String(d.año_edicion):'')||'nº ?'):(d.coleccion_numero?'nº '+d.coleccion_numero:'');
  const cards=r.miembros.length?r.miembros.map(d=>miembroCard(d,etiq(d))).join(''):`<div class="empty">Sin ${esRev?'números':'libros'} registrados</div>`;
  $('#p-detalle').innerHTML=head+`<div class="card"><h3>${esRev?'Números':'Libros'}</h3><div class="vol-grid">${cards}</div></div>`;
  $$('#p-detalle .vol[data-doc]').forEach(el=>el.onclick=()=>verDoc(el.dataset.doc,{coleccion:{_id:c._id,nombre:c.nombre}}));
  attachRating('#p-detalle');
}

async function verDoc(id,ctx){detalle={tipo:'doc',id,ctx};mostrarDetalle();apilarVista({v:'det',tipo:'doc',id,ctx});$('#title').textContent='Ficha';
  $('#p-detalle').innerHTML='<div class="card"><div class="muted">Cargando ficha…</div></div>';
  try{const r=await api('/documentos/'+encodeURIComponent(id));pintarDoc(r,ctx);}
  catch(e){$('#p-detalle').innerHTML=`<div class="crumb"><a onclick="go('obras')">← Obras</a></div><div class="empty">${esc(e.message)}</div>`;}}

// Etiquetas legibles de los roles de ISBN alternativo (otras ediciones).
const ROL_ISBN={tapa_dura:'tapa dura',tapa_blanda:'tapa blanda',ebook:'ebook/digital',obra:'obra completa',volumen:'volumen',barras:'código de barras',otro:'otra edición'};
const ROL_ISBN_OPC=[['tapa_blanda','Tapa blanda'],['tapa_dura','Tapa dura'],['ebook','Ebook / digital'],['obra','Obra completa'],['volumen','Volumen'],['barras','Código de barras'],['otro','Otra edición']];
const CAMPOS_FICHA=[['subtitulo','Subtítulo'],['_autores','Autores'],['_editorial','Editorial'],['_coleccion','Colección'],
  ['año_edicion','Año'],['numero_edicion','Edición'],['idioma','Idioma'],['_formatos','Formatos'],['paginas','Páginas'],['_dimensiones','Tamaño'],
  ['_isbn','ISBN'],['_isbns_alt','Otras ediciones'],['_issn','ISSN'],['lccn','LCCN'],
  ['volumen_numero','Volumen nº'],['volumen_titulo','Título del volumen'],['obra_titulo','Obra'],['_isbn_obra','ISBN obra'],
  ['coleccion_numero','Nº en colección'],['numero_issue','Número'],['mes_publicacion','Mes'],
  ['_estado','Estado'],['_ubicacion','Ubicación'],['nombre_archivo','Archivo'],['_ruta','Ruta'],['hash_contenido','Hash'],
  ['_ingreso','Ingresado'],['_actualizado','Actualizado']];
...
function filaClas(x,sinSist){
  ...
  const cnt=(x.n>1)?`<a class="cntclas" data-sist="${esc(x.sistema)}" data-cod="${esc(x.codigo)}" title="Ver los ${x.n} documentos">${x.n}</a>`:`<span class="muted">${x.n||1}</span>`;
...
function attachClas(scope){
  ...
  $$(scope+' .cntclas').forEach(b=>b.onclick=()=>filtrarPorClasificacion(b.dataset.sist,b.dataset.cod));
}
async function infoClasificacion(sist,cod){
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:520px"><h3 style="margin-top:0">${esc((sist||'').toUpperCase())} <span class="mono">${esc(cod)}</span></h3><div id="iclasBody" class="muted">Cargando…</div><div style="margin-top:14px;text-align:right"><button class="btn" id="iclasClose">Cerrar</button></div></div>`;
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';$('#iclasClose').onclick=cerrarCmp;$('#cmpScrim').onclick=cerrarCmp;
  try{const r=await api('/clasificacion?sistema='+encodeURIComponent(sist)+'&codigo='+encodeURIComponent(cod));const b=$('#iclasBody');if(!b)return;b.classList.remove('muted');
    b.innerHTML=r.ok?`${r.titulo?`<div style="font-weight:600;margin-bottom:8px">${esc(r.titulo)}</div>`:''}<div style="line-height:1.65;font-size:13px">${esc(r.descripcion||'Sin descripción disponible (la IA no la generó).')}</div>`:`<span class="muted">${esc(r.motivo||'no disponible')}</span>`;
  }catch(e){const b=$('#iclasBody');if(b)b.textContent=e.message;}
}
...
function irBusquedaFiltro(extra){
  estadoBusqueda.extra=extra;estadoBusqueda.page=1;
  if($('#sqQ'))$('#sqQ').value='';if($('#sqCdu'))$('#sqCdu').value='';
  go('search');
}
// Búsqueda por TEXTO libre (drill de ISSN/ISBN desde la ficha): pone la consulta en la caja y busca.
function buscarTexto(q){
  estadoBusqueda.extra=null;estadoBusqueda.page=1;
  go('search');
...
function filtrarPorClasificacion(sist,cod){irBusquedaFiltro({clasSistema:sist,clasCodigo:cod,etiqueta:`${sist.toUpperCase()} ${cod}`});}
function filtrarColeccion(id,nombre){irBusquedaFiltro({coleccion:id,etiqueta:`Colección: ${nombre}`});}
function filtrarCatalogo(filtro,etiqueta){irBusquedaFiltro({filtro,etiqueta:etiqueta||filtro});}
function filtrarPorDia(dia){irBusquedaFiltro({dia,etiqueta:`Ingresados el ${dia}`});}

function pintarDoc(r,ctx){
  const d=r.doc,vo=ctx&&ctx.obra,vc=ctx&&ctx.coleccion,vol=ctx&&ctx.volver;
  let inicio,back;
  if(vo){inicio=`<a onclick="go('obras')">Obras</a> › <a onclick="verObra('${esc(vo._id)}')">${esc(recortar(vo.titulo,32))}</a>`;back=`verObra('${esc(vo._id)}')`;}
  else if(vc){inicio=`<a onclick="go('colecciones')">Colecciones</a> › <a onclick="verColeccion('${esc(vc._id)}')">${esc(recortar(vc.nombre,32))}</a>`;back=`verColeccion('${esc(vc._id)}')`;}
  else if(vol){inicio=`<a onclick="go('${esc(vol)}')">${esc(ctx.etiqueta||vol)}</a>`;back=`go('${esc(vol)}')`;}
  else{inicio=`<a onclick="go('obras')">Obras</a>`;back=`go('obras')`;}
  ...
  const fmtFecha=v=>{const t=new Date(v);return isNaN(t)?esc(v):t.toLocaleString('es-ES');};
  const especiales={
    _autores:r.autores&&r.autores.length?r.autores.map(esc).join(', '):null,
    _editorial:r.editorial?esc(r.editorial):null,
    _coleccion:r.coleccion?((r.coleccion_id?`<a class="rowlink" data-colid="${esc(r.coleccion_id)}" data-colnom="${esc(r.coleccion)}">${esc(r.coleccion)}</a>`:esc(r.coleccion))+(d.coleccion_numero?` · nº ${esc(d.coleccion_numero)}`:'')):null,
    _formatos:(d.formatos||[]).length?d.formatos.map(f=>`<span class="fmt">${esc(f)}</span>`).join(''):null,
    _estado:d.estado_verificacion?`<span class="tag ${d.estado_verificacion==='completado'?'ok':'warn'}">${esc(d.estado_verificacion)}</span>`:null,
    _ubicacion:d.ubicacion?(esc([d.ubicacion.ambito,d.ubicacion.estanteria].filter(Boolean).join(' · '))||null):null,
    _dimensiones:(d.ancho_cm&&d.alto_cm)?`${esc(String(d.ancho_cm).replace('.',','))} × ${esc(String(d.alto_cm).replace('.',','))} cm`:null,
    ...
    _ruta:d.ruta_base?(()=>{const rb=d.ruta_base.replace(/^\/recursos/,'');return `<span class="mono">${esc(rb)}</span> <button class="rbtn copybtn" data-copy="${esc(rb)}" title="Copiar la ruta">📋</button>`;})():null,
    obra_titulo:r.obra?`<a onclick="verObra('${esc(r.obra._id)}')" class="rowlink">${esc(r.obra.titulo||d.obra_titulo||'(obra)')}</a>`:(d.obra_titulo?esc(d.obra_titulo):null),
    ...
    _issn:d.issn?`<a class="rowlink" data-q="${esc(d.issn)}" title="Ver todo lo que comparte este ISSN">${esc(d.issn)}</a>`:null,
    _isbn:d.isbn?`<a class="rowlink" data-q="${esc(d.isbn)}" title="Ver todo lo que comparte este ISBN">${esc(d.isbn)}</a>`:null,
    // Otras ediciones (e-ISBN, tapa dura/blanda, obra completa, código de barras…), drillables.
    _isbns_alt:(d.isbns_alternativos&&d.isbns_alternativos.length)?d.isbns_alternativos.map(a=>`<a class="rowlink" data-q="${esc(a.isbn)}" title="Buscar este ISBN">${esc(a.isbn)}</a> <span class="muted" style="font-size:11px">(${esc(ROL_ISBN[a.rol]||a.rol||'otra')})</span>`).join('<br>'):null,
    _isbn_obra:d.isbn_obra?`<a class="rowlink" data-q="${esc(d.isbn_obra)}" title="Ver todo lo que comparte este ISBN de obra">${esc(d.isbn_obra)}</a>`:null,
  };
  const valor=k=>{if(k in especiales)return especiales[k];let v=d[k];if(v==null||v===''||(Array.isArray(v)&&!v.length))return null;return esc(v);};
  const dl=CAMPOS_FICHA.map(([k,lab])=>{const v=valor(k);return v?`<dt>${lab}</dt><dd>${v}</dd>`:'';}).join('');
  const sinopsis=d.sinopsis?`<div class="card" style="margin-top:14px"><h3>Sinopsis</h3><p class="sinopsis-text">${esc(d.sinopsis)}</p></div>`:'';
  const palabras=(d.palabras_clave||[]).length?`<div style="margin-top:12px;display:flex;flex-wrap:wrap;gap:5px">${d.palabras_clave.map(p=>`<span class="tag mut">${esc(p)}</span>`).join('')}</div>`:'';
  const clas=(r.clasificaciones||[]).length?`<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--line)"><div class="muted" style="font-size:11px;text-transform:uppercase;letter-spacing:.5px;font-weight:600;margin-bottom:6px">Clasificación</div><table class="clastab">${r.clasificaciones.map(x=>filaClas(x)).join('')}</table></div>`:'';
  const alertas=(d.alertas_agente||[]).length?`<details style="margin-top:10px"><summary class="muted" style="cursor:pointer;font-size:12px">Notas del agente (${d.alertas_agente.length})</summary><div style="margin-top:6px">${d.alertas_agente.map(a=>`<div class="muted" style="font-size:12px">• ${esc(a)}</div>`).join('')}</div></details>`:'';
  const imgs=(r.imagenes&&r.imagenes.length)?r.imagenes:(r.portada?[{ruta:r.portada,tipo:'portada'}]:[]);
  const nfcOv=nfcBadge(d);
  const carrusel=imgs.length?`<div class="carousel" style="position:relative">${nfcOv}<div class="track" id="carTrack">${imgs.map(im=>`<img src="${esc(encUrl(im.ruta))}" loading="lazy" onclick="window.open('${esc(encUrl(im.ruta))}','_blank')">`).join('')}</div>${imgs.length>1?`<button class="cnav prev" onclick="carMove(-1)">‹</button><button class="cnav next" onclick="carMove(1)">›</button><div class="cdots" id="carDots">1 / ${imgs.length}</div>`:''}</div>`:`<div class="filebox" style="position:relative">${nfcOv}<div class="ic">🖼️</div><div class="muted">Sin imágenes</div></div>`;
  ...
  const origen=_fisico
    ? (_tag?'Estos son los datos que lleva grabada la etiqueta NFC de este ejemplar.':'Ficha reconstruida a partir de la base de datos (este ejemplar aún no tiene etiqueta NFC).')
    : 'Ejemplar digital: en vez de ubicación física se ofrece la descarga.';
  const filasFmin=[['Autor',especiales._autores],['Editorial',especiales._editorial],['Colección',especiales._coleccion],
    ['CDU',d.cdu?`<span class="mono">${esc(d.cdu)}</span>`:null],['ISBN',especiales._isbn],['ISSN',especiales._issn]]
    .filter(p=>p[1]).map(p=>`<dt>${p[0]}</dt><dd>${p[1]}</dd>`).join('');
  ...
  const fmin=fichaMinima({
    titulo:d.titulo, subtitulo:d.subtitulo||subDoc, esDigital:!_fisico, exlibris:EX_LIBRIS,
    descargaUrl:r.archivo_url?encUrl(r.archivo_url):'', descargaNombre:r.nombre_archivo||'',
    estrellasHTML:ratingBar('documentos',d._id,d.valoracion,d.nsfw)+' '+badgesDoc(d),
    datosHTML:filasFmin, ubicacionHTML:ubicFmin, origen});
  ...
  const secLect=lector?`<details class="card foldcard" id="lectDet" open style="margin-top:14px"><summary>📖 Leer / archivo</summary><div style="margin-top:10px">${lector}</div></details>`:'';
  ...
  const secSin=d.sinopsis?`<details class="card foldcard" open style="margin-top:14px"><summary>📝 Sinopsis</summary><p class="sinopsis-text" style="margin-top:10px">${esc(d.sinopsis)}</p></details>`:'';
  // Imágenes y sinopsis DESPLEGADAS y ANTES de las acciones; el resto (lectura, catalográficos) plegado, después.
  $('#p-detalle').innerHTML=`${crumb}<div style="margin:2px 0 12px"><button class="det-back" title="Volver" onclick="${back}">←</button></div>${fmin}${secImg}${secSin}${secAcc}${secLect}${secCat}`;
  attachClas('#p-detalle');
  attachRating('#p-detalle');
  $$('#p-detalle .copybtn').forEach(b=>b.onclick=e=>{e.stopPropagation();copiar(b.dataset.copy);});
  {const cf=$('#actConf'),ce=$('#actEnr'),cr=$('#actRepr');
   ...
   const cd=$('#actDel');if(cd)cd.onclick=()=>fichaEliminar(d._id);
   const ce2=$('#actEdit');if(ce2)ce2.onclick=()=>fichaEditar(d,r);
   const ci=$('#actImgs');if(ci)ci.onclick=()=>editarImagenes(d._id,r.imagenes||(r.portada?[{ruta:r.portada,tipo:'portada'}]:[]));
   const cm2=$('#actMedir');if(cm2)cm2.onclick=()=>medirDimensiones(d._id,r.imagenes||(r.portada?[{ruta:r.portada,tipo:'portada'}]:[]));
   ...
   const cn=$('#actNfc');if(cn&&_fisico&&('NDEFReader' in window)){cn.style.display='';cn.onclick=()=>grabarNFC(d,r);}
   const cs=$('#actShare');if(cs)cs.onclick=()=>compartirDoc(d);
   const uc=$('#ubicChip');if(uc)uc.onclick=()=>{const u=d.ubicacion||{};estadoBusqueda.extra={ambito:u.ambito,estanteria:(u.estanteria&&u.estanteria!=='Sin asignar')?u.estanteria:undefined,etiqueta:'📍 '+_txtUbic(d)};estadoBusqueda.page=1;go('search');};}
  ...
  carIdx=0;const tr=$('#carTrack');
  if(tr&&tr.children.length>1)tr.onscroll=()=>{const i=Math.round(tr.scrollLeft/tr.clientWidth);if(i!==carIdx){carIdx=i;const dd=$('#carDots');if(dd)dd.textContent=(i+1)+' / '+tr.children.length;}};
  ...
  const ld=$('#lectDet');
  if(ld){let hecho=false;const arranca=()=>{if(!hecho){hecho=true;initLector();}};
    if(ld.open)arranca(); else ld.addEventListener('toggle',()=>{if(ld.open)arranca();});}
}
...
async function iniciarLectorComic(id){
  const wrap=$('#comicWrap'),msg=$('#comicMsg'),img=$('#comicImg');if(!wrap||!img)return;
  if(comicState){for(const u of comicState.blobs.values())URL.revokeObjectURL(u);} // liberar el cómic anterior
  comicState={id,n:0,total:0,blobs:new Map()};
  try{
    const r=await api('/documentos/'+encodeURIComponent(id)+'/paginas');
    const total=r.paginas||0;
    if(!total){if(msg)msg.textContent='No se pudieron extraer las páginas de este documento.';return;}
    comicState.total=total;$('#comicTotal').textContent=total;
    $('#comicBar').style.display='';$('#comicFs').style.display='';
    if(total>1){$('#comicPrev').style.display='';$('#comicNext').style.display='';}
    $('#comicPrev').onclick=()=>comicIr(-1);$('#comicNext').onclick=()=>comicIr(1);
...
async function comicBlob(n){
  ...
  const res=await fetch('/api/documentos/'+encodeURIComponent(comicState.id)+'/paginas/'+n,{headers:TOKEN?{Authorization:'Bearer '+TOKEN}:{}});
  if(!res.ok)throw new Error('no se pudo cargar la página '+(n+1));
  const u=URL.createObjectURL(await res.blob());comicState.blobs.set(n,u);return u;
}
async function comicMostrar(n){
  const img=$('#comicImg'),msg=$('#comicMsg');if(!img||!comicState)return;
  try{const u=await comicBlob(n);img.src=u;comicState.n=n;$('#comicCur').textContent=n+1;if(msg)msg.style.display='none';
    if(n+1<comicState.total)comicBlob(n+1).catch(()=>{}); // prefetch de la siguiente
  }catch(e){if(msg){msg.style.display='';msg.textContent=e.message;}}
}
function comicIr(d){if(!comicState)return;const n=Math.max(0,Math.min(comicState.total-1,comicState.n+d));if(n!==comicState.n)comicMostrar(n);}

let carIdx=0;
function carMove(dir){const t=$('#carTrack');if(!t)return;const n=t.children.length;carIdx=Math.max(0,Math.min(n-1,carIdx+dir));
  t.scrollTo({left:t.clientWidth*carIdx,behavior:'smooth'});const dd=$('#carDots');if(dd)dd.textContent=(carIdx+1)+' / '+n;}
...
function ratingBar(ent,id,v,nsfw){v=Number(v)||0;const admin=ROL==='admin';
  const st=[5,4,3,2,1].map(n=>`<span class="st${n<=v?' on':''}" data-v="${n}">★</span>`).join(''); // 5→1 + row-reverse: relleno al pasar el ratón
  // Invitado: solo VE la valoración (estrellas de solo lectura, sin botones de quitar/NSFW ni clic).
  if(!admin)return `<span class="ratebar ro"><span class="stars ro" title="Valoración: ${v}/5">${st}</span></span>`;
  return `<span class="ratebar" data-ent="${ent}" data-id="${esc(id)}" data-v="${v}">`
    ...
    +`</span>`;}
function attachRating(scope){$$(scope+' .ratebar:not(.ro)').forEach(bar=>{
  const ent=bar.dataset.ent,id=bar.dataset.id;
  const setVal=async nv=>{try{await api('/'+ent+'/'+encodeURIComponent(id)+'/valoracion',{method:'POST',body:JSON.stringify({valoracion:nv})});
      bar.dataset.v=nv;bar.querySelectorAll('.st').forEach(x=>x.classList.toggle('on',Number(x.dataset.v)<=nv));
      toast(nv?('Valorado con '+nv+'★'):'Valoración quitada');}catch(e){toast(e.message,'bad');}};
  bar.querySelectorAll('.st').forEach(s=>s.onclick=e=>{e.stopPropagation();const n=Number(s.dataset.v),cur=Number(bar.dataset.v)||0;setVal(n===cur?0:n);});
  const clr=bar.querySelector('.rclear');if(clr)clr.onclick=e=>{e.stopPropagation();setVal(0);};
  const nb=bar.querySelector('.rnsfw');if(nb)nb.onclick=async e=>{e.stopPropagation();const nv=!nb.classList.contains('on');
    try{const r=await api('/'+ent+'/'+encodeURIComponent(id)+'/nsfw',{method:'POST',body:JSON.stringify({nsfw:nv})});nb.classList.toggle('on',nv);
      const ext=r.propagado?` (${r.propagado} miembro${r.propagado===1?'':'s'})`:'';toast((nv?'Marcado NSFW 🔞':'NSFW quitado')+ext);
      if(r.propagado&&detalle&&(detalle.tipo==='obra'||detalle.tipo==='coleccion')){detalle.tipo==='obra'?verObra(detalle.id):verColeccion(detalle.id);} // refrescar badges de los miembros
    }catch(err){toast(err.message,'bad');}};
});}

// ── acciones de la ficha: Conformar / Enriquecer (con resumen de cambios) y Reprocesar ──
async function fichaAccion(tipo,id,btn){
  const lbl=btn.innerHTML;btn.disabled=true;btn.textContent=tipo==='conformar'?'⏳ Conformando…':'⏳ Enriqueciendo…';
  try{const r=await api('/documentos/'+encodeURIComponent(id)+'/'+tipo,{method:'POST',body:'{}'});
    if(!r.ok){toast(r.motivo||'sin cambios','warn');return;}
...
function mostrarCambios(titulo,cambios){
  const fila=c=>`<tr><td class="k">${esc(c.campo)}</td><td>${(c.de!=null&&c.de!=='')?`<span class="de">${esc(recortar(String(c.de),80))}</span><br>`:''}<span class="a">${esc(recortar(String(c.a),140))||'—'}</span></td></tr>`;
  const cuerpo=cambios.length?`<table class="chgtab">${cambios.map(fila).join('')}</table>`:'<div class="muted">Sin cambios: el documento ya estaba al día.</div>';
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:540px"><h3 style="margin-top:0">${esc(titulo)} — ${cambios.length} cambio${cambios.length===1?'':'s'}</h3>${cuerpo}<div style="margin-top:14px;text-align:right"><button class="btn" id="chgClose">Cerrar</button></div></div>`;
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';$('#chgClose').onclick=cerrarCmp;$('#cmpScrim').onclick=cerrarCmp;}
async function fichaReprocesar(id){
  const pw=await modalPassword({titulo:'♻️ Reprocesar documento',aviso:'Se borrará el documento, su fichero volverá al <b>Inbox</b> para re-catalogarse y su carpeta actual (sidecars e imágenes) irá a la <b>Papelera</b> (recuperable). El Vigilante debe estar activo para que se vuelva a procesar.'});
  if(pw==null)return;
  try{const r=await api('/documentos/'+encodeURIComponent(id)+'/reprocesar',{method:'POST',body:JSON.stringify({password:pw})});
    if(!r.ok){toast(r.motivo,'bad');return;}
    toast('Reprocesando: «'+r.inbox+'» devuelto al Inbox');go('search');
  }catch(e){toast(e.message,'bad');}}
async function fichaEliminar(id){
  const pw=await modalPassword({titulo:'🗑 Eliminar documento',aviso:'Se borrará el registro de la base y su carpeta (sidecars e imágenes) se moverá a la <b>Papelera</b> (recuperable). El fichero NO vuelve al Inbox.'});
  if(pw==null)return;
  try{const r=await api('/documentos/'+encodeURIComponent(id)+'/eliminar',{method:'POST',body:JSON.stringify({password:pw})});
    if(!r.ok){toast(r.motivo,'bad');return;}
    toast('Documento eliminado (su carpeta está en la Papelera)');go('search');
  }catch(e){toast(e.message,'bad');}}
// ════════ GESTIÓN DE IMÁGENES DEL CARRUSEL (reordenar/borrar/añadir + editor rotar/recortar/perspectiva) ════════
const fileADataURL=blob=>new Promise((res,rej)=>{const fr=new FileReader();fr.onload=()=>res(fr.result);fr.onerror=rej;fr.readAsDataURL(blob);});
let _imgState=null;
function editarImagenes(id, imagenes){ _imgState={ id, imgs:(imagenes||[]).map(im=>({...im})) }; pintarGestorImagenes(); }
async function apiImg(op, body){
  const {id}=_imgState;
  try{const r=await api('/documentos/'+encodeURIComponent(id)+'/imagenes/'+op,{method:'POST',body:JSON.stringify(body)});
    if(!r.ok){toast(r.motivo||'error','bad');return false;}
    _imgState.imgs=(r.imagenes||[]).map(im=>({...im}));_imgState.cambiado=true;pintarGestorImagenes();return true;
  }catch(e){toast(e.message,'bad');return false;}
}
function pintarGestorImagenes(){
  const {id,imgs}=_imgState;
  const filas=imgs.map((im,i)=>`<div style="display:flex;gap:8px;align-items:center;padding:6px 0;border-top:1px solid var(--line)">
...
function _gauss(A,b){const n=b.length,M=A.map((r,i)=>[...r,b[i]]);for(let c=0;c<n;c++){let p=c;for(let r=c+1;r<n;r++)if(Math.abs(M[r][c])>Math.abs(M[p][c]))p=r;[M[c],M[p]]=[M[p],M[c]];const pv=M[c][c]||1e-9;for(let r=0;r<n;r++){if(r===c)continue;const f=M[r][c]/pv;for(let k=c;k<=n;k++)M[r][k]-=f*M[c][k];}}return M.map((r,c)=>r[n]/(r[c]||1e-9));}
function _homografia(src,dst){const A=[],b=[];for(let i=0;i<4;i++){const[x,y]=src[i],[u,v]=dst[i];A.push([x,y,1,0,0,0,-u*x,-u*y]);b.push(u);A.push([0,0,0,x,y,1,-v*x,-v*y]);b.push(v);}const h=_gauss(A,b);return[h[0],h[1],h[2],h[3],h[4],h[5],h[6],h[7],1];}
function _mapH(H,x,y){const d=H[6]*x+H[7]*y+H[8];return[(H[0]*x+H[1]*y+H[2])/d,(H[3]*x+H[4]*y+H[5])/d];}
...
function _rgb2hs(r,g,b){const mx=Math.max(r,g,b),mn=Math.min(r,g,b),d=mx-mn;let h=0;
  if(d>0){if(mx===r)h=(((g-b)/d)%6+6)%6;else if(mx===g)h=(b-r)/d+2;else h=(r-g)/d+4;h*=60;}
  return {h,s:mx>0?d/mx:0,mx};}
function _normCal(c){if(c&&c.h===undefined&&c.r!==undefined){const o=_rgb2hs(c.r,c.g,c.b);c.h=o.h;c.s=o.s;c.crom=o.s>=0.12;c.sum=c.sum||(c.r+c.g+c.b);}return c;}
function cargarTapeteCal(){try{_tapeteCal=_normCal(JSON.parse(localStorage.getItem('tapete_cal')||'null'));}catch(_){_tapeteCal=null;}}
cargarTapeteCal();
function limpiarTapeteCal(){localStorage.removeItem('tapete_cal');_tapeteCal=null;}
...
function _coincideTapete(r,g,b,c){
  ...
  if(c.crom){let dh=Math.abs(o.h-c.h);if(dh>180)dh=360-dh; return dh<=30 && o.s>=Math.max(0.12,c.s*0.5);}
...
function _verdeMat(r,g,b){
  ...
  const mn=r<b?r:b; return (g-r)>10 && g>=(b-25) && !(g>170&&(g-mn)<g*0.18);
}
...
async function calibrarTapete(file){
  ...
  const maxD=600, sc=Math.min(1,maxD/Math.max(img.naturalWidth,img.naturalHeight));
  const w=Math.max(1,Math.round(img.naturalWidth*sc)),h=Math.max(1,Math.round(img.naturalHeight*sc)),N=w*h;
  ...
  const med=hh=>{let a=0;for(let v=0;v<256;v++){a+=hh[v];if(a>=N/2)return v;}return 128;};
  const r=med(hr),g=med(hg),b=med(hb), o=_rgb2hs(r,g,b);
  const cal={r,g,b,h:o.h,s:o.s,sum:r+g+b,crom:o.s>=0.12};
  localStorage.setItem('tapete_cal',JSON.stringify(cal)); _tapeteCal=cal; return cal;
}
// Estado de la calibración bajo el botón: muestra el color fijado + enlace para quitarla (vuelve al auto).
function pintarTapeteCalEstado(){const e=document.getElementById('inTapeteCalEstado');if(!e)return;
  if(_tapeteCal){const c=_tapeteCal;
    ...
    const clr=document.getElementById('inTapeteCalClr');if(clr)clr.onclick=()=>{limpiarTapeteCal();pintarTapeteCalEstado();toast('Calibración borrada (vuelve a la detección verde/teal automática)');};}
...
async function verMascaraTapete(file){
  ...
  const work=document.createElement('canvas');work.width=iw;work.height=ih;work.getContext('2d').drawImage(img,0,0);
  const quad=detectarBordesVerde(work), pxcm=detectarRejillaPxCm(work);
  let dims=null; if(quad&&pxcm){const W=(_dist(quad[0],quad[1])+_dist(quad[3],quad[2]))/2/pxcm,H=(_dist(quad[0],quad[3])+_dist(quad[1],quad[2]))/2/pxcm;dims={W,H};}
  const maxD=680, sc=Math.min(1,maxD/Math.max(iw,ih));
  const w=Math.max(1,Math.round(iw*sc)),h=Math.max(1,Math.round(ih*sc)),N=w*h;
  const cv=document.createElement('canvas');cv.width=w;cv.height=h;const ctx=cv.getContext('2d');ctx.drawImage(img,0,0,w,h);
  try{URL.revokeObjectURL(img.src);}catch(_){}
  const im=ctx.getImageData(0,0,w,h),d=im.data;let nv=0;
  for(let i=0;i<N;i++){if(_verdeMat(d[i*4],d[i*4+1],d[i*4+2])){nv++;d[i*4]=255;d[i*4+1]=40;d[i*4+2]=40;}}
  ctx.putImageData(im,0,0);
  if(quad){ctx.strokeStyle='#28ff90';ctx.lineWidth=3;ctx.beginPath();quad.forEach((p,k)=>{const X=p[0]*sc,Y=p[1]*sc;k?ctx.lineTo(X,Y):ctx.moveTo(X,Y);});ctx.closePath();ctx.stroke();
    quad.forEach(p=>{ctx.beginPath();ctx.arc(p[0]*sc,p[1]*sc,5,0,7);ctx.fillStyle='#28ff90';ctx.fill();});}
  const pct=Math.round(nv/N*100), url=cv.toDataURL('image/jpeg',0.85), c=_tapeteCal;
  const medida=dims?`${dims.W.toFixed(1).replace('.',',')}×${dims.H.toFixed(1).replace('.',',')} cm`:(quad?'libro ✓ (rejilla ✗)':'libro ✗');
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:520px;max-height:92vh;overflow:auto"><h3 style="margin-top:0">🔍 Probar tapete</h3>
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
  const x=$('#vmX');if(x)x.onclick=cerrarCmp;$('#cmpScrim').onclick=cerrarCmp;
}
// Fracción de píxeles de tapete (diagnóstico para calibrar): submuestrea y cuenta verdes.
function fraccionVerde(canvas){const maxD=400,sc=Math.min(1,maxD/Math.max(canvas.width,canvas.height));
  const w=Math.max(1,Math.round(canvas.width*sc)),h=Math.max(1,Math.round(canvas.height*sc)),N=w*h;
  const t=document.createElement('canvas');t.width=w;t.height=h;t.getContext('2d').drawImage(canvas,0,0,w,h);
  const d=t.getContext('2d').getImageData(0,0,w,h).data;let nv=0;
  for(let i=0;i<N;i++)if(_verdeMat(d[i*4],d[i*4+1],d[i*4+2]))nv++; return nv/N;}
function detectarBordesVerde(canvas){
  const maxD=720, sc=Math.min(1,maxD/Math.max(canvas.width,canvas.height));
  const w=Math.max(1,Math.round(canvas.width*sc)),h=Math.max(1,Math.round(canvas.height*sc)),N=w*h;
  const t=document.createElement('canvas');t.width=w;t.height=h;t.getContext('2d').drawImage(canvas,0,0,w,h);
  const d=t.getContext('2d').getImageData(0,0,w,h).data;
  const verde=new Uint8Array(N); let nVerde=0;
  for(let i=0;i<N;i++){const v=_verdeMat(d[i*4],d[i*4+1],d[i*4+2])?1:0;verde[i]=v;nVerde+=v;}
  ...
  const r=2, tmp=new Uint8Array(N), mat=new Uint8Array(N);
  for(let y=0;y<h;y++){const o=y*w;for(let x=0;x<w;x++){let v=0;for(let k=-r;k<=r;k++){const xx=x+k;if(xx>=0&&xx<w&&verde[o+xx]){v=1;break;}}tmp[o+x]=v;}}
  for(let x=0;x<w;x++)for(let y=0;y<h;y++){let v=0;for(let k=-r;k<=r;k++){const yy=y+k;if(yy>=0&&yy<h&&tmp[yy*w+x]){v=1;break;}}mat[y*w+x]=v;}
  // flood-fill del EXTERIOR (no-tapete alcanzable desde los bordes) sobre el tapete YA SÓLIDO.
  const fuera=new Uint8Array(N), pila=[];
  const meter=i=>{if(!mat[i]&&!fuera[i]){fuera[i]=1;pila.push(i);}};
  for(let x=0;x<w;x++){meter(x);meter((h-1)*w+x);} for(let y=0;y<h;y++){meter(y*w);meter(y*w+w-1);}
  while(pila.length){const i=pila.pop(),x=i%w,y=(i/w)|0;if(x>0)meter(i-1);if(x<w-1)meter(i+1);if(y>0)meter(i-w);if(y<h-1)meter(i+w);}
  ...
  const comp=new Int32Array(N); let id=0,bestId=0,bestSz=0;
  for(let s=0;s<N;s++){
    if(mat[s]||fuera[s]||comp[s])continue;
    id++; let sz=0; const st=[s]; comp[s]=id;
    while(st.length){const i=st.pop();sz++;const x=i%w,y=(i/w)|0;
  ...
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){const i=y*w+x; if(!dela(i))continue;
    let nb=0; if(dela(i-1))nb++; if(dela(i+1))nb++; if(dela(i-w))nb++; if(dela(i+w))nb++; if(nb<3)continue;
    const s=x+y, df=x-y;
    if(tl===null||s<tl[2])tl=[x,y,s]; if(br===null||s>br[2])br=[x,y,s];
    if(bl===null||df<bl[2])bl=[x,y,df]; if(tr===null||df>tr[2])tr=[x,y,df];
  }
...
function _quadGenericoDeImagen(img){
  const w=img.width,h=img.height,d=img.data,N=w*h; if(w<40||h<40)return null;
  const g=new Float32Array(N);
  for(let i=0;i<N;i++)g[i]=0.299*d[i*4]+0.587*d[i*4+1]+0.114*d[i*4+2];
  const gx=new Float32Array(N),gy=new Float32Array(N),mag=new Float32Array(N); let magSum=0;
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){const i=y*w+x;
    const a=g[i-w-1],b=g[i-w],c=g[i-w+1],dd=g[i-1],f=g[i+1],p=g[i+w-1],q=g[i+w],r=g[i+w+1];
    const sx=(c+2*f+r)-(a+2*dd+p), sy=(p+2*q+r)-(a+2*b+c);
    gx[i]=sx;gy[i]=sy;const m=Math.abs(sx)+Math.abs(sy);mag[i]=m;magSum+=m;}
  const thr=Math.max(40,(magSum/N)*2.5);           // umbral de borde (media·k)
  const SL=0.6,NS=25,ds=(2*SL)/(NS-1),slope=k=>(-SL+k*ds);
  const accH=[],accV=[]; for(let k=0;k<NS;k++){accH.push(new Float32Array(h+2));accV.push(new Float32Array(w+2));}
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){const i=y*w+x; if(mag[i]<thr)continue;
    if(Math.abs(gy[i])>=Math.abs(gx[i])){ for(let k=0;k<NS;k++){const bb=Math.round(y-slope(k)*x); if(bb>=0&&bb<=h)accH[k][bb]+=mag[i];} }
    else { for(let k=0;k<NS;k++){const bb=Math.round(x-slope(k)*y); if(bb>=0&&bb<=w)accV[k][bb]+=mag[i];} }
  }
  // Extrae líneas (picos con NMS por coordenada central) y devuelve {m,b,cc(score-centro),s}.
  const lineas=(acc,blen,centro)=>{
    let mx=0; for(let k=0;k<NS;k++)for(let b=0;b<=blen;b++)if(acc[k][b]>mx)mx=acc[k][b];
    if(mx<=0)return []; const cut=mx*0.33, cand=[];
    for(let k=0;k<NS;k++)for(let b=0;b<=blen;b++){const s=acc[k][b]; if(s<cut)continue;
      let lm=true; for(let db=-3;db<=3;db++){const bb=b+db; if(bb>=0&&bb<=blen&&acc[k][bb]>s){lm=false;break;}} if(!lm)continue;
      cand.push({m:slope(k),b,s,cc:slope(k)*centro+b});}
    cand.sort((a,b)=>b.s-a.s); const out=[];
    for(const c of cand){ if(out.every(o=>Math.abs(o.cc-c.cc)>blen*0.10))out.push(c); }
    return out;
  };
  const H=lineas(accH,h,w/2), V=lineas(accV,w,h/2);
  if(H.length<2||V.length<2)return null;
  const top=H.reduce((a,b)=>b.cc<a.cc?b:a), bot=H.reduce((a,b)=>b.cc>a.cc?b:a);
  const izq=V.reduce((a,b)=>b.cc<a.cc?b:a), der=V.reduce((a,b)=>b.cc>a.cc?b:a);
  if(bot.cc-top.cc<h*0.35 || der.cc-izq.cc<w*0.35)return null;   // par de bordes demasiado juntos
  const cruce=(hl,vl)=>{const den=1-vl.m*hl.m; if(Math.abs(den)<1e-6)return null; const x=(vl.m*hl.b+vl.b)/den; return [x,hl.m*x+hl.b];};
  const TL=cruce(top,izq),TR=cruce(top,der),BR=cruce(bot,der),BL=cruce(bot,izq);
  if(!TL||!TR||!BR||!BL)return null;
  const pad=0.18, dentro=p=>p[0]>=-w*pad&&p[0]<=w*(1+pad)&&p[1]>=-h*pad&&p[1]<=h*(1+pad);
...
function detectarCuadrilateroGenerico(canvas){
  const maxD=480, sc=Math.min(1,maxD/Math.max(canvas.width,canvas.height));
  const w=Math.max(1,Math.round(canvas.width*sc)),h=Math.max(1,Math.round(canvas.height*sc));
  const t=document.createElement('canvas');t.width=w;t.height=h;t.getContext('2d').drawImage(canvas,0,0,w,h);
  const q=_quadGenericoDeImagen(t.getContext('2d').getImageData(0,0,w,h)); if(!q)return null;
...
function detectarRejillaPxCm(canvas){
  const PI=Math.PI, maxD=1100, sc=Math.min(1,maxD/Math.max(canvas.width,canvas.height));
  const w=Math.max(1,Math.round(canvas.width*sc)), h=Math.max(1,Math.round(canvas.height*sc)), N=w*h;
  const t=document.createElement('canvas');t.width=w;t.height=h;t.getContext('2d').drawImage(canvas,0,0,w,h);
  const d=t.getContext('2d').getImageData(0,0,w,h).data;
  const gris=new Float32Array(N), verde=new Uint8Array(N); let nVerde=0;
  for(let i=0;i<N;i++){const r=d[i*4],g=d[i*4+1],b=d[i*4+2];gris[i]=0.299*r+0.587*g+0.114*b;if(_verdeMat(r,g,b)){verde[i]=1;nVerde++;}}
  if(nVerde < N*0.10) return null;                       // poca alfombrilla a la vista
  // Píxeles de TAPETE con gradiente significativo → arrays compactos (x,y,gx,gy) + histograma de orientación.
  const PX=new Int16Array(N),PY=new Int16Array(N),GX=new Float32Array(N),GY=new Float32Array(N); let m=0;
  const NB=180, hist=new Float32Array(NB);
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){const i=y*w+x;if(!verde[i])continue;
    const gx=gris[i+1]-gris[i-1], gy=gris[i+w]-gris[i-w]; if(Math.abs(gx)+Math.abs(gy)<10)continue;
    PX[m]=x;PY[m]=y;GX[m]=gx;GY[m]=gy;m++;
    let an=Math.atan2(gy,gx);an=((an%PI)+PI)%PI;hist[Math.min(NB-1,(an/PI*NB)|0)]+=Math.hypot(gx,gy);}
  if(m < 200) return null;
  const sm=new Float32Array(NB);for(let i=0;i<NB;i++){let s=0;for(let k=-2;k<=2;k++)s+=hist[(i+k+NB)%NB];sm[i]=s;}
  let pk=0;for(let i=0;i<NB;i++)if(sm[i]>sm[pk])pk=i; const phi0=(pk+0.5)/NB*PI; // ángulo aproximado
  // Periodo dominante = mejor MÁXIMO LOCAL de la autocorrelación (salta el lóbulo central). {L,conf}.
  const periodo=sig=>{
    const n=sig.length; let mn=0;for(let i=0;i<n;i++)mn+=sig[i];mn/=n;
    const s=new Float32Array(n);let ac0=0;for(let i=0;i<n;i++){s[i]=sig[i]-mn;ac0+=s[i]*s[i];}
    if(ac0<=0)return {L:0,conf:0};
    const minL=Math.max(8,Math.round(n*0.01)), maxL=Math.floor(n/3); const ac=new Float32Array(maxL+2);
    for(let L=minL-1;L<=maxL+1&&L<n;L++){let a=0;for(let i=0;i+L<n;i++)a+=s[i]*s[i+L];ac[L]=a/ac0;}
    let best=0,bestL=0;for(let L=minL;L<=maxL;L++)if(ac[L]>ac[L-1]&&ac[L]>=ac[L+1]&&ac[L]>best){best=ac[L];bestL=L;}
  ...
  const periodoDir=dir=>{const cd=Math.cos(dir),sd=Math.sin(dir);
    let tmin=Infinity,tmax=-Infinity; for(let k=0;k<m;k++){const tt=PX[k]*cd+PY[k]*sd;if(tt<tmin)tmin=tt;if(tt>tmax)tmax=tt;}
    const n=Math.max(8,Math.ceil(tmax-tmin)+1); const P=new Float32Array(n);
    for(let k=0;k<m;k++){const wgt=Math.abs(GX[k]*cd+GY[k]*sd);if(wgt<=0)continue;const bi=(PX[k]*cd+PY[k]*sd-tmin)|0;if(bi>=0&&bi<n)P[bi]+=wgt;}
    return periodo(P);};
  ...
  for(let dg=-8;dg<=8;dg+=0.5){const dir=phi0+dg*PI/180; const a=periodoDir(dir), b=periodoDir(dir+PI/2);
    const ok=a.L&&b.L&&Math.abs(a.L-b.L)/Math.min(a.L,b.L)<0.15; const s=ok?Math.min(a.conf,b.conf):0;
    if(s>best.s)best={s,a,b,dir};}
  const pasoDs=(best.a&&best.b)?(best.a.L+best.b.L)/2:0; const pxcm=pasoDs?pasoDs/sc:0;
  const angDeg=(((best.dir*180/PI)%90)+90)%90;
  try{window._rejillaDbg={ang:+angDeg.toFixed(1),cx:best.a?best.a.L:0,cc:best.a?+best.a.conf.toFixed(2):0,rx:best.b?best.b.L:0,rc:best.b?+best.b.conf.toFixed(2):0,sc:+sc.toFixed(2),pxcm:+pxcm.toFixed(1)};}catch(_){}
...
function _editorImagen(opts){
  ...
  src.onload=()=>{
    let work=document.createElement('canvas'); work.width=src.naturalWidth; work.height=src.naturalHeight; work.getContext('2d').drawImage(src,0,0);
    let modo='none', drag=-1, crop=null, quad=null;
    const TOPE=2400; // tope del lado mayor al exportar (protege memoria del móvil)
    $('#cmpModal').innerHTML=`<div class="box card" style="max-width:560px;max-height:92vh;overflow:auto"><h3 style="margin-top:0">✎ Editar imagen</h3>
    ...
    $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
    const c=$('#edC'),ctx=c.getContext('2d');
    let escala=1; const base=document.createElement('canvas'), bctx=base.getContext('2d');
    ...
    const ajustar=()=>{const maxW=Math.min(520,(window.innerWidth||520)-60);escala=Math.min(1,maxW/work.width);c.width=Math.round(work.width*escala);c.height=Math.round(work.height*escala);base.width=c.width;base.height=c.height;bctx.clearRect(0,0,base.width,base.height);bctx.drawImage(work,0,0,base.width,base.height);};
    const W2V=p=>[p[0]*escala,p[1]*escala], V2W=(x,y)=>[x/escala,y/escala];
    let raf=0; const pedir=()=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;render();});}; // throttle a 1/frame
    const render=()=>{
      ctx.clearRect(0,0,c.width,c.height);ctx.drawImage(base,0,0);
      const pts = modo==='crop'?[[crop.x0,crop.y0],[crop.x1,crop.y0],[crop.x1,crop.y1],[crop.x0,crop.y1]] : modo==='persp'?quad : null;
      if(pts){ ctx.save();ctx.strokeStyle='#28d9a8';ctx.lineWidth=2;ctx.fillStyle='rgba(40,217,168,.12)';
        ctx.beginPath();pts.forEach((p,k)=>{const v=W2V(p);k?ctx.lineTo(v[0],v[1]):ctx.moveTo(v[0],v[1]);});ctx.closePath();ctx.fill();ctx.stroke();
        pts.forEach(p=>{const v=W2V(p);ctx.beginPath();ctx.arc(v[0],v[1],13,0,7);ctx.fillStyle='rgba(40,217,168,.92)';ctx.fill();ctx.lineWidth=3;ctx.strokeStyle='#fff';ctx.stroke();});ctx.restore(); }
    };
    ...
    let _rc=null;const pos=ev=>{const rc=_rc||c.getBoundingClientRect();const t=ev.touches?ev.touches[0]:ev;return V2W(t.clientX-rc.left,t.clientY-rc.top);}; // _rc cacheado en pointerdown (evita reflow por evento)
    const handles=()=> modo==='crop'?[[crop.x0,crop.y0],[crop.x1,crop.y0],[crop.x1,crop.y1],[crop.x0,crop.y1]] : modo==='persp'?quad : [];
    c.addEventListener('pointerdown',ev=>{ if(modo==='none')return; ev.preventDefault(); _rc=c.getBoundingClientRect(); const p=pos(ev),hs=handles();let best=-1,bd=34/escala; hs.forEach((h,k)=>{const dd=_dist(h,p);if(dd<bd){bd=dd;best=k;}}); drag=best; if(best>=0)try{c.setPointerCapture(ev.pointerId);}catch(_){} });
    c.addEventListener('pointermove',ev=>{ if(drag<0)return; ev.preventDefault(); let[x,y]=pos(ev);const mX=work.width*0.03,mY=work.height*0.03;x=Math.max(mX,Math.min(work.width-mX,x));y=Math.max(mY,Math.min(work.height-mY,y));
      if(modo==='crop'){ if(drag===0){crop.x0=x;crop.y0=y;}else if(drag===1){crop.x1=x;crop.y0=y;}else if(drag===2){crop.x1=x;crop.y1=y;}else{crop.x0=x;crop.y1=y;} }
      else { quad[drag]=[x,y]; } pedir(); });
    const soltar=()=>{drag=-1;};c.addEventListener('pointerup',soltar);c.addEventListener('pointercancel',soltar);
    $('#edApply').onclick=()=>{
      if(modo==='crop'){ const x0=Math.min(crop.x0,crop.x1),y0=Math.min(crop.y0,crop.y1),w=Math.abs(crop.x1-crop.x0),h=Math.abs(crop.y1-crop.y0);
        if(w<8||h<8){toast('Recorte demasiado pequeño','warn');return;}
        const nc=document.createElement('canvas');nc.width=Math.round(w);nc.height=Math.round(h);nc.getContext('2d').drawImage(work,x0,y0,w,h,0,0,nc.width,nc.height);work=nc; }
      else if(modo==='persp'){
        const[TL,TR,BR,BL]=quad; let W=Math.round((_dist(TL,TR)+_dist(BL,BR))/2),H=Math.round((_dist(TL,BL)+_dist(TR,BR))/2);
        if(W<8||H<8){toast('Esquinas no válidas','warn');return;}
        const f=Math.min(1,TOPE/Math.max(W,H));W=Math.round(W*f);H=Math.round(H*f);
        const Hi=_homografia([[0,0],[W,0],[W,H],[0,H]],quad); // salida-rect → fuente
        const simg=work.getContext('2d').getImageData(0,0,work.width,work.height),sd=simg.data,sw=work.width,sh=work.height;
        const out=ctx.createImageData(W,H),od=out.data;
        for(let y=0;y<H;y++)for(let x=0;x<W;x++){const[sx,sy]=_mapH(Hi,x+0.5,y+0.5);const ix=sx|0,iy=sy|0;const o=(y*W+x)*4;
          if(ix>=0&&iy>=0&&ix<sw&&iy<sh){const s=(iy*sw+ix)*4;od[o]=sd[s];od[o+1]=sd[s+1];od[o+2]=sd[s+2];od[o+3]=255;}else od[o+3]=255;}
        const nc=document.createElement('canvas');nc.width=W;nc.height=H;nc.getContext('2d').putImageData(out,0,0);work=nc; }
      modo='none';$('#edApply').style.display='none';reset();
    };
    $('#edX').onclick=()=>{ if(opts.onClose)opts.onClose(); };
    $('#edSave').onclick=async()=>{
      // exportar (cap del lado mayor) a JPEG
      let cw=work.width,ch=work.height;const f=Math.min(1,TOPE/Math.max(cw,ch));
      let exp=work; if(f<1){exp=document.createElement('canvas');exp.width=Math.round(cw*f);exp.height=Math.round(ch*f);exp.getContext('2d').drawImage(work,0,0,exp.width,exp.height);}
...
function abrirEditorImagen(i){
  const im=_imgState.imgs[i]; if(!im)return;
  _editorImagen({ src:encUrl(im.ruta)+'?t='+Date.now(), onClose:pintarGestorImagenes,
    onSave:async(b64)=>{ const ok=await apiImg('reemplazar',{ruta:im.ruta,base64:b64}); if(ok)toast('Imagen actualizada'); } });
}
...
function medirDimensiones(id, imagenes){
  const im=(imagenes||[])[0]; if(!im){toast('No hay imagen para medir','warn');return;}
  const src=new Image(); src.crossOrigin='anonymous';
  src.onerror=()=>toast('No se pudo cargar la imagen','bad');
  src.onload=()=>{
    const iw=src.naturalWidth, ih=src.naturalHeight;
    let quad=[[iw*0.14,ih*0.14],[iw*0.86,ih*0.14],[iw*0.86,ih*0.86],[iw*0.14,ih*0.86]];
    let regla=[[iw*0.20,ih*0.95],[iw*0.50,ih*0.95]], cm=10, drag=null, pxcmAuto=null;
    $('#cmpModal').innerHTML=`<div class="box card" style="max-width:560px;max-height:92vh;overflow:auto"><h3 style="margin-top:0">📐 Medir tamaño</h3>
    ...
    $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
    const c=$('#mdC'),ctx=c.getContext('2d');
    const maxW=Math.min(520,(window.innerWidth||520)-60), escala=Math.min(1,maxW/iw);
    c.width=Math.round(iw*escala);c.height=Math.round(ih*escala);
    const W2V=p=>[p[0]*escala,p[1]*escala], V2W=(x,y)=>[x/escala,y/escala];
    // base = imagen ya escalada (una vez); arrastrar solo blitea base + dibuja recuadro/regla (sin freeze).
    const base=document.createElement('canvas');base.width=c.width;base.height=c.height;base.getContext('2d').drawImage(src,0,0,base.width,base.height);
    let raf=0; const pedir=()=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;render();});}; // throttle a 1/frame
    const calc=()=>{const pxcm=pxcmAuto||(_dist(regla[0],regla[1])/(cm||1));if(!pxcm||!isFinite(pxcm))return null;
      return {W:(_dist(quad[0],quad[1])+_dist(quad[3],quad[2]))/2/pxcm, H:(_dist(quad[0],quad[3])+_dist(quad[1],quad[2]))/2/pxcm};};
    const aro=(p,fill)=>{const v=W2V(p);ctx.beginPath();ctx.arc(v[0],v[1],11,0,7);ctx.fillStyle=fill;ctx.fill();ctx.lineWidth=2.5;ctx.strokeStyle='#fff';ctx.stroke();};
    const render=()=>{ ctx.clearRect(0,0,c.width,c.height);ctx.drawImage(base,0,0);
      ctx.save();ctx.strokeStyle='#28d9a8';ctx.lineWidth=2;ctx.fillStyle='rgba(40,217,168,.10)';ctx.beginPath();quad.forEach((p,k)=>{const v=W2V(p);k?ctx.lineTo(v[0],v[1]):ctx.moveTo(v[0],v[1]);});ctx.closePath();ctx.fill();ctx.stroke();quad.forEach(p=>aro(p,'rgba(40,217,168,.92)'));
      ctx.strokeStyle='#1ba3ff';ctx.lineWidth=3;const a=W2V(regla[0]),b=W2V(regla[1]);ctx.beginPath();ctx.moveTo(a[0],a[1]);ctx.lineTo(b[0],b[1]);ctx.stroke();regla.forEach(p=>aro(p,'rgba(27,163,255,.95)'));ctx.restore();
      const r=calc();$('#mdOut').textContent=r?`≈ ${r.W.toFixed(1).replace('.',',')} × ${r.H.toFixed(1).replace('.',',')} cm${pxcmAuto?' · auto':''}`:''; };
    ...
    const intentarAuto=()=>{
      const tmp=document.createElement('canvas');tmp.width=iw;tmp.height=ih;tmp.getContext('2d').drawImage(src,0,0);
      const mX=iw*0.03,mY=ih*0.03,cl=(v,lo,hi)=>Math.max(lo,Math.min(hi,v));
      const q=detectarBordesVerde(tmp); if(q)quad=q.map(p=>[cl(p[0],mX,iw-mX),cl(p[1],mY,ih-mY)]);
      const pxcm=detectarRejillaPxCm(tmp);
      let dims=null; if(pxcm&&q){const W=(_dist(quad[0],quad[1])+_dist(quad[3],quad[2]))/2/pxcm,H=(_dist(quad[0],quad[3])+_dist(quad[1],quad[2]))/2/pxcm;dims={W,H};}
      const ok=!!dims&&dims.W>=4&&dims.W<=50&&dims.H>=4&&dims.H<=50;
      if(ok){pxcmAuto=pxcm;cm=+(_dist(regla[0],regla[1])/pxcm).toFixed(1);const ci=$('#mdCm');if(ci)ci.value=String(cm);} else pxcmAuto=null;
      render(); return {q,pxcm,dims,ok,tmp};
    };
    // Al CARGAR: si hay tapete, auto-mide (silencioso si falla). Foto normal sin tapete → no cambia nada.
    {const a0=intentarAuto(); if(a0.ok)toast(`Auto: ${a0.dims.W.toFixed(1).replace('.',',')} × ${a0.dims.H.toFixed(1).replace('.',',')} cm — ajusta si hace falta`);}
    let _rc=null;const pos=ev=>{const rc=_rc||c.getBoundingClientRect();const t=ev.touches?ev.touches[0]:ev;return V2W(t.clientX-rc.left,t.clientY-rc.top);}; // _rc cacheado en pointerdown (evita reflow por evento)
    c.addEventListener('pointerdown',ev=>{ev.preventDefault();_rc=c.getBoundingClientRect();const p=pos(ev);let best=null,bd=34/escala;
      quad.forEach((h,i)=>{const dd=_dist(h,p);if(dd<bd){bd=dd;best={s:'q',i};}});
      regla.forEach((h,i)=>{const dd=_dist(h,p);if(dd<bd){bd=dd;best={s:'r',i};}});drag=best;if(best)try{c.setPointerCapture(ev.pointerId);}catch(_){}});
    c.addEventListener('pointermove',ev=>{if(!drag)return;ev.preventDefault();let[x,y]=pos(ev);const mX=iw*0.03,mY=ih*0.03;x=Math.max(mX,Math.min(iw-mX,x));y=Math.max(mY,Math.min(ih-mY,y));if(drag.s==='r')pxcmAuto=null;/* tocar la regla → escala manual */(drag.s==='q'?quad:regla)[drag.i]=[x,y];pedir();});
    const soltar=()=>{drag=null;};c.addEventListener('pointerup',soltar);c.addEventListener('pointercancel',soltar);
    $('#mdCm').oninput=()=>{cm=parseFloat($('#mdCm').value)||0;pxcmAuto=null;/* la regla manual manda */render();};
    $('#mdAutoMed').onclick=()=>{const {q,pxcm,dims,ok,tmp}=intentarAuto();
      const dg=window._rejillaDbg; const dbg=dg?` · ${dg.ang}° X[${dg.cx}|${dg.cc}] Y[${dg.rx}|${dg.rc}] sc${dg.sc} ${dg.pxcm}px/cm`:'';
      try{console.log('AutoMedir',{libro:!!q,pxcm,dims,dbg:dg});}catch(_){}
      if(ok){toast('Auto: rejilla '+pxcm.toFixed(1).replace('.',',')+' px/cm'+dbg);return;}
      if(pxcm&&dims){toast(`Rejilla DUDOSA → ${dims.W.toFixed(0)}×${dims.H.toFixed(0)} cm absurdo; mide a mano${dbg}`,'warn');return;}
      const fv=Math.round(fraccionVerde(tmp)*100); // % de tapete detectado
      toast(`${q?'libro ✓':'libro ✗'} · rejilla ✗ · tapete ${fv}%${dbg}`,'warn');};
    $('#mdAuto').onclick=()=>{const tmp=document.createElement('canvas');tmp.width=iw;tmp.height=ih;tmp.getContext('2d').drawImage(src,0,0);let q=detectarBordesVerde(tmp);if(q){const mX=iw*0.03,mY=ih*0.03,cl=(v,lo,hi)=>Math.max(lo,Math.min(hi,v));quad=q.map(p=>[cl(p[0],mX,iw-mX),cl(p[1],mY,ih-mY)]);render();toast('Bordes detectados');}else toast('No detecté el libro sobre fondo verde','warn');};
    $('#mdX').onclick=cerrarCmp;$('#cmpScrim').onclick=cerrarCmp;
    $('#mdSave').onclick=async()=>{const r=calc();if(!r||!(r.W>0&&r.H>0)){toast('Coloca la regla y el recuadro primero','warn');return;}
      try{const res=await api('/documentos/'+encodeURIComponent(id)+'/dimensiones',{method:'POST',body:JSON.stringify({ancho_cm:r.W,alto_cm:r.H})});
        if(!res.ok){toast(res.motivo||'error','bad');return;}
        cerrarCmp();toast(`Tamaño guardado: ${String(res.ancho_cm).replace('.',',')} × ${String(res.alto_cm).replace('.',',')} cm`);verDoc(id,detalle&&detalle.ctx);
      }catch(e){toast(e.message,'bad');}};
  };
  src.src=encUrl(im.ruta)+'?t='+Date.now();
}
function fichaEditar(d,r,opts){
  opts=opts||{};const sup=!!opts.supervisado;
  const v=x=>x==null?'':String(x);
  const campo=(id,lab,val,tag='input',extra='')=>`<label style="display:block;margin-top:8px">${lab}</label>${tag==='textarea'?`<textarea id="${id}" rows="10" ${extra}>${esc(val)}</textarea>`:`<input id="${id}" value="${esc(val)}" ${extra} autocomplete="off">`}`;
  const ub=d.ubicacion||{};
  const btnScanIsbn=('BarcodeDetector' in window)?'<button class="btn" type="button" id="edScan" title="Escanear código de barras con la cámara">📷</button>':'';
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:560px;max-height:88vh;overflow:auto"><h3 style="margin-top:0">${sup?'🚥 Revisar alta':'✏️ Editar documento'}</h3>
  ...
  const setupUbicEdit=()=>montarSelUbic({sa:'edAmbSel',ia:'edAmb',se:'edEstSel',ie:'edEst',curA:ub.ambito,curE:ub.estanteria});
  if(mapaUbicaciones.length)setupUbicEdit();else cargarUbicaciones().then(setupUbicEdit);
  // Editor de OTRAS EDICIONES (ISBN alternativo + rol). Filas añadibles/borrables.
  const rolOpts=sel=>ROL_ISBN_OPC.map(([val,lab])=>`<option value="${val}"${val===sel?' selected':''}>${lab}</option>`).join('');
  const edAltFila=a=>`<div class="edAltRow" style="display:flex;gap:6px;margin-top:6px;align-items:center"><input class="edAltIsbn" value="${esc((a&&a.isbn)||'')}" placeholder="ISBN" autocomplete="off" style="flex:1"><select class="edAltRol" style="flex:0 0 auto">${rolOpts((a&&a.rol)||'otro')}</select><button type="button" class="btn bad edAltDel" title="Quitar" style="padding:2px 9px">✕</button></div>`;
  const edAltWire=b=>{b.onclick=()=>b.closest('.edAltRow').remove();};
  {const L=$('#edAltList');if(L){L.innerHTML=(d.isbns_alternativos||[]).map(edAltFila).join('');$$('#edAltList .edAltDel').forEach(edAltWire);}}
  if($('#edAltAdd'))$('#edAltAdd').onclick=()=>{const L=$('#edAltList');if(!L)return;L.insertAdjacentHTML('beforeend',edAltFila(null));edAltWire(L.lastElementChild.querySelector('.edAltDel'));};
  if($('#edScan'))$('#edScan').onclick=()=>escanearISBN('edIsbn');   // escanear ISBN en la ficha (supervisado)
  const onGuardar=async()=>{
    const campos={titulo:$('#edTit').value,subtitulo:$('#edSub').value,tipo_recurso:$('#edTipo').value,autores:$('#edAut').value,editorial:$('#edEdi').value,
      'año_edicion':$('#edAno').value,idioma:$('#edIdi').value,paginas:$('#edPag').value,isbn:$('#edIsbn').value,issn:$('#edIssn').value,
      cdu:$('#edCdu').value,numero_edicion:$('#edEd').value,palabras_clave:$('#edPal').value,sinopsis:$('#edSin').value,
      ubicacion:{ambito:$('#edAmb').value,estanteria:$('#edEst').value},locked:$('#edLock').checked,
      isbns_alternativos:[...document.querySelectorAll('#edAltList .edAltRow')].map(row=>({isbn:row.querySelector('.edAltIsbn').value.trim(),rol:row.querySelector('.edAltRol').value})).filter(a=>a.isbn)};
    try{const res=await api('/documentos/'+encodeURIComponent(d._id)+'/editar',{method:'POST',body:JSON.stringify(campos)});
      if(!res.ok){$('#edErr').textContent=res.motivo;toast(res.motivo,'bad');return;}
      cerrarCmp();toast('Guardado'+((res.avisos||[]).length?' · '+res.avisos.join('; '):''));
      if(sup)go('inbox');else verDoc(d._id,detalle&&detalle.ctx);
    }catch(e){$('#edErr').textContent=e.message;toast(e.message,'bad');}};
  ...
  const onCancelar=sup?async()=>{
    if(!confirm('¿Descartar este alta? Se borra de la base y su carpeta va a la Papelera (recuperable).'))return;
    try{const res=await api('/documentos/'+encodeURIComponent(d._id)+'/descartar',{method:'POST',body:JSON.stringify({})});
      if(!res.ok){toast(res.motivo||'No se pudo descartar','bad');return;}
      cerrarCmp();toast('Alta descartada (en la Papelera)');go('inbox');
    }catch(e){toast(e.message,'bad');}
  }:cerrarCmp;
  // Botones duplicados arriba y abajo (no hace falta hacer scroll para Aceptar/Cancelar).
  ['edX','edXTop'].forEach(id=>{const b=$('#'+id);if(b)b.onclick=onCancelar;});
  ['edOk','edOkTop'].forEach(id=>{const b=$('#'+id);if(b)b.onclick=onGuardar;});
...
function previewArchivo(r){
  ...
  const nombre=r.nombre_archivo||'archivo',url=encUrl(r.archivo_url),ext=(nombre.split('.').pop()||'').toLowerCase();
  ...
  if(ext==='pdf')return `<div class="fileprev"><h3 style="margin:16px 0 8px;color:var(--mut);font-size:13px">📄 ${esc(nombre)}</h3>
  ...
  if(ext==='epub')return `<div class="fileprev"><h3 style="margin:16px 0 8px;color:var(--mut);font-size:13px">📗 ${esc(nombre)}</h3>
  ...
  if(['cbz','cbr','cb7','djvu'].includes(ext))return `<div class="fileprev"><h3 style="margin:16px 0 8px;color:var(--mut);font-size:13px">${ext==='djvu'?'📘':'🗂️'} ${esc(nombre)}</h3>
...
let epubRendition=null,epubResize=null;
async function iniciarLectorEpub(url){
  const wrap=$('#epubWrap'),area=$('#epubArea'),msg=$('#epubMsg');if(!area)return;
  try{
    await cargarEpubLib();
    ...
    epubRendition.themes.default({
      ...
      'img,image,svg':{'max-width':'100% !important','max-height':'100% !important','object-fit':'contain'},
    });
    // Clavar la altura del iframe al marco (epub.js a veces la reajusta al contenido); tras dos rAF.
    const fijar=()=>{const h=area.clientHeight;if(!h)return;const ifr=area.querySelector('iframe');if(ifr)ifr.style.height=h+'px';};
    const pin=()=>requestAnimationFrame(()=>requestAnimationFrame(fijar));
    epubRendition.on('rendered',pin);
    await epubRendition.display();
    pin();
    if(msg)msg.style.display='none';
    const prev=$('#epubPrev'),next=$('#epubNext'),fs=$('#epubFs'),bar=$('#epubBar');
    [prev,next,fs].forEach(b=>b.style.display='grid');bar.style.display='flex';
    prev.onclick=()=>epubRendition.prev();next.onclick=()=>epubRendition.next();

    // Índice (tabla de contenidos).
    book.loaded.navigation.then(nav=>{
      const toc=$('#epubToc');if(!toc)return;const items=[];
      const rec=(arr,sub)=>arr.forEach(it=>{items.push(`<a href="#" data-href="${esc(it.href)}"${sub?' class="sub"':''}>${esc((it.label||'').trim()||'—')}</a>`);if(it.subitems&&it.subitems.length)rec(it.subitems,true);});
      rec(nav.toc||[],false);
      toc.innerHTML=items.join('')||'<div class="muted" style="padding:14px">Sin índice</div>';
      $$('#epubToc a[data-href]').forEach(a=>a.onclick=e=>{e.preventDefault();epubRendition.display(a.dataset.href);toc.classList.remove('open');});
    }).catch(()=>{});
    ...
    let epubFont=100;
    const aplicarFuente=()=>{try{epubRendition.themes.fontSize(epubFont+'%');}catch{}};
    $('#epubFontDown').onclick=()=>{epubFont=Math.max(60,epubFont-10);aplicarFuente();};
    $('#epubFontUp').onclick=()=>{epubFont=Math.min(250,epubFont+10);aplicarFuente();};

    // Progreso (%). Las localizaciones se generan en segundo plano; hasta entonces, página/total.
    let locsReady=false;
    const progreso=(loc)=>{if(!loc){try{loc=epubRendition.currentLocation();}catch{return;}}if(!loc||!loc.start)return;
      const bi=$('#epubProg>i'),pc=$('#epubPct');let pct=null;
      if(locsReady){try{pct=book.locations.percentageFromCfi(loc.start.cfi);}catch{}}
      if(pct!=null){if(bi)bi.style.width=Math.round(pct*100)+'%';if(pc)pc.textContent=Math.round(pct*100)+'%';}
      else if(pc&&loc.start.displayed)pc.textContent=`${loc.start.displayed.page}/${loc.start.displayed.total}`;};
    epubRendition.on('relocated',loc=>{pin();progreso(loc);});
    book.ready.then(()=>book.locations.generate(1600)).then(()=>{locsReady=true;progreso();}).catch(()=>{});
    $('#epubProg').onclick=e=>{if(!locsReady)return;const rc=e.currentTarget.getBoundingClientRect();
      const p=Math.max(0,Math.min(1,(e.clientX-rc.left)/rc.width));try{epubRendition.display(book.locations.cfiFromPercentage(p));}catch{}};
    ...
    const refit=()=>requestAnimationFrame(()=>requestAnimationFrame(()=>{
      if(!epubRendition)return;const dd=dims();let cfi=null;
      try{const l=epubRendition.currentLocation();cfi=l&&l.start&&l.start.cfi;}catch{}
      try{epubRendition.resize(dd.w,dd.h);}catch{}
      if(cfi){try{epubRendition.display(cfi);}catch{}}
      pin();
    }));
    // Pantalla completa: overlay por CSS (NO la Fullscreen API — iOS Safari no la soporta en <div>).
    fs.onclick=()=>{const f=wrap.classList.toggle('full');fs.textContent=f?'✕':'⛶';
      fs.title=f?'Salir de pantalla completa':'Pantalla completa';refit();};
    if(epubResize)window.removeEventListener('resize',epubResize);
    epubResize=()=>{clearTimeout(epubResize._t);epubResize._t=setTimeout(refit,150);};
    window.addEventListener('resize',epubResize);
    document.onkeydown=e=>{if(!$('#epubArea'))return;
      if(e.key==='ArrowLeft')epubRendition.prev();
      else if(e.key==='ArrowRight')epubRendition.next();
      else if(e.key==='Escape'&&wrap.classList.contains('full')){wrap.classList.remove('full');fs.textContent='⛶';fs.title='Pantalla completa';refit();}};
  }catch(e){if(msg)msg.innerHTML=`No se pudo abrir el EPUB en el navegador (${esc(e.message)}).<br>Usa las opciones de descarga de abajo.`;}
}

// ── lector PDF (PDF.js vendored) — render propio en canvas: previsualiza IGUAL en PC y móvil ──
let pdfLibPromise=null,pdfDoc=null,pdfObs=null;
function cargarPdfLib(){
  if(window.pdfjsLib)return Promise.resolve();
  if(pdfLibPromise)return pdfLibPromise;
  pdfLibPromise=new Promise((res,rej)=>{const s=document.createElement('script');s.src='/vendor/pdf.min.js';s.onload=res;s.onerror=()=>rej(new Error('no se pudo cargar pdf.js'));document.head.appendChild(s);});
  return pdfLibPromise;
}
async function iniciarLectorPdf(url){
  const wrap=$('#pdfWrap'),scroll=$('#pdfScroll'),msg=$('#pdfMsg');if(!scroll)return;
  try{
    await cargarPdfLib();
    ...
    if(!$('#pdfScroll'))return;
    if(msg)msg.style.display='none';
    $('#pdfBar').style.display='flex';$('#pdfFs').style.display='grid';$('#pdfTotal').textContent=pdfDoc.numPages;
    scroll.innerHTML='';
    if(pdfObs)pdfObs.disconnect();
    // Render perezoso: cada página se dibuja al acercarse y se libera al alejarse (acota memoria).
    pdfObs=new IntersectionObserver(ents=>ents.forEach(en=>{en.isIntersecting?renderPdfPage(en.target):liberarPdfPage(en.target);}),{root:scroll,rootMargin:'300px'});
    for(let n=1;n<=pdfDoc.numPages;n++){const ph=document.createElement('div');ph.className='pdfpage';ph.dataset.n=n;ph.style.aspectRatio='1/1.414';scroll.appendChild(ph);pdfObs.observe(ph);}
    // Indicador de página: la más alta cuyo borde superior ya pasó el del visor.
    scroll.onscroll=()=>{const y=scroll.scrollTop+80;let cur=1;for(const p of scroll.children){if(p.offsetTop<=y)cur=Number(p.dataset.n);else break;}const e=$('#pdfCur');if(e)e.textContent=cur;};
    // Pantalla completa: overlay por CSS (igual que el epub).
    const fs=$('#pdfFs');fs.onclick=()=>{const f=wrap.classList.toggle('full');fs.textContent=f?'✕':'⛶';fs.title=f?'Salir de pantalla completa':'Pantalla completa';};
    document.onkeydown=e=>{if(!$('#pdfScroll'))return;if(e.key==='Escape'&&wrap.classList.contains('full')){wrap.classList.remove('full');fs.textContent='⛶';fs.title='Pantalla completa';}};
  }catch(e){if(msg)msg.innerHTML=`No se pudo abrir el PDF en el navegador (${esc(e.message)}).<br>Usa Descargar.`;}
}
async function renderPdfPage(el){
  if(!pdfDoc||el.dataset.done)return;el.dataset.done='1';
  try{
    ...
    el.style.aspectRatio='';el.innerHTML='';el.appendChild(canvas);
    await page.render({canvasContext:canvas.getContext('2d'),viewport:vp}).promise;
  }catch(e){if(el)el.innerHTML=`<div class="muted" style="padding:10px;font-size:12px">Página ${esc(el.dataset.n)}: ${esc(e.message)}</div>`;}
}
function liberarPdfPage(el){if(!el.dataset.done)return;el.dataset.done='';el.innerHTML='';el.style.aspectRatio='1/1.414';}

// ── búsqueda + catálogo ──
let estadoBusqueda={page:1},busqTimer=null;
// ── selección múltiple + agrupado (añadir a colección / obra) ──
let selDocs=new Set(),paginaIds=[];
function toggleSel(id,on){if(on)selDocs.add(id);else selDocs.delete(id);
  const c=$('#searchResults .vol[data-doc="'+id+'"]');if(c)c.classList.toggle('sel',on);renderBulk();}
function renderBulk(){const el=$('#searchBulk');if(!el)return;
...
async function eliminarSeleccionados(){
  const n=selDocs.size;if(!n)return;
  const pw=await modalPassword({titulo:`🗑 Eliminar ${n} libro(s)`,aviso:`Se borrarán <b>${n}</b> documento(s); sus carpetas (sidecars e imágenes) irán a la <b>Papelera</b> (recuperable). Acción MASIVA. Confirma con tu contraseña de administrador.`});
  if(pw==null)return;
  try{const r=await api('/documentos/eliminar-lote',{method:'POST',body:JSON.stringify({ids:[...selDocs],password:pw})});
    if(!r.ok){toast(r.motivo||'No se pudo eliminar','bad');return;}
    toast(`Eliminado(s) ${r.eliminados} libro(s)${r.fallidos?` · ${r.fallidos} fallido(s)`:''} → Papelera`);
    selDocs.clear();buscarCatalogo(estadoBusqueda.page||1);
  }catch(e){toast(e.message,'bad');}
}
...
async function pickerGrupo(kind){
  ...
  let items=[];try{items=await api(esCol?'/colecciones':'/obras');}catch{}
  const ico=esCol?'📚':'📖';
  const nom=o=>esCol?o.nombre:o.titulo;
  const sub=o=>esCol?`${o.tipo==='revista'?'📰 revista':'📚 libro'} · ${o.miembros||0} miembro(s)`:`${o.volumenes_presentes||0}/${o.total_volumenes||'?'} tomos`;
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:560px"><h3 style="margin-top:0">${ico} Añadir <b>${selDocs.size}</b> doc(s) a una ${esCol?'colección':'obra'}</h3>
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';$('#pkX').onclick=cerrarCmp;$('#cmpScrim').onclick=cerrarCmp;
  const pintar=q=>{q=(q||'').toLowerCase().trim();
    const fil=items.filter(o=>(nom(o)||'').toLowerCase().includes(q));
    $('#pkLista').innerHTML=fil.length?fil.map(o=>`<div class="pkitem" data-id="${esc(o._id)}"><div class="pkcov">${o.portada?`<img src="${esc(encUrl(o.portada))}" loading="lazy" onerror="this.parentNode.textContent='${ico}'">`:ico}</div><div class="pkmeta"><div class="pkn">${esc(nom(o)||'—')}</div><div class="pks muted">${esc(sub(o))}</div></div></div>`).join(''):'<div class="muted" style="padding:14px">Sin resultados</div>';
    $$('#pkLista .pkitem').forEach(el=>el.onclick=()=>aplicarGrupo(kind,{id:el.dataset.id}));};
  pintar('');
  $('#pkFiltro').oninput=()=>pintar($('#pkFiltro').value);setTimeout(()=>$('#pkFiltro').focus(),30);
  $('#pkCrear').onclick=()=>{const nombre=$('#pkNom').value.trim();if(!nombre){$('#pkErr').textContent='Escribe un nombre';return;}aplicarGrupo(kind,{nombre,tipo:esCol?$('#pkTipo').value:undefined});};
}
async function aplicarGrupo(kind,{id,nombre,tipo}){
  const esCol=kind==='coleccion';
  const body=esCol?{ids:[...selDocs],coleccionId:id||null,nombre:nombre||null,tipo}:{ids:[...selDocs],obraId:id||null,titulo:nombre||null};
  try{const r=await api(esCol?'/documentos/agrupar/coleccion':'/documentos/agrupar/obra',{method:'POST',body:JSON.stringify(body)});
    if(!r.ok){const e=$('#pkErr');if(e)e.textContent=r.motivo;else toast(r.motivo,'bad');return;}
    cerrarCmp();toast(`${r.n} doc(s) → ${esCol?'colección «'+r.coleccion.nombre:'obra «'+r.obra.titulo}»`);selDocs.clear();buscarCatalogo(estadoBusqueda.page||1);
  }catch(e){const el=$('#pkErr');if(el)el.textContent=e.message;else toast(e.message,'bad');}}
function construirSearch(){
  ...
  {const fl=$('#sqFiltros');if(fl){const sav=localStorage.getItem('sq_filtros');fl.open=(sav===null)?matchMedia('(min-width:860px)').matches:(sav==='1');fl.addEventListener('toggle',()=>localStorage.setItem('sq_filtros',fl.open?'1':'0'));}}
  $('#sqQ').oninput=()=>{clearTimeout(busqTimer);busqTimer=setTimeout(()=>buscarCatalogo(1),350);};
  $('#sqQ').onkeydown=e=>{if(e.key==='Enter'){clearTimeout(busqTimer);buscarCatalogo(1);if($('#sqDescubrir')&&$('#sqDescubrir').checked)lanzarDescubrir();}};
  $('#sqCdu').oninput=()=>{clearTimeout(busqTimer);busqTimer=setTimeout(()=>buscarCatalogo(1),350);};
  ...
  if($('#sqAmbito'))$('#sqAmbito').onchange=()=>{pintarEstanteriaSearch();buscarCatalogo(1);};
  ...
  if($('#sqGoUbic'))$('#sqGoUbic').onclick=()=>{const a=($('#sqAmbito')&&$('#sqAmbito').value)||'';ubicPendiente=a?{a,e:($('#sqEstanteria')&&$('#sqEstanteria').value)||null}:null;go('ubic');};
  ...
  $$('#p-search .sqStar').forEach(c=>c.onchange=()=>{actualizarSumEstrellas();buscarCatalogo(1);});
  if($('#sqNsfw'))$('#sqNsfw').onchange=()=>{actualizarSumEstrellas();buscarCatalogo(1);};
  $('#sqClear').onclick=()=>{$('#sqQ').value='';$('#sqCdu').value='';$('#sqTipo').value='';if($('#sqSoporte'))$('#sqSoporte').value='';$('#sqOrden').value='reciente';
    if($('#sqAmbito'))$('#sqAmbito').value='';if($('#sqEstanteria')){$('#sqEstanteria').value='';pintarEstanteriaSearch();}
    if($('#sqDescubrir'))$('#sqDescubrir').checked=false;if($('#searchExternal'))$('#searchExternal').innerHTML='';
    if($('#sqNsfw'))$('#sqNsfw').checked=false;if($('#sqNfc'))$('#sqNfc').value='';
    $$('#p-search .sqStar').forEach(c=>c.checked=false);estadoBusqueda.extra=null;actualizarSumEstrellas();buscarCatalogo(1);};
}
function estrellasSel(){return $$('#p-search .sqStar:checked').map(c=>c.value);}
// ¿Hay ALGÚN criterio de búsqueda además del 🔞? (para decidir incluir-NSFW vs solo-NSFW)
function hayOtrosCriteriosBusqueda(){
  const v=id=>($('#'+id)&&$('#'+id).value||'').trim();
  if(v('sqQ')||v('sqTipo')||v('sqSoporte')||v('sqCdu')||v('sqAmbito')||v('sqEstanteria')||v('sqNfc'))return true;
  const e=estrellasSel(); if(e.length&&e.length<6)return true;       // subconjunto de estrellas (no "Todas")
  if(estadoBusqueda&&estadoBusqueda.extra)return true;               // viene de un chip/enlace (ubicación, clasificación…)
  return false;
}
function actualizarSumEstrellas(){const s=$('#sqStarsSum');if(!s)return;const sel=estrellasSel();
...
async function buscarCatalogo(page){
  ...
  catch(e){$('#searchResults').innerHTML=`<div class="empty">${esc(e.message)}</div>`;if($('#searchCount'))$('#searchCount').textContent='—';}
  ...
  if(ext){if($('#sqDescubrir')&&$('#sqDescubrir').checked)pintarDescubrirBoton();else ext.innerHTML='';}
}
// ── Descubrir: candidatos del Fichero (OL+BNE) BAJO DEMANDA (worker) + paginación en cliente ──
let descResultados=[],descPagina=1,descQ='';
...
function pintarDescubrirBoton(){
  const ext=$('#searchExternal');if(!ext)return;
  const q=$('#sqQ').value.trim();
  ext.innerHTML=descCaja(`<div class="row" style="align-items:center"><b>🔭 Fuera de tu biblioteca</b><span class="muted">Fichero · Open Library + BNE (58,7 M)</span><div style="flex:1"></div>
...
async function lanzarDescubrir(){
  const ext=$('#searchExternal');if(!ext)return;
  const q=$('#sqQ').value.trim();if(q.length<2)return;
  descQ=q;descResultados=[];descPagina=1;
  ext.innerHTML=descCaja('<div class="muted" style="padding:4px">🔭 Buscando en el Fichero (58,7 M)…</div>');
  try{const r=await api('/descubrir?q='+encodeURIComponent(q));
    if(!r.disponible){ext.innerHTML=descCaja(`<b>🔭 Fuera de tu biblioteca</b><div class="muted" style="margin-top:6px">No disponible: ${esc(r.motivo||'el Fichero no tiene índice de texto en este equipo')}.</div>`);return;}
    descResultados=r.candidatos||[];pintarDescubrir();
  }catch(e){ext.innerHTML=descCaja(`<div class="empty">${esc(e.message)}</div><div style="margin-top:8px"><button class="btn" id="descRetry">↻ Reintentar</button></div>`);if($('#descRetry'))$('#descRetry').onclick=lanzarDescubrir;}
}
function pintarDescubrir(){
  const ext=$('#searchExternal');if(!ext)return;
  const tot=descResultados.length,tp=Math.max(1,Math.ceil(tot/DESC_POR_PAG));
  ...
  const pager=tp>1?`<div style="display:flex;gap:10px;align-items:center;justify-content:center;margin-top:12px"><button class="btn" id="descPrev"${descPagina<=1?' disabled':''}>‹ Anterior</button><span class="muted">Página ${descPagina} de ${tp}</span><button class="btn" id="descNext"${descPagina>=tp?' disabled':''}>Siguiente ›</button></div>`:'';
  ext.innerHTML=descCaja(`<div class="row" style="align-items:center;margin-bottom:6px"><b>🔭 Fuera de tu biblioteca</b><span class="muted">${tot}${tot>=100?'+':''} candidato${tot===1?'':'s'} del Fichero · «${esc(recortar(descQ,36))}»</span><div style="flex:1"></div><button class="btn" id="descAgain" title="Buscar de nuevo">↻</button></div>
  ...
  if($('#descPrev'))$('#descPrev').onclick=()=>{descPagina--;pintarDescubrir();};
  if($('#descNext'))$('#descNext').onclick=()=>{descPagina++;pintarDescubrir();};
  if($('#descAgain'))$('#descAgain').onclick=lanzarDescubrir;
}
function descRow(c){
  const aut=(c.autores&&c.autores.length?c.autores.join(', '):'')||c.editorial||'autor desconocido';
  const bits=[c.anio,c.idioma?String(c.idioma).toUpperCase():'',c.cdu?('CDU '+c.cdu):(c.dewey?('Dewey '+c.dewey):''),c.isbn?('ISBN '+c.isbn):''].filter(Boolean).join(' · ');
  const badge=c.enBiblioteca?' <span class="fmt" style="background:rgba(74,222,128,.18);color:var(--ok)" title="Ya está en tu biblioteca">✓ la tienes</span>':'';
  const chip='display:inline-block;padding:4px 9px;border-radius:7px;background:rgba(120,160,255,.16);color:var(--accent,#9ab8ff);text-decoration:none;font-size:12px;white-space:nowrap';
  const links=(c.enlaces||[]).map(l=>`<a href="${esc(l.url)}" target="_blank" rel="noopener" style="${chip}">${esc(l.nombre)} ↗</a>`).join('');
...
function docCard(d){
  ...
  const cov=d.portada?`<img src="${esc(encUrl(d.portada))}" loading="lazy" onerror="this.parentNode.innerHTML='<div class=ph>${ph}</div>'">`:`<div class="ph">${ph}</div>`;
  const fmt=(d.formatos||[]).slice(0,2).map(f=>`<span class="fmt">${esc(f)}</span>`).join('');
...
function pintarBusqueda(r){
  ...
  $('#searchResults').innerHTML=r.docs.length?`<div class="vol-grid">${r.docs.map(docCard).join('')}</div>`:'<div class="empty">Sin resultados</div>';
  $$('#searchResults .vol[data-doc]').forEach(el=>el.onclick=()=>verDoc(el.dataset.doc,{volver:'search',etiqueta:'Búsqueda'}));
  $$('#searchResults .selchk').forEach(cb=>{cb.onclick=e=>e.stopPropagation();cb.onchange=()=>toggleSel(cb.dataset.id,cb.checked);});
  attachRating('#searchResults');
  renderBulk();
  const p=r.page,tp=r.paginas;
  // Paginación ARRIBA y ABAJO de los thumbnails. Al cambiar de página, desliza hasta el primer resultado.
  const pagerHtml=tp>1?`<button class="btn pgPrev" ${p<=1?'disabled':''}>‹ Anterior</button><span class="muted">Página ${p} de ${tp}</span><button class="btn pgNext" ${p>=tp?'disabled':''}>Siguiente ›</button>`:'';
  $('#searchPager').innerHTML=pagerHtml; if($('#searchPagerTop'))$('#searchPagerTop').innerHTML=pagerHtml;
  const irA=async np=>{await buscarCatalogo(np);
    const sb=$('#searchBulk');const a=(sb&&sb.textContent.trim())?sb:($('#searchPagerTop')||$('#searchResults'));
    if(a)a.scrollIntoView({behavior:'smooth',block:'start'});}; // muestra la barra «seleccionar todos» (y la nav fija)
  ['#searchPagerTop','#searchPager'].forEach(sel=>{const c=$(sel);if(!c)return;
    const pv=c.querySelector('.pgPrev'),nx=c.querySelector('.pgNext');
    if(pv&&!pv.disabled)pv.onclick=()=>irA(p-1); if(nx&&!nx.disabled)nx.onclick=()=>irA(p+1);});
}

// ── logs ──
function pintarLogs(lineas){const v=$('#logView');const cerca=v.scrollTop+v.clientHeight>=v.scrollHeight-50;
  v.innerHTML=lineas.map(l=>{const c=/❌|⛔|🚫|error|falló|inalcanzable/i.test(l)?'e':/⚠/i.test(l)?'w':/✅|🎛|👁|🧹|📥|♻/i.test(l)?'k':'';return `<div class="${c}">${esc(l)}</div>`;}).join('');
  if(cerca)v.scrollTop=v.scrollHeight;}
async function loadLogs(){try{const [g,info]=await Promise.all([api('/logs?n=500'),api('/logs/info')]);
  pintarLogs(g.lineas);$('#logInfo').textContent=`${fmtBytes(info.bytes)} en disco · ${info.lineas_buffer} líneas en vivo`;
  if($('#logVerbose')&&typeof info.verbose==='boolean'&&document.activeElement!==$('#logVerbose'))$('#logVerbose').checked=info.verbose;}catch(e){}}
function logAuto(){if(logTimer){clearInterval(logTimer);logTimer=null;}loadLogs();if($('#logAuto').checked)logTimer=setInterval(loadLogs,4000);}
$('#logAuto').onchange=logAuto;
if($('#logVerbose'))$('#logVerbose').onchange=async()=>{try{const r=await api('/logs/verbose',{method:'POST',body:JSON.stringify({verbose:$('#logVerbose').checked})});toast(r.verbose?'Log detallado (verboso)':'Log simple');loadLogs();}catch(e){toast(e.message,'bad');}};
async function purgaLog(body,msg){try{await api('/logs/purgar',{method:'POST',body:JSON.stringify(body)});toast(msg);loadLogs();}catch(e){toast(e.message,'bad');}}
...
$('#logClear').onclick=()=>{if(confirm('¿Vaciar TODO el log?'))purgaLog({todo:true},'Log vaciado');};

// ── integridad ──
function loadInteg(){$('#integDiag').onclick=()=>correrIntegridad(false);if($('#integFix'))$('#integFix').onclick=()=>correrIntegridad(true);
  if($('#sanDiag')){$('#sanDiag').onclick=sanearDiag;$('#sanRun').onclick=sanearRun;}
  if($('#visRefresh')){$('#visRefresh').onclick=loadVision;loadVision();}
  if($('#reindexBtn')){$('#reindexBtn').onclick=reindexar;loadIndice();}
  if($('#guestNsfw')){loadGuestNsfw();$('#guestNsfw').onchange=async()=>{const cb=$('#guestNsfw');try{const r=await api('/ajustes/guest-nsfw',{method:'POST',body:JSON.stringify({enabled:cb.checked})});toast(r.enabled?'Invitados: NSFW permitido':'Invitados: NSFW restringido');}catch(e){toast(e.message,'bad');cb.checked=!cb.checked;}};}}
async function loadGuestNsfw(){try{const r=await api('/ajustes/guest-nsfw');if($('#guestNsfw'))$('#guestNsfw').checked=!!r.enabled;}catch(e){}}
// ── Búsqueda: índice FTS local (estado + reindexar en 2º plano con progreso) ──
let reindexTimer=null;
async function loadIndice(){const el=$('#indiceEstado');if(!el)return;
  try{const r=await api('/busqueda/estado');const i=r.indice||{},t=r.trabajo||{};
    if(t.en_curso){const p=t.progreso||{};el.innerHTML=`<span class="tag warn">reindexando</span> ${p.hechos||0}/${p.total||'?'}`;
      if(!reindexTimer)reindexTimer=setInterval(loadIndice,1200);return;}
    if(reindexTimer){clearInterval(reindexTimer);reindexTimer=null;}
    if(t.error){el.innerHTML=`<span class="tag bad">error</span> ${esc(t.error)}`;return;}
    el.innerHTML=i.disponible?`<span class="tag ok">activo</span> ${i.total} documento(s) indexados`:'<span class="tag warn">sin índice</span> · búsqueda por Mongo — pulsa Reindexar';
  }catch(e){el.innerHTML='';}}
async function reindexar(){const b=$('#reindexBtn');if(!b)return;b.disabled=true;
  try{const r=await api('/busqueda/reindexar',{method:'POST',body:JSON.stringify({})});
    if(!r.ok){toast(r.motivo||'no se pudo reindexar','bad');b.disabled=false;return;}
    toast('Reindexando…');loadIndice();
  }catch(e){toast(e.message,'bad');}finally{setTimeout(()=>{b.disabled=false;},800);}}
async function loadVision(){const el=$('#visOut');if(!el)return;el.innerHTML='<div class="muted">Cargando…</div>';
  try{const r=await api('/vision/proveedores');const ps=r.proveedores||[];
    if(!ps.length){el.innerHTML='<div class="muted">Sin claves de visión en .env. Añade p. ej. GROQ_API_KEY_1=… y reinicia.</div>';return;}
    el.innerHTML=`<table><tr><th>Proveedor</th><th>Clave</th><th>Estado</th><th></th></tr>${ps.map(p=>`<tr data-id="${esc(p.id)}">
      ...
      <td style="text-align:right;white-space:nowrap"><button class="btn vprobar" data-id="${esc(p.id)}">Probar</button> <label class="switch" style="vertical-align:middle;margin-left:6px"><input type="checkbox" class="vena" data-id="${esc(p.id)}" ${p.enabled?'checked':''}><span class="slider"></span></label></td></tr>`).join('')}</table>`;
    $$('#visOut .vena').forEach(cb=>cb.onchange=async()=>{try{await api('/vision/proveedor',{method:'POST',body:JSON.stringify({id:cb.dataset.id,enabled:cb.checked})});toast(cb.checked?'Activado':'Desactivado');}catch(e){toast(e.message,'bad');cb.checked=!cb.checked;}});
    $$('#visOut .vprobar').forEach(b=>b.onclick=async()=>{b.disabled=true;const t=b.textContent;b.textContent='…';try{const r=await api('/vision/probar',{method:'POST',body:JSON.stringify({id:b.dataset.id})});toast(r.ok?`✓ OK (${r.ms} ms)`:`✗ ${r.motivo}`,r.ok?'ok':'bad');loadVision();}catch(e){toast(e.message,'bad');b.disabled=false;b.textContent=t;}});
  }catch(e){el.innerHTML='';toast(e.message,'bad');}}
// ── Sanear catálogo (re-home #, portadas, re-clasificar) ──
function sanResumen(inf){const d=inf.diagnostico||{},h=inf.hecho;
  const fila=(lab,v)=>`<tr><td>${esc(lab)}</td><td style="text-align:right"><b ${(typeof v==='number'?v>0:true)?'style="color:var(--warn)"':''}>${v}</b></td></tr>`;
  ...
  for(const[k,lab] of [['rehome','Carpetas a re-alojar'],['portada','Portadas a recuperar'],['reclasificar','Cómics mal archivados']]){
    if(m[k]?.length)html+=`<details style="margin-top:8px"><summary class="muted" style="cursor:pointer;font-size:12px">${lab} (${m[k].length}${m[k].length>=12?'+':''})</summary>${m[k].map(x=>`<div class="mono" style="font-size:11px"><a class="rowlink" data-doc="${esc(x.id)}">${esc(x.titulo||x.id)}</a>${x.ruta?' → '+esc(x.ruta):''}</div>`).join('')}</details>`;}
  return html;}
function sanWire(){$$('#sanOut [data-doc]').forEach(a=>a.onclick=()=>verDoc(a.dataset.doc,{volver:'activity',etiqueta:'Sanear'}));}
async function sanearDiag(){const b=$('#sanDiag');b.disabled=true;$('#sanOut').innerHTML='<div class="muted">Diagnosticando…</div>';
  try{const r=await api('/sanear',{method:'POST',body:'{}'});$('#sanOut').innerHTML=sanResumen(r);sanWire();}
  catch(e){$('#sanOut').innerHTML='';toast(e.message,'bad');}finally{b.disabled=false;}}
let sanTimer=null;
async function sanearRun(){const reclas=$('#sanReclas').checked;
  if(!confirm('Sanear el catálogo'+(reclas?' (incluye re-clasificación con IA, más lenta)':'')+'. Lo retirado va a la Papelera (recuperable). ¿Seguir?'))return;
  $('#sanRun').disabled=true;$('#sanDiag').disabled=true;
  try{const r=await api('/sanear/ejecutar',{method:'POST',body:JSON.stringify({reclasificar:reclas})});
    if(!r.ok){toast(r.motivo,'warn');$('#sanRun').disabled=false;$('#sanDiag').disabled=false;return;}
    if(sanTimer)clearInterval(sanTimer);
    sanTimer=setInterval(async()=>{try{const s=await api('/sanear/estado');
      if(s.en_curso)$('#sanOut').innerHTML=`<div class="muted">Saneando… ${s.hechos}/${s.total||'?'}</div>`;
      else{clearInterval(sanTimer);sanTimer=null;$('#sanRun').disabled=false;$('#sanDiag').disabled=false;
        if(s.error)toast('Saneo: '+s.error,'bad');
        else if(s.informe){$('#sanOut').innerHTML=sanResumen(s.informe);sanWire();toast('Saneo terminado');}}
    }catch(e){}},1500);
  }catch(e){toast(e.message,'bad');$('#sanRun').disabled=false;$('#sanDiag').disabled=false;}}
// Página "Actividad": vigilante + conformador + mantenimiento + integridad + logs (al final).
function loadActivity(){refrescarEstado();loadInteg();logAuto();aplicarPliegueActividad();}
...
function aplicarPliegueActividad(){
  ...
  $$('#p-activity details.foldcard[data-fold]').forEach(d=>{
    const id=d.id, sav=id?localStorage.getItem('fold_'+id):null;
    d.open = (sav!==null)?(sav==='1'):(d.dataset.fold==='open'?true:(d.dataset.fold==='pc'?pc:false));
    if(id&&!d._foldWired){d._foldWired=1;d.addEventListener('toggle',()=>localStorage.setItem('fold_'+id,d.open?'1':'0'));}
  });
}
const INTEG_FASES={cargando:'Cargando documentos…','docs-sin-carpeta':'Comprobando carpetas','docs-sin-fichero':'Comprobando ficheros','recorrido-arbol':'Recorriendo el árbol CDU','duplicados-hash':'Buscando duplicados por hash',cuarentena:'Revisando Cuarentena',reparando:'Reparando (a la Papelera)…',hecho:'Terminado'};
let integTimer=null;
async function correrIntegridad(reparar){
  if(reparar&&!confirm('Reparará la integridad: poda ramas vacías, deduplica por hash, resuelve Cuarentena/duplicados y ajusta ruta_base. Todo lo retirado va a la Papelera. ¿Seguir?'))return;
  const dB=$('#integDiag'),fB=$('#integFix');dB.disabled=true;if(fB)fB.disabled=true;
  ...
  try{
    ...
    if(!r.ok){toast(r.motivo||'no se pudo iniciar','warn');dB.disabled=false;if(fB)fB.disabled=false;return;}
    if(integTimer)clearInterval(integTimer);
    integTimer=setInterval(async()=>{try{const s=await api('/integridad/estado');
      if(s.en_curso){const p=s.progreso||{};const det=p.total?` (${p.i||0}/${p.total})`:(p.carpetas?` (${p.carpetas} carpetas)`:'');
        $('#integOut').innerHTML=`<div class="card"><div class="muted">⏳ ${esc(INTEG_FASES[s.fase]||s.fase||'Procesando')}${det}…</div></div>`;}
      else{clearInterval(integTimer);integTimer=null;dB.disabled=false;if(fB)fB.disabled=false;
        if(s.error){$('#integOut').innerHTML='';toast('Integridad: '+s.error,'bad');}
        else if(s.informe){pintarInteg(s.informe);toast(reparar?'Integridad: diagnóstico + reparación':'Integridad: diagnóstico completado');}}
    }catch(e){}},1500);
  }catch(e){$('#integOut').innerHTML='';toast(e.message,'bad');dB.disabled=false;if(fB)fB.disabled=false;}
}
// ¿qué fila se AUTO-repara con el botón? (las demás son solo diagnóstico → acción manual en la ficha)
const INTEG_AUTOREPARA=new Set(['ramasMuertas','carpetasHuerfanas','rutaBaseDesajustada','hashDuplicadosGrupos','hashDuplicadosDocs','cuarentenaDuplicados']);
function pintarInteg(r){
  const d=r.diagnostico||{},m=r.muestras||{};
  const et={docsSinCarpeta:'Docs sin carpeta',docsSinFicheroOriginal:'Docs sin fichero original',rutaBaseCompartida:'Varios docs en la misma carpeta',ramasMuertas:'Ramas vacías / muertas',registroSinDocumento:'Registro sin documento',carpetasHuerfanas:'Carpetas huérfanas',rutaBaseDesajustada:'ruta_base desajustada',hashDuplicadosGrupos:'Duplicados por hash (grupos)',hashDuplicadosDocs:'Duplicados por hash (sobrantes)',cuarentenaDuplicados:'Cuarentena/duplicados'};
  // clave de muestras por fila (las dos de hash comparten muestra)
  const mDe={docsSinCarpeta:'docsSinCarpeta',docsSinFicheroOriginal:'docsSinFicheroOriginal',rutaBaseCompartida:'rutaBaseCompartida',ramasMuertas:'ramasMuertas',registroSinDocumento:'registroSinDocumento',carpetasHuerfanas:'carpetasHuerfanas',rutaBaseDesajustada:'rutaBaseDesajustada',hashDuplicadosGrupos:'hashDuplicados',hashDuplicadosDocs:'hashDuplicados',cuarentenaDuplicados:'cuarentenaDuplicados'};
  const fila=(k)=>{const v=d[k]??0,mk=mDe[k],tiene=v>0&&m[mk]&&m[mk].length;
    const sello=v>0?(INTEG_AUTOREPARA.has(k)?'<span class="tag ok" title="Se arregla con «Diagnosticar y reparar»">auto</span>':'<span class="tag warn" title="Acción manual: abre la ficha y usa Reprocesar/Eliminar">manual</span>'):'';
  ...
  if(r.reparado){const rp=r.reparado,er={ramasPodadas:'Ramas podadas',rutasReparadas:'ruta_base reparadas',carpetasHuerfanasRecicladas:'Huérfanas recicladas',hashDuplicadosEliminados:'Hash-duplicados eliminados',cuarentenaResueltos:'Cuarentena resueltos'};
    h+=`<div class="card" style="margin-bottom:14px"><h3>🛠 Reparado (a la Papelera)</h3><table>`+Object.keys(er).map(k=>`<tr><td>${esc(er[k])}</td><td style="text-align:right"><b>${rp[k]??0}</b></td></tr>`).join('')+'</table></div>';}
...
function drillInteg(mk,m){
  const el=$('#integDetalle');if(!el)return;
  const arr=m[mk]||[];if(!arr.length){el.innerHTML='';return;}
  const dl=x=>`<a class="rowlink" data-doc="${esc(x.id)}">${esc(x.titulo||x.id)}</a>${x.archivo?` <span class="muted mono">(${esc(x.archivo)})</span>`:''}${x.isbn?` <span class="muted mono">ISBN ${esc(x.isbn)}</span>`:''}`;
  let h='';
  if(mk==='hashDuplicados')h=arr.map(g=>`<div class="intgrp">${g.docs.map(dl).join(' · ')}</div>`).join('');
  else if(mk==='rutaBaseCompartida')h=arr.map(g=>`<div class="intgrp"><span class="mono">${esc(g.ruta||'')}</span><br>${g.docs.map(dl).join('<br>')}</div>`).join('');
  else if(mk==='rutaBaseDesajustada')h=arr.map(x=>`<div class="intgrp">${dl(x)}<br><span class="muted">disco</span> <span class="mono">${esc(x.enDisco||'')}</span></div>`).join('');
  else if(mk==='docsSinCarpeta'||mk==='docsSinFicheroOriginal')h=arr.map(x=>`<div class="intgrp">${dl(x)}${x.ruta?` → <span class="mono">${esc(x.ruta)}</span>`:''}</div>`).join('');
  else h=arr.map(x=>`<div class="intgrp mono">${esc(typeof x==='string'?x:JSON.stringify(x))}</div>`).join('');
  ...
  $('#intX').onclick=()=>{el.innerHTML='';};
  $$('#integDetalle [data-doc]').forEach(a=>a.onclick=()=>verDoc(a.dataset.doc,{volver:'activity',etiqueta:'Integridad'}));
}
// ── Inbox OPERATIVO: subir ficheros/fotos con METADATOS que VIAJAN (ISBN/colección/obra/ubicación) ──
let inboxWired=false, camFotos=[];
let colaInbox=[], jobsHechos=[], colaCorriendo=false, jobSeq=0;   // cola de subida (autopiloto, no bloquea)
function loadInbox(){
  if(!inboxWired){inboxWired=true;wireInbox();}
...
async function escanearISBN(targetId='inIsbn'){
  if(!('BarcodeDetector' in window)){toast('Este navegador no trae lector de códigos (usa Chrome en Android). Escribe el ISBN.','warn');return;}
  let stream;
  try{stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}}});}
  catch(e){toast('No se pudo abrir la cámara: '+e.message,'bad');return;}
  let formatos=['ean_13','ean_8','upc_a','upc_e'];
  try{const sop=await BarcodeDetector.getSupportedFormats();formatos=formatos.filter(f=>sop.includes(f));if(!formatos.length)formatos=['ean_13'];}catch(_){}
  ...
  ov.style.cssText='position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.85);display:grid;place-items:center;padding:16px';
  ...
  const vid=ov.querySelector('#scVid');vid.srcObject=stream;try{await vid.play();}catch(_){}
  let activo=true;
  const parar=()=>{activo=false;try{stream.getTracks().forEach(t=>t.stop());}catch(_){}};
  const cerrar=()=>{parar();ov.remove();};
  ov.querySelector('#scX').onclick=cerrar;ov.onclick=e=>{if(e.target===ov)cerrar();};
  const loop=async()=>{
    if(!activo)return;
    try{
      const codes=await det.detect(vid);
      const hit=(codes||[]).map(c=>String(c.rawValue||'').replace(/\D/g,'')).find(v=>/^97[89]\d{10}$/.test(v));
      if(hit){parar();const inp=$('#'+targetId);if(inp)inp.value=hit;ov.remove();toast('ISBN leído: '+hit);return;}
    }catch(_){}
    setTimeout(loop,300);
  };
  loop();
...
async function detectarISBNenFiles(files){
  ...
  let det;try{det=new BarcodeDetector({formats:['ean_13']});}catch(_){return null;}
  for(const f of imgs){
    try{
      const bmp=await createImageBitmap(f);
      const codes=await det.detect(bmp);if(bmp.close)bmp.close();
      const hit=(codes||[]).map(c=>String(c.rawValue||'').replace(/\D/g,'')).find(v=>/^97[89]\d{10}$/.test(v));
      if(hit)return hit;
    }catch(_){}
  }
...
async function indiceConBarcode(files){
  ...
  let det;try{det=new BarcodeDetector({formats:['ean_13']});}catch(_){return -1;}
  for(let i=0;i<files.length;i++){const f=files[i];if(!/^image\//.test(f.type||''))continue;
    try{const bmp=await createImageBitmap(f);const codes=await det.detect(bmp);if(bmp.close)bmp.close();
      if((codes||[]).some(c=>/^97[89]\d{10}$/.test(String(c.rawValue||'').replace(/\D/g,''))))return i;}catch(_){}}
  return -1;
}
...
function elegirPortada(files){
  return new Promise(async resolve=>{
    ...
    let bcFile=null; const bi=await indiceConBarcode(orden).catch(()=>-1);
    if(bi>=0)bcFile=orden[bi];
    if(bi===0){const alt=orden.find((f,i)=>i!==0 && /^image\//.test(f.type||''));if(alt)orden=[alt,...orden.filter(f=>f!==alt)];}
    const cerrar=()=>{for(const u of urlMap.values())try{URL.revokeObjectURL(u);}catch(_){};$('#cmpScrim').style.display='none';$('#cmpModal').style.display='none';$('#cmpModal').innerHTML='';};
    const pintar=()=>{$('#cpGrid').innerHTML=orden.map((f,i)=>`<button type="button" class="cpThumb${i===0?' port':''}" data-i="${i}"><img src="${esc(urlMap.get(f))}" loading="lazy">${i===0?'<span class="cpBadge">PORTADA</span>':''}${f===bcFile?'<span class="cpBC">código</span>':''}</button>`).join('');
      $$('#cpGrid .cpThumb').forEach(el=>el.onclick=()=>{const i=+el.dataset.i;if(i>0){const f=orden[i];orden=[f,...orden.filter(x=>x!==f)];pintar();}});};
    $('#cmpModal').innerHTML=`<div class="box card" style="max-width:560px;max-height:92vh;overflow:auto"><h3 style="margin-top:0">🖼️ Elegir portada</h3>
    ...
    $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
    pintar();
    $('#cpX').onclick=()=>{cerrar();resolve(null);};
    $('#cpOk').onclick=()=>{cerrar();resolve(orden);};
  });
}
function wireInbox(){
  const dz=$('#dropZone'),fi=$('#fileInput');
  if($('#pickBtn'))$('#pickBtn').onclick=()=>fi&&fi.click();
  if(fi)fi.onchange=()=>{if(fi.files.length)subirInbox([...fi.files]);fi.value='';};
  ...
  if(fa)fa.onchange=()=>{if(fa.files.length)subirInbox([...fa.files]);fa.value='';};
  ...
  if(ac){ac.checked=localStorage.getItem('auto_compartir')!=='0';ac.onchange=()=>localStorage.setItem('auto_compartir',ac.checked?'1':'0');}
  // Modo Supervisado PERSISTENTE (no se reinicia al volver de Adobe Scan / recargar la PWA).
  const sv=$('#inSupervisar');
  if(sv){sv.checked=localStorage.getItem('inbox_supervisar')==='1';sv.onchange=()=>localStorage.setItem('inbox_supervisar',sv.checked?'1':'0');}
  // Modo Tapete PERSISTENTE: medir + recortar el libro sobre la alfombrilla al subir.
  const tp=$('#inTapete');
  if(tp){tp.checked=localStorage.getItem('inbox_tapete')==='1';tp.onchange=()=>localStorage.setItem('inbox_tapete',tp.checked?'1':'0');}
  // Elegir portada PERSISTENTE (por defecto activado).
  const inP=$('#inPortada');
  if(inP){inP.checked=localStorage.getItem('inbox_portada')!=='0';inP.onchange=()=>localStorage.setItem('inbox_portada',inP.checked?'1':'0');}
  // Calibración del tapete (foto del tapete vacío): fija su color de sesión (cualquier color / luz).
  const tcb=$('#inTapeteCal'),tcf=$('#inTapeteCalFile');
  if(tcb&&tcf){tcb.onclick=()=>tcf.click();
    tcf.onchange=async()=>{if(tcf.files[0]){try{const c=await calibrarTapete(tcf.files[0]);toast(`Tapete calibrado: rgb(${c.r},${c.g},${c.b})${c.crom?'':' · acromático'}`);}catch(e){toast('No se pudo calibrar: '+e.message,'bad');}}tcf.value='';pintarTapeteCalEstado();};
    pintarTapeteCalEstado();}
  const ttb=$('#inTapeteTest'),ttf=$('#inTapeteTestFile');
  if(ttb&&ttf){ttb.onclick=()=>ttf.click();
    ttf.onchange=async()=>{if(ttf.files[0]){try{await verMascaraTapete(ttf.files[0]);}catch(e){toast('No se pudo: '+e.message,'bad');}}ttf.value='';};}
  ...
  if(/Android/i.test(navigator.userAgent)){
    const aa=$('#appAdobe');if(aa)aa.style.display='';
    if($('#openScan'))$('#openScan').onclick=()=>{location.href='intent:#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;package=com.adobe.scan.android;S.browser_fallback_url=https%3A%2F%2Fplay.google.com%2Fstore%2Fapps%2Fdetails%3Fid%3Dcom.adobe.scan.android;end';};
  }
  if(dz){
    ['dragenter','dragover'].forEach(ev=>dz.addEventListener(ev,e=>{e.preventDefault();dz.classList.add('drag');}));
    ['dragleave','drop'].forEach(ev=>dz.addEventListener(ev,e=>{e.preventDefault();dz.classList.remove('drag');}));
    dz.addEventListener('drop',e=>{const f=[...((e.dataTransfer&&e.dataTransfer.files)||[])];if(f.length)subirInbox(f);});
  }
  if($('#inClear'))$('#inClear').onclick=()=>{['inIsbn','inColeccion','inObra','inAmbito','inEstanteria'].forEach(id=>{const el=$('#'+id);if(el)el.value='';});refrescarInboxUbic();};
  const ci=$('#camInput');
  ...
  if(oci)oci.onchange=()=>{const f=[...oci.files];oci.value='';if(f.length)subirInbox(f);};
  ...
  if($('#isbnIn'))$('#isbnIn').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();isbnBuscar();}});
}
...
async function isbnBuscar(){
  const inp=$('#isbnIn'),msg=$('#isbnMsg'),res=$('#isbnRes');
  const raw=((inp&&inp.value)||'').replace(/[^0-9Xx]/g,'');
  if(!raw){if(msg)msg.textContent='Escribe o escanea un ISBN.';return;}
...
function isbnRender(){
  const res=$('#isbnRes'); if(!res||!_isbnEstado)return; const m=_isbnEstado.meta;
...
function isbnConformar(id){
  const remoto=id[0]==='c'; const i=+id.slice(1);
  const item=remoto?_isbnEstado.portadas[i]:_isbnEstado.extra[i]; if(!item)return;
  const src=(!remoto&&item.base64)?(item.previa||item.base64)
...
async function isbnCrear(completar){
  ...
  _isbnEstado=null;$('#isbnRes').innerHTML='';if($('#isbnIn'))$('#isbnIn').value='';
  if(r.ya_existia){toast('Este libro ya estaba en la biblioteca','warn');if(msg)msg.innerHTML=avisoYaIngresado(r);}
  else{toast('📗 Documento creado');if(msg)msg.textContent='';}
...
async function cargarDatalistColecciones(){
  const dl=$('#dlColecciones');if(!dl)return;
  try{const r=await api('/colecciones');const cols=r.colecciones||r.items||(Array.isArray(r)?r:[]);
    dl.innerHTML=(Array.isArray(cols)?cols:[]).map(c=>`<option value="${esc(c.nombre||'')}">`).join('');
  }catch(e){}
}
...
let mapaUbicaciones=[];
function llenarDatalist(id,valores){const dl=$('#'+id);if(!dl)return;dl.innerHTML=(valores||[]).map(v=>`<option value="${esc(v)}">`).join('');}
function estanteriasDe(ambito){const a=(ambito||'').trim().toLowerCase();const e=mapaUbicaciones.find(x=>(x.ambito||'').trim().toLowerCase()===a);return e?e.estanterias:[];}
...
function montarSelUbic(o){
  ...
  const sa=$('#'+o.sa),ia=$('#'+o.ia),se=$('#'+o.se),ie=$('#'+o.ie);if(!sa||!ia||!se||!ie)return;
  const curA=(o.curA&&o.curA!==SIN)?o.curA:'',curE=(o.curE&&o.curE!==SIN)?o.curE:'';
  const mk=(arr,cur)=>{const set=[...new Set((arr||[]).filter(Boolean))];if(cur&&!set.includes(cur))set.unshift(cur);
    return ['<option value="">— sin asignar —</option>',...set.map(v=>`<option${v===cur?' selected':''}>${esc(v)}</option>`),'<option value="__new__">➕ Otra…</option>'].join('');};
  sa.innerHTML=mk(mapaUbicaciones.map(x=>x.ambito),curA);
  se.innerHTML=mk(estanteriasDe(curA),curE);
  ia.value=curA;ie.value=curE;ia.style.display='none';ie.style.display='none';
  sa.onchange=()=>{if(sa.value==='__new__'){ia.style.display='';ia.value='';ia.focus();}else{ia.style.display='none';ia.value=sa.value;}
    se.innerHTML=mk(estanteriasDe(ia.value),'');ie.value=(se.value==='__new__'?'':se.value);ie.style.display=(se.value==='__new__'?'':'none');};
  se.onchange=()=>{if(se.value==='__new__'){ie.style.display='';ie.value='';ie.focus();}else{ie.style.display='none';ie.value=se.value;}};
}
function refrescarInboxUbic(){if(!$('#inAmbSel'))return;montarSelUbic({sa:'inAmbSel',ia:'inAmbito',se:'inEstSel',ie:'inEstanteria',curA:($('#inAmbito')&&$('#inAmbito').value)||'',curE:($('#inEstanteria')&&$('#inEstanteria').value)||''});}
async function cargarUbicaciones(){
  try{const r=await api('/ubicaciones');mapaUbicaciones=r.ambitos||[];}catch(e){return;}
...
function pintarUbicSearch(){
  const sa=$('#sqAmbito');if(sa){const cur=sa.value;
    sa.innerHTML='<option value="">Todos</option>'+mapaUbicaciones.map(x=>`<option value="${esc(x.ambito)}">${esc(x.ambito)}</option>`).join('');
    sa.value=cur;}
  pintarEstanteriaSearch();
}
function pintarEstanteriaSearch(){
  const se=$('#sqEstanteria');if(!se)return;
  const amb=($('#sqAmbito')&&$('#sqAmbito').value)||'',cur=se.value,ests=estanteriasDe(amb);
  se.innerHTML='<option value="">Todas</option>'+ests.map(e=>`<option value="${esc(e)}">${esc(e)}</option>`).join('');
  se.disabled=!amb;
...
function reducirImagen(file,max=2000,q=0.85){
  return new Promise((res,rej)=>{
    ...
    img.onload=()=>{URL.revokeObjectURL(url);const w=img.naturalWidth,h=img.naturalHeight,s=Math.min(1,max/Math.max(w,h));
      if(s>=1){res(file);return;}
      const c=document.createElement('canvas');c.width=Math.round(w*s);c.height=Math.round(h*s);
      c.getContext('2d').drawImage(img,0,0,c.width,c.height);
      c.toBlob(b=>res(b?new File([b],(file.name||'foto')+'.jpg',{type:'image/jpeg'}):file),'image/jpeg',q);};
    img.onerror=()=>{URL.revokeObjectURL(url);rej(new Error('img'));};
    img.src=url;
  });
}
function renderCamThumbs(){
  const t=$('#camThumbs');if(t)t.innerHTML=camFotos.map((b,i)=>`<span style="position:relative;display:inline-block"><img src="${URL.createObjectURL(b)}" style="width:62px;height:82px;object-fit:cover;border-radius:6px;border:1px solid var(--line)"><button class="btn bad" type="button" data-rm="${i}" title="Quitar" style="position:absolute;top:-7px;right:-7px;padding:0 6px;border-radius:50%;line-height:18px">✕</button></span>`).join('');
...
async function camCatalogar(){
  ...
  const files=camFotos.slice();camFotos=[];renderCamThumbs();
...
function metaSnapshot(){
  const v=id=>($('#'+id)&&$('#'+id).value||'').trim();
  return { isbn:v('inIsbn'), coleccion:v('inColeccion'), obra:v('inObra'), ambito:v('inAmbito'), estanteria:v('inEstanteria') };
}
function fdDesdeSnap(snap, files){
  ...
  if(snap.ambito||snap.estanteria)fd.append('ubicacion',JSON.stringify({ambito:snap.ambito||'Sin asignar',estanteria:snap.estanteria||'Sin asignar'}));
  for(const f of files)fd.append('files',f,f.name);
  return fd;
}
async function enviarIngesta(fd){
  const r=await fetch('/api/ingestar',{method:'POST',headers:{'Authorization':'Bearer '+TOKEN},body:fd});
  if(r.status===403)throw new Error('Solo los administradores pueden dar de alta recursos.');
  return await r.json().catch(()=>({}));
}
function setInboxEstado(txt){const el=$('#inboxEstado');if(el)el.innerHTML=txt?`<span class="muted" style="font-size:13px">${esc(txt)}</span>`:'';}
const esPdf=f=>/\.pdf$/i.test((f&&f.name)||'')||((f&&f.type)==='application/pdf');
function opsImagenPdf(){const O=(window.pdfjsLib&&window.pdfjsLib.OPS)||{};return new Set([O.paintImageXObject,O.paintJpegXObject,O.paintInlineImageXObject,O.paintImageMaskXObject].filter(v=>v!=null));}
...
async function pdfAImagenes(file,{maxPag=60,ancho=1500}={}){
  ...
  try{
    const total=pdf.numPages,setImg=opsImagenPdf(),comprobar=Math.min(total,2);
    let conImg=0;
    for(let i=1;i<=comprobar;i++){const pg=await pdf.getPage(i);const ol=await pg.getOperatorList();if(ol.fnArray.some(fn=>setImg.has(fn)))conImg++;}
    if(!(comprobar>0&&conImg===comprobar))return {escaneo:false,pages:[]};   // PDF digital → no explotar
    const pages=[],n=Math.min(total,maxPag);
    for(let i=1;i<=n;i++){
      ...
      if(blob)pages.push(new File([blob],'pag-'+String(i).padStart(3,'0')+'.jpg',{type:'image/jpeg'}));
      c.width=c.height=0;
    }
...
function fileAImagen(f){return new Promise((res,rej)=>{const img=new Image();img.onload=()=>res(img);img.onerror=rej;img.src=URL.createObjectURL(f);});}
const _esImg=f=>/^image\//.test((f&&f.type)||'')||/\.(jpe?g|png|webp)$/i.test((f&&f.name)||'');
...
function rectificarLibro(work, quad, TOPE=2200){
  ...
  let W=Math.round((_dist(TL,TR)+_dist(BL,BR))/2), H=Math.round((_dist(TL,BL)+_dist(TR,BR))/2);
  if(W<16||H<16)return null;
  const f=Math.min(1,TOPE/Math.max(W,H));W=Math.round(W*f);H=Math.round(H*f);
  const Hi=_homografia([[0,0],[W,0],[W,H],[0,H]],quad); // rect de salida → fuente
  const sd=work.getContext('2d').getImageData(0,0,work.width,work.height).data,sw=work.width,sh=work.height;
  ...
  for(let y=0;y<H;y++)for(let x=0;x<W;x++){const[sx,sy]=_mapH(Hi,x+0.5,y+0.5);const ix=sx|0,iy=sy|0;const o=(y*W+x)*4;
    if(ix>=0&&iy>=0&&ix<sw&&iy<sh){const s=(iy*sw+ix)*4;od[o]=sd[s];od[o+1]=sd[s+1];od[o+2]=sd[s+2];od[o+3]=255;}else od[o+3]=255;}
  octx.putImageData(out,0,0);return oc;
}
...
async function recortarYMedirTapete(files){
  let dims=null, recortadas=0, tapetePct=null; const out=[];
  for(const f of files){
    if(!_esImg(f)){out.push(f);continue;}
    let url=null;
    try{
      const img=await fileAImagen(f); url=img.src;
      const work=document.createElement('canvas');work.width=img.naturalWidth;work.height=img.naturalHeight;work.getContext('2d').drawImage(img,0,0);
      if(tapetePct===null){try{tapetePct=Math.round(fraccionVerde(work)*100);}catch(_){}} // diagnóstico
      const q=detectarBordesVerde(work);
      if(!q){out.push(f);continue;}                          // sin tapete/libro → intacta
      if(!dims){const pxcm=detectarRejillaPxCm(work);
        if(pxcm){const W=(_dist(q[0],q[1])+_dist(q[3],q[2]))/2/pxcm,H=(_dist(q[0],q[3])+_dist(q[1],q[2]))/2/pxcm;
          if(W>=4&&W<=50&&H>=4&&H<=50)dims={ancho_cm:+W.toFixed(1),alto_cm:+H.toFixed(1)};}}
      const oc=rectificarLibro(work,q);
      if(oc){const blob=await new Promise(r=>oc.toBlob(r,'image/jpeg',0.9));
        out.push(blob?new File([blob],(f.name||'tapete.jpg').replace(/\.(png|webp|jpeg)$/i,'.jpg'),{type:'image/jpeg'}):f);recortadas++;}
      else out.push(f);
    }catch(e){console.warn('[tapete]',e&&e.message);out.push(f);}
...
async function guardarDimsResultados(resultados, dims){
  ...
  for(const x of resultados){ if(x&&x.ok&&x.id){ try{ await api('/documentos/'+encodeURIComponent(x.id)+'/dimensiones',{method:'POST',body:JSON.stringify(dims)}); }catch(_){} } }
}
...
async function subirInbox(files, extra){
  ...
  const msgs=[]; const log=(m,vivo=true)=>{if(m){msgs.push(m);if(vivo)setInboxEstado(m);}};
  ...
  if(files.length===1&&esPdf(files[0])&&!extra.isbn){
    try{
      ...
      if(res.escaneo&&res.pages.length){
        ...
        files=res.pages;
        log(`🧩 PDF → ${res.pages.length} página(s)${isbn?` · 📱 ISBN ${isbn} leído en el móvil`:' · sin código de barras (lo leerá el servidor)'}`);
      }
    }catch(e){console.warn('[PDF cliente] no explotado:',e.message);setInboxEstado('');}
  }
  ...
  if($('#inTapete')&&$('#inTapete').checked&&files.some(_esImg)){
    ...
    try{ const r=await recortarYMedirTapete(files); files=r.files; if(r.dims)extra.dims=r.dims;
      const pct=r.tapetePct!=null?` · tapete ${r.tapetePct}%`:'';
      if(r.recortadas)log(r.dims?`📐 ${String(r.dims.ancho_cm).replace('.',',')}×${String(r.dims.alto_cm).replace('.',',')} cm · recortado${pct}`:`✂️ Recortado (sin medida fiable)${pct}`);
      else log(`⚠️ No detecté el tapete${pct} — ¿deja margen alrededor del libro?`);
    }catch(e){console.warn('[tapete]',e&&e.message);}
  }
  ...
  {const imgs=files.filter(_esImg);
   if($('#inPortada')&&$('#inPortada').checked && imgs.length>=2 && imgs.length<=12 && imgs.length===files.length){
     setInboxEstado('🖼️ Elige la portada…');
     const r=await elegirPortada(files); if(!r){setInboxEstado('');return;} files=r; setInboxEstado('');
   }}
  const supervisado=$('#inSupervisar')&&$('#inSupervisar').checked;
  if(supervisado){
    let isbn=extra.isbn||null,origenMovil=extra.isbnOrigen==='movil';
    if(!isbn){setInboxEstado('🔎 Buscando código de barras en el móvil…');isbn=await detectarISBNenFiles(files);if(isbn)origenMovil=true;}
    if(isbn&&$('#inIsbn')&&!$('#inIsbn').value.trim())$('#inIsbn').value=isbn;
    if(isbn&&origenMovil)log(`📱 ISBN ${isbn} leído en el móvil`,false);
    setInboxEstado(isbn?`📱 ISBN ${isbn} · ⏳ subiendo y catalogando…`:'⏳ Subiendo y catalogando… (identificará el servidor)');
    const snap=metaSnapshot();if(isbn&&!snap.isbn)snap.isbn=isbn;if(snap.isbn&&origenMovil)snap.isbnOrigen='movil';
    try{
      ...
      setInboxEstado('');
      jobsHechos.unshift({estado:'ok',resultados:j.resultados||[],mensajes:msgs});cortarJobs();pintarCola();
      if($('#inIsbn'))$('#inIsbn').value='';
      cargarDatalistColecciones();cargarUbicaciones();
      const ok=(j.resultados||[]).filter(x=>x.ok&&x.id);
      if(ok.length===1)revisarSupervisado(ok[0].id);
    }catch(e){setInboxEstado('');jobsHechos.unshift({estado:'error',msg:e.message,mensajes:msgs});cortarJobs();pintarCola();}
    return;
  }
  // AUTOPILOTO: encolar (NO bloquea). La cola procesa una a una en segundo plano.
  const snap=metaSnapshot();
  if(extra.isbn){snap.isbn=extra.isbn;if(extra.isbnOrigen)snap.isbnOrigen=extra.isbnOrigen;}
  if($('#inIsbn'))$('#inIsbn').value='';
  setInboxEstado('');
  colaInbox.push({id:++jobSeq, files, snap, n:files.length, estado:'cola', dims:extra.dims||null, mensajes:msgs,
    titulo:extra.titulo||files.map(f=>f.name).join(', '), isbnCliente:(extra.isbnOrigen==='movil'?extra.isbn:null)});
  pintarCola();
  procesarCola();
}
async function procesarCola(){
  ...
  while(colaInbox.length){
    const job=colaInbox[0];job.estado='procesando';job.mensajes=job.mensajes||[];pintarCola();
    try{
      if(!job.snap.isbn){const isbn=await detectarISBNenFiles(job.files);if(isbn){job.snap.isbn=isbn;job.snap.isbnOrigen='movil';job.isbnCliente=isbn;job.mensajes.push(`📱 ISBN ${isbn} leído en el móvil`);}}
      ...
      job.estado='ok';job.resultados=j.resultados||[];
    }catch(e){job.estado='error';job.msg=e.message;}
    colaInbox.shift();jobsHechos.unshift(job);cortarJobs();pintarCola();
  }
  colaCorriendo=false;
  cargarDatalistColecciones();cargarUbicaciones();
}
function cortarJobs(){if(jobsHechos.length>25)jobsHechos.length=25;}
const _filaInb=html=>`<div style="padding:7px 0;border-top:1px solid rgba(255,255,255,.06)">${html}</div>`;
function filaResultado(r){
  if(!r.ok)return _filaInb(`<span class="tag bad">${esc(r.destino||'error')}</span> <span class="muted">${esc(r.error||'')}</span>`);
  const det=[r.tipo_recurso,(r.formatos||[]).join(','),(r.nImagenes?`${r.nImagenes} pág/img`:'')].filter(Boolean).join(' · ');
  const nota=r.nota?`<div class="muted" style="font-size:11px;margin-top:2px">↳ ${esc(r.nota)}</div>`:'';
  return _filaInb(`<span class="tag ${r.estado==='completado'?'ok':'warn'}">${esc(r.operacion||'ok')}</span> <b>${esc(recortar(r.titulo||'(sin título)',64))}</b> <span class="muted mono" style="font-size:11px">${esc(r.isbn||r.issn||'')}</span>${r.id?` · <a class="rowlink" data-doc="${esc(r.id)}">ver ficha</a>`:''}${det?`<div class="muted" style="font-size:11px">${esc(det)}</div>`:''}${nota}${avisoYaIngresado(r)}`);
}
...
function avisoYaIngresado(r){
  ...
  const dia=f&&!isNaN(f)?f.toLocaleDateString('es-ES',{day:'2-digit',month:'2-digit',year:'numeric'}):'—';
  const u=r.ubicacion_existente||r.ubicacion;
  const ub=(u&&(u.ambito||u.estanteria))?[u.ambito,u.estanteria].filter(x=>x&&x!=='Sin asignar').join(' · ')||'Sin asignar':'Sin asignar';
...
function pintarCola(){
  const out=$('#inboxResults');if(!out)return;
  // Mensajes del proceso (tapete, ISBN, visión…) BAJO el trabajo, en muted pequeño.
  const msgsHtml=j=>(j.mensajes||[]).map(m=>`<div class="muted" style="font-size:11px;margin-left:2px">· ${esc(m)}</div>`).join('');
  const cola=colaInbox.map(j=>_filaInb(`<span class="tag ${j.estado==='procesando'?'warn':'mut'}">${j.estado==='procesando'?'⏳ procesando':'🕒 en cola'}</span> <span class="muted">${esc(recortar(j.titulo,56))} · ${j.n} fich.</span>${msgsHtml(j)}`)).join('');
  const hechas=[];
  for(const j of jobsHechos){
    if(j.estado==='error'){hechas.push(_filaInb(`<span class="tag bad">error</span> <span class="muted">${esc(j.msg||'')}</span>${msgsHtml(j)}`));continue;}
    const ms=msgsHtml(j); if(ms)hechas.push(_filaInb(ms));
    for(const r of (j.resultados||[]))hechas.push(filaResultado(r));
  }
  out.innerHTML=(colaInbox.length||jobsHechos.length)
    ...
    :'';
  $$('#inboxResults [data-doc]').forEach(a=>a.onclick=()=>verDoc(a.dataset.doc,{volver:'inbox',etiqueta:'Inbox'}));
}
// Compat: resultados directos (p. ej. al descartar lo compartido) → a la lista de hechos.
function pintarInboxResultados(res){if(res&&res.length){jobsHechos.unshift({estado:'ok',resultados:res});cortarJobs();}pintarCola();}
...
async function revisarSupervisado(id){
  try{const r=await api('/documentos/'+encodeURIComponent(id));fichaEditar(r.doc,r,{supervisado:true});}
  catch(e){toast('No se pudo abrir para revisar: '+e.message,'bad');}
}
...
function docIdDeURL(s){try{const u=new URL(s,location.origin);return u.searchParams.get('doc')||'';}catch(_){return '';}}
async function leerNFC(){
  if(!('NDEFReader' in window)){toast('Este navegador no soporta NFC (Android + Chrome)','bad');return;}
  const out=$('#nfcOut');if(out)out.textContent='Acerca la etiqueta…';
  try{
    const reader=new NDEFReader();await reader.scan();
    reader.onreading=async ev=>{
      let url='',bib='',ex='';
      for(const rec of ev.message.records){try{
        const val=(rec.recordType==='text'||rec.recordType==='url')?new TextDecoder(rec.encoding||'utf-8').decode(rec.data):'';
        if(rec.recordType==='url'&&!url)url=val;
        else if(rec.recordType==='text'){ if(/^BIB1:/.test(val)){if(!bib)bib=val;} else if(!ex)ex=val; }
      }catch(_){}}
      ...
      const id=docIdDeURL(url), off=_offDeURL(url)||_parseOffline(bib);
      if(id){
        if(out)out.textContent='Conectando con la ficha…';
        let ok=false;
        try{const ctrl=new AbortController(),to=setTimeout(()=>ctrl.abort(),4000);
          const r=await fetch('/api/documentos/'+encodeURIComponent(id),{headers:TOKEN?{Authorization:'Bearer '+TOKEN}:{},signal:ctrl.signal});
          clearTimeout(to);ok=r.ok;}catch(_){ok=false;}
        if(ok){if(out)out.textContent='';verDoc(id,{volver:'inbox',etiqueta:'Inbox'});return;}
        if(off){if(out)out.textContent='';mostrarOfflineNFC(off,ex);return;}
        if(out)out.textContent='Sin conexión con el servidor y la etiqueta no trae datos offline.';return;
      }
      if(off){if(out)out.textContent='';mostrarOfflineNFC(off,ex);return;}     // etiqueta sin URL pero con datos
      // Etiqueta de ESTANTERÍA (?amb=&est=): muestra ex-libris + nombre + «Ver libros».
      try{const u=new URL(url,location.origin),amb=u.searchParams.get('amb');
        if(amb){const est=u.searchParams.get('est')||'';if(out)out.textContent='';mostrarEstanteriaNFC((est?est+' · ':'')+amb,amb,est,ex);return;}}catch(_){}
      const q=(ex||url||'').trim();
      if(q){if(out)out.innerHTML=`Leído: <b class="mono">${esc(q)}</b>`;go('search');const inp=$('#sqQ');if(inp){inp.value=q;buscarCatalogo(1);}}
      else if(out)out.textContent='Etiqueta sin texto legible.';
    };
    reader.onreadingerror=()=>{if(out)out.textContent='No se pudo leer la etiqueta.';};
  }catch(e){if(out)out.textContent='NFC: '+e.message;}
}
...
function _txtUbic(d){const u=(d&&d.ubicacion)||{};const a=u.ambito&&u.ambito!=='Sin asignar'?u.ambito:'';const e=u.estanteria&&u.estanteria!=='Sin asignar'?u.estanteria:'';return [a,e].filter(Boolean).join(' · ');}
...
function _payloadOffline(d,r,budget){
  ...
  let t=cut(d.titulo,40), a=cut((r&&r.autores&&r.autores.length)?r.autores.join(', '):'',28),
      e=cut((r&&r.editorial)||'',22), x=cut((r&&r.coleccion)||'',22),
      c=String(d.cdu||''), i=d.isbn||'', dd=String(d._id||''), u=cut(_txtUbic(d),30),
  ...
  const build=()=>'BIB1:'+JSON.stringify(Object.fromEntries(Object.entries({v:1,d:dd,u,t,i,c,x,a,e,s:va}).filter(([,v])=>v!=='')));
  // Salvaguarda final SOLO si la URL fuese muy larga: nunca toca ubicación/título/isbn/cdu.
  const pasos=[()=>e='',()=>x='',()=>a=cut(a,18),()=>a=''];
  let s=build(),k=0; while(_blen(s)>budget && k<pasos.length){pasos[k]();k++;s=build();}
  return s;
}
function _parseOffline(txt){if(!txt||txt.indexOf('BIB1:')!==0)return null;try{return JSON.parse(txt.slice(5));}catch(_){return null;}}
// base64url (UTF-8) para embeber los datos offline en la URL de la etiqueta (?o=…).
function _b64uEnc(s){try{return btoa(unescape(encodeURIComponent(s))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');}catch(_){return '';}}
function _b64uDec(s){try{return decodeURIComponent(escape(atob(String(s).replace(/-/g,'+').replace(/_/g,'/'))));}catch(_){return '';}}
// Datos offline embebidos en una URL de etiqueta (?o=<base64url(BIB1)>): {…} o null.
function _offDeURL(url){try{const o=new URL(url,location.origin).searchParams.get('o');return o?_parseOffline(_b64uDec(o)):null;}catch(_){return null;}}
...
function inspeccionarTagNFC(message){
  let url='',txt='';
  for(const rec of (message&&message.records)||[]){try{
    const val=(rec.recordType==='text'||rec.recordType==='url')?new TextDecoder(rec.encoding||'utf-8').decode(rec.data):'';
    if(rec.recordType==='url'&&!url)url=val; else if(rec.recordType==='text'&&!txt)txt=val;
  }catch(_){}}
  const docId=docIdDeURL(url); let off=_parseOffline(txt)||_offDeURL(url);
...
function _recordsDoc(d,r){
  ...
  return {url, records:[{recordType:'url',data:url},{recordType:'text',data:EX_LIBRIS}]};
}
...
function escribirNFC(records, docIdActual, outerSignal, forzar){
  return new Promise((resolve,reject)=>{
    ...
    const onAb=()=>{try{ac.abort();}catch(_){}};
    if(outerSignal){ if(outerSignal.aborted)onAb(); else outerSignal.addEventListener('abort',onAb,{once:true}); }
    let done=false; const fin=()=>{done=true;try{ac.abort();}catch(_){}if(outerSignal)try{outerSignal.removeEventListener('abort',onAb);}catch(_){}};
    let rd; try{rd=new NDEFReader();}catch(e){fin();return reject(e);}
    rd.onreading=async ev=>{ if(done)return; const uid=ev.serialNumber||null;
      if(!forzar){ const prev=inspeccionarTagNFC(ev.message); if(prev && prev.docId!==docIdActual){ fin(); reject({code:'OCUPADA',prev,uid}); return; } }
      try{ await rd.write({records},{signal:ac.signal}); fin(); resolve(uid); }
      catch(err){ if(!done){fin(); reject(err);} } };
    rd.scan({signal:ac.signal}).catch(err=>{ if(!done){fin(); reject(err);} });
  });
}
...
function heroExlibris(ex){
  if(!ex)return '';
  const p=String(ex).split('·').map(s=>s.trim()).filter(Boolean);
  const lib=p[0]||ex, prop=p.slice(1).join(' · ');
...
const estrellasHTML=n=>{n=+n||0;return n?`<span style="color:#ffcf5a;font-size:16px;letter-spacing:2px" title="${n}★">${'★'.repeat(n)}${'☆'.repeat(Math.max(0,5-n))}</span>`:'';};
...
function fichaMinima(o){
  const badge=o.origen?`<div class="fmin-badge" onclick="this.classList.toggle('open')" title="¿De dónde salen estos datos?">ⓘ<span class="fmin-pop">${esc(o.origen)}</span></div>`:'';
  const starsInner=(o.estrellasHTML!=null)?o.estrellasHTML:(o.estrellas?estrellasHTML(o.estrellas):'');
  const hero=o.esDigital
    ? (o.descargaUrl?`<a class="btn pri fmin-dl" href="${esc(o.descargaUrl)}" download="${esc(o.descargaNombre||'')}">⬇ Descargar</a>`:'')
    : heroExlibris(o.exlibris||EX_LIBRIS);
  let filas=o.datosHTML;
  if(filas==null){
    filas=[['Autor',o.autor],['Editorial',o.editorial],['Colección',o.coleccion],['CDU',o.cdu,'mono'],['ISBN',o.isbn,'mono']]
      .filter(function(p){return p[1];}).map(function(p){return `<dt>${p[0]}</dt><dd${p[2]?` class="${p[2]}"`:''}>${esc(p[1])}</dd>`;}).join('');
  }
  const pie=(o.esDigital||o.ocultarUbicacion)?''
    :((o.ubicacionHTML!=null)?o.ubicacionHTML
...
function qrCanvas(qr,px){
  const quiet=4,n=qr.size,total=n+quiet*2,scale=Math.max(2,Math.floor((px||260)/total)),W=total*scale;
  const cv=document.createElement('canvas');cv.width=W;cv.height=W;
  cv.style.cssText='width:'+W+'px;max-width:100%;height:auto;border-radius:10px;image-rendering:pixelated';
  const g=cv.getContext('2d');g.fillStyle='#fff';g.fillRect(0,0,W,W);g.fillStyle='#000';
  for(let y=0;y<n;y++)for(let x=0;x<n;x++)if(qr.mod[y][x])g.fillRect((quiet+x)*scale,(quiet+y)*scale,scale,scale);
  return cv;
}
...
async function compartirDoc(d){
  ...
  let token;
  try{const res=await api('/documentos/'+encodeURIComponent(d._id)+'/compartir',{method:'POST',body:'{}'});token=res.token;}
  catch(e){toast(e.message,'bad');return;}
  const url=location.origin+'/?s='+token;
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:360px;text-align:center"><h3 style="margin-top:0">🔗 Compartir ficha</h3>
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';$('#cmpScrim').onclick=cerrarCmp;
  let cv=null;
  try{cv=qrCanvas(qrGenerar(url),260);const box=$('#qrBox');if(box)box.appendChild(cv);}catch(e){const box=$('#qrBox');if(box)box.innerHTML='<span class="muted" style="font-size:12px">No se pudo generar el QR; usa el enlace.</span>';}
    ...
    sh.onclick=async()=>{
      try{
        if(cv&&cv.toBlob&&navigator.canShare){
          ...
          if(file&&navigator.canShare({files:[file]})){await navigator.share({title:d.titulo||'Ficha',text:url,files:[file]});return;}
        }
...
function _sharedCont(){
  ...
  if(!c){c=document.createElement('div');c.id='sharedView';c.style.cssText='max-width:640px;margin:0 auto;padding:16px 14px 40px';document.body.appendChild(c);}
  return c;
}
...
const _brandLine='<div style="text-align:center;margin:8px 0 16px"><div style="display:inline-block;text-align:justify;text-align-last:justify;font-family:Georgia,\'Times New Roman\',serif;color:var(--acc)"><div style="font-weight:700;letter-spacing:1.5px;font-size:14px">BIBLIOTHECA LUDOVICIANA</div><div class="muted" style="font-size:10px;margin-top:3px">© Luis Ortuño Molina · MMXXVI</div></div></div>';
...
function pantallaEsperaNFC(){
  ...
  const b=document.getElementById('offRead');if(b)b.onclick=()=>{try{leerNFC();}catch(_){}}
  if('NDEFReader' in window)setTimeout(()=>{try{leerNFC();}catch(_){}},300);
}
...
async function vistaCompartida(token){
  ...
  let data;
  try{data=await fetch('/api/compartido/'+encodeURIComponent(token)).then(r=>r.json());}catch(e){data=null;}
  if(!data||!data.ok||!data.ficha){cont.innerHTML='<p class="muted" style="text-align:center;margin-top:50px">Enlace no válido o documento no disponible.</p>';return;}
  const f=data.ficha;
  const portada=f.portada?`<div style="text-align:center;margin-bottom:14px"><img src="${esc(encUrl(f.portada))}" style="max-height:260px;max-width:78%;border-radius:10px;box-shadow:0 6px 20px rgba(0,0,0,.5)"></div>`:'';
  const fmin=fichaMinima({
    titulo:f.titulo,subtitulo:f.subtitulo,estrellas:f.valoracion,
    esDigital:f.es_digital,exlibris:EX_LIBRIS,ocultarUbicacion:true,   // nunca se expone la ubicación física
    descargaUrl:f.descarga_url?encUrl(f.descarga_url):'',descargaNombre:f.nombre_archivo||'',
    autor:(f.autores||[]).join(', '),editorial:f.editorial||'',coleccion:f.coleccion||'',
    cdu:f.cdu||'',isbn:f.isbn||'',
    origen:'Ficha compartida (solo lectura). No da acceso al resto de la biblioteca.'
  });
  const sinopsis=f.sinopsis?`<details class="card foldcard" open style="margin-top:14px"><summary>📝 Sinopsis</summary><p class="sinopsis-text" style="margin-top:10px">${esc(f.sinopsis)}</p></details>`:'';
...
function mostrarOfflineNFC(o,ex){
  ...
  const fmin=fichaMinima({titulo:o.t,estrellas:o.s,esDigital:false,exlibris:ex||EX_LIBRIS,
    autor:o.a||'',editorial:o.e||'',coleccion:o.x||'',cdu:o.c||'',isbn:o.i||'',ubicacion:o.u||'',
  ...
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:460px;padding:0;overflow:hidden;border:none">${fmin}
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
  const cl=$('#nfcCerr');if(cl)cl.onclick=cerrarCmp; $('#cmpScrim').onclick=cerrarCmp;
}
...
function mostrarEstanteriaNFC(label,amb,est,ex){
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
  const c=$('#nfcCerr2');if(c)c.onclick=cerrarCmp; $('#cmpScrim').onclick=cerrarCmp;
  const v=$('#nfcVerEst');if(v)v.onclick=()=>{cerrarCmp();estadoBusqueda.extra={ambito:amb,estanteria:est||undefined,etiqueta:'📍 '+(label||amb)};estadoBusqueda.page=1;go('search');};
}
...
async function grabarNFC(d,r){
  ...
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:420px;text-align:center"><h3 style="margin-top:0">📶 Grabar etiqueta NFC</h3>
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
  const ctrl=new AbortController();
  const cerrar=()=>{try{ctrl.abort();}catch(_){}cerrarCmp();};
  $('#nfcWrX').onclick=cerrar;$('#cmpScrim').onclick=cerrar;
  const marcar=async uid=>{try{return await api('/documentos/'+encodeURIComponent(d._id)+'/nfc',{method:'POST',body:JSON.stringify({uid,url})});}catch(_){return null;}};
  ...
  const escribir=async records=>{
    try{ return await escribirNFC(records,d._id,ctrl.signal,false); }
    catch(eo){
      if(eo&&eo.code==='OCUPADA'){
        const det=eo.prev&&eo.prev.titulo?`de «${recortar(eo.prev.titulo,40)}»`:(eo.prev&&eo.prev.docId?'de OTRO libro':'con datos ajenos');
        if(!confirm('⚠️ Esta etiqueta YA está grabada '+det+'.\n¿Quizá es el libro equivocado? Pulsa Aceptar para SOBRESCRIBIR.')) throw {code:'CANCELADO'};
        const m=$('#nfcWrMsg');if(m){m.textContent='Acerca de nuevo la etiqueta para sobrescribir…';m.style.color='var(--warn)';}
  ...
  try{
    const uid=await escribir(recs); const res=await marcar(uid);
    const rea=res&&res.reasignado;
    const m=$('#nfcWrMsg');if(m){m.innerHTML='✅ Etiqueta grabada y vinculada'+(uid?' · UID '+esc(uid):'')+'.'+(rea?`<br><span style="color:var(--warn)">⚠️ Estaba en «${esc(recortar(rea.titulo||'?',40))}» — reasignada a este libro.</span>`:'');m.style.color='var(--ok)';}
    toast(rea?'Etiqueta reasignada (estaba en otro libro)':'Etiqueta NFC grabada',rea?'warn':'ok');setTimeout(cerrarCmp,rea?2200:1100);
  }catch(e){
    if(e&&(e.code==='CANCELADO')){cerrarCmp();return;}
    if(e&&e.name==='AbortError')return;                            // cancelado por el usuario
    const m=$('#nfcWrMsg');if(m){m.innerHTML=`No cupo con datos offline (${esc(e.message||'')}). <button class="btn" id="nfcSolo" style="margin-top:8px">Grabar solo el enlace</button>`;m.style.color='var(--bad)';}
    const so=$('#nfcSolo');if(so)so.onclick=async()=>{try{const uid=await escribir([{recordType:'url',data:url},{recordType:'text',data:EX_LIBRIS}]);await marcar(uid);if(m){m.textContent='✅ Enlace grabado (sin datos offline).';m.style.color='var(--ok)';}toast('Etiqueta NFC grabada');setTimeout(cerrarCmp,1100);}catch(e2){if(e2&&e2.code==='CANCELADO'){cerrarCmp();return;}if(e2&&e2.name!=='AbortError'&&m){m.textContent='No se pudo grabar: '+(e2.message||'');}}};
  }
}
...
let _etq=null, _etqGen=0; const ETQ_LS='nfc_cola';
function guardarColaEtq(){try{ if(_etq&&_etq.i<_etq.ids.length) localStorage.setItem(ETQ_LS,JSON.stringify({ids:_etq.ids.slice(_etq.i),auto:_etq.auto})); else localStorage.removeItem(ETQ_LS); }catch(_){}}
function colaEtqGuardada(){try{const j=JSON.parse(localStorage.getItem(ETQ_LS)||'null');return (j&&Array.isArray(j.ids)&&j.ids.length)?j:null;}catch(_){return null;}}
function iniciarEtiquetadoLote(ids,auto){
  if(!('NDEFReader' in window)){toast('Este navegador no soporta NFC (Android + Chrome)','bad');return;}
  ids=[...new Set((ids||[]).filter(Boolean))];
  if(!ids.length){toast('No hay libros seleccionados','warn');return;}
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';$('#cmpScrim').onclick=null; // no cerrar por fuera
  procesarEtq();
}
async function procesarEtq(){
  if(!_etq)return;
  if(_etq.abort){try{_etq.abort.abort();}catch(_){}}            // cancela el anterior (sus resultados se ignoran por gen)
  const gen=++_etqGen;
  if(_etq.i>=_etq.ids.length){finEtq();return;}
  guardarColaEtq();
  pintarEtq(null,null,'cargando');
  const id=_etq.ids[_etq.i]; let r=null,doc=null;
  try{ r=await api('/documentos/'+encodeURIComponent(id)); doc=r.doc; }catch(_){}
  if(gen!==_etqGen)return;
  if(!doc){pintarEtq(null,null,'errorDoc');return;}
  const {url,records:recs}=_recordsDoc(doc,r);
  _etq.actual={doc,r,url}; pintarEtq(doc,r,'esperando');
  const abort=new AbortController(); _etq.abort=abort;
  try{
    const uid=await escribirNFC(recs,id,abort.signal,false); if(gen!==_etqGen)return;
    await etqTrasGrabar(doc,r,uid,url,gen);
  }catch(e){ if(gen!==_etqGen)return;
    if(e&&e.code==='OCUPADA'){ _etq.pend={recs,id,url}; pintarEtq(doc,r,'ocupada',e.prev); return; }
    if(e&&e.name==='AbortError'){pintarEtq(doc,r,'pausa');return;}
...
async function etqTrasGrabar(doc,r,uid,url,gen){
  let res=null; try{res=await api('/documentos/'+encodeURIComponent(doc._id)+'/nfc',{method:'POST',body:JSON.stringify({uid,url})});}catch(_){}
  doc.nfc={uid:uid||undefined,fecha_vinculacion:new Date().toISOString()};
  try{navigator.vibrate&&navigator.vibrate(70);}catch(_){}
  if(res&&res.reasignado)toast('⚠️ Etiqueta reasignada (estaba en «'+recortar(res.reasignado.titulo||'?',30)+'»)','warn');
  pintarEtq(doc,r,'ok',uid);
  if(_etq&&_etq.auto)setTimeout(()=>{ if(_etq&&_etqGen===gen){_etq.i++;procesarEtq();} },900);
}
// SOBRESCRIBIR (confirmado) la etiqueta ocupada del documento actual.
async function etqSobrescribir(){
  if(!_etq||!_etq.pend||!_etq.actual)return; const {recs,id,url}=_etq.pend; _etq.pend=null;
  const {doc,r}=_etq.actual; const gen=++_etqGen; const abort=new AbortController(); _etq.abort=abort; pintarEtq(doc,r,'esperando');
  try{ const uid=await escribirNFC(recs,id,abort.signal,true); if(gen!==_etqGen)return; await etqTrasGrabar(doc,r,uid,url,gen); }
  catch(e){ if(gen!==_etqGen)return; if(e&&e.name==='AbortError'){pintarEtq(doc,r,'pausa');return;} pintarEtq(doc,r,'error',e&&e.message); }
}
async function etqSoloEnlace(){
  if(!_etq||!_etq.actual)return; const {doc,r,url}=_etq.actual; const gen=++_etqGen;
  const abort=new AbortController(); _etq.abort=abort; pintarEtq(doc,r,'esperando');
  try{ const uid=await escribirNFC([{recordType:'url',data:url},{recordType:'text',data:EX_LIBRIS}],doc._id,abort.signal,true); if(gen!==_etqGen)return; await etqTrasGrabar(doc,r,uid,url,gen); }
  catch(e){ if(gen!==_etqGen)return; if(e&&e.name==='AbortError'){pintarEtq(doc,r,'pausa');return;} pintarEtq(doc,r,'error',e&&e.message); }
}
function finEtq(){const n=_etq?_etq.ids.length:0;localStorage.removeItem(ETQ_LS);_etqGen++;_etq=null;cerrarCmp();toast(`Etiquetado terminado (${n} libro/s)`);if($('#sqQ'))buscarCatalogo(estadoBusqueda.page||1);}
function vaciarEtq(){_etqGen++;if(_etq&&_etq.abort){try{_etq.abort.abort();}catch(_){}}localStorage.removeItem(ETQ_LS);_etq=null;cerrarCmp();toast('Cola de etiquetado vaciada','warn');if($('#sqQ'))buscarCatalogo(estadoBusqueda.page||1);}
function cerrarEtqGuardando(){_etqGen++;if(_etq&&_etq.abort){try{_etq.abort.abort();}catch(_){}}guardarColaEtq();_etq=null;cerrarCmp();toast('Etiquetado en pausa — «Reanudar» cuando quieras');if($('#sqQ'))buscarCatalogo(estadoBusqueda.page||1);}
function pintarEtq(doc,r,estado,extra){
  const b=$('#etqBody'); if(!b||!_etq)return;
  const tot=_etq.ids.length, n=Math.min(_etq.i+1,tot);
  const cov=doc&&doc.portada?`<img src="${esc(encUrl(doc.portada))}" style="width:64px;height:86px;object-fit:cover;border-radius:6px;border:1px solid var(--line)">`:`<div style="width:64px;height:86px;display:grid;place-items:center;background:var(--bg2);border-radius:6px;font-size:26px">📕</div>`;
  ...
  const cP=`<button class="btn" id="etqPausa">⏸ Pausar</button>`, cS=`<button class="btn" id="etqSaltar">⏭ Saltar</button>`, cX=`<button class="btn bad" id="etqCerrar">✕ Vaciar cola</button>`;
  let est='',bot='';
  if(estado==='esperando'){est='<span style="color:var(--acc);font-weight:600">📶 Acerca la etiqueta al móvil…</span>';bot=cP+cS+cX;}
  else if(estado==='ok'){est=`<span style="color:var(--ok);font-weight:600">✅ Grabada${extra?' · UID '+esc(extra):''}</span>`;bot=(_etq.auto?'<span class="muted">siguiente…</span>':`<button class="btn pri" id="etqSig">Siguiente ▶</button>`)+cX;}
  else if(estado==='error'){est=`<span style="color:var(--bad)">No se pudo grabar: ${esc(extra||'')}</span>`;bot=`<button class="btn" id="etqReint">↻ Reintentar</button><button class="btn" id="etqSolo">Solo enlace</button>`+cS+cX;}
  else if(estado==='pausa'){est='<span class="muted">⏸ En pausa</span>';bot=`<button class="btn pri" id="etqCont">▶ Continuar</button>`+cS+cX;}
  else if(estado==='ocupada'){est=`<span style="color:var(--warn)">⚠️ Esta etiqueta YA está grabada${extra&&extra.titulo?` de «${esc(recortar(extra.titulo,30))}»`:(extra&&extra.docId?' de OTRO libro':' (datos ajenos)')}. ¿Libro equivocado?</span>`;bot=`<button class="btn pri" id="etqSobre">Sobrescribir</button>`+cS+cX;}
  else if(estado==='errorDoc'){est='<span style="color:var(--bad)">No se pudo cargar este documento.</span>';bot=cS+cX;}
  else {est='<span class="muted">Cargando…</span>';bot=cX;}
  ...
  const A=$('#etqAuto'); if(A)A.onchange=()=>{_etq.auto=A.checked;guardarColaEtq();if(_etq.auto&&estado==='ok'){_etq.i++;procesarEtq();}};
  const w=(id,fn)=>{const e=$('#'+id);if(e)e.onclick=fn;};
  w('etqCerrar',vaciarEtq);
  w('etqMin',cerrarEtqGuardando);
  w('etqSaltar',()=>{_etq.i++;procesarEtq();});
  w('etqSig',()=>{_etq.i++;procesarEtq();});
  w('etqCont',()=>procesarEtq());
  w('etqPausa',()=>{ if(_etq.abort){try{_etq.abort.abort();}catch(_){}} });
...
async function loadUbic(){
  const cont=$('#p-ubic');if(cont&&!cont.innerHTML)cont.innerHTML='<div class="muted" style="padding:22px">Cargando…</div>';
  try{const r=await api('/ubicaciones/gestion');ubicArbol=r.ambitos||[];}catch(e){if(cont)cont.innerHTML=`<div class="empty">${esc(e.message)}</div>`;return;}
  pintarUbic();
}
function pintarUbic(){
...
function wireUbic(){
  const modo=$('#ubModo');
  const sync=()=>{const lote=modo&&modo.value==='lote';if($('#ubUna'))$('#ubUna').style.display=lote?'none':'';if($('#ubLote'))$('#ubLote').style.display=lote?'':'none';ubicPreview();};
  if(modo)modo.onchange=sync;
  ['ubPre','ubDesde','ubHasta','ubAncho','ubEstUna'].forEach(id=>{const el=$('#'+id);if(el)el.oninput=ubicPreview;});
...
function ubicLoteNombres(){
  ...
  const d=parseInt($('#ubDesde').value,10),h=parseInt($('#ubHasta').value,10),w=Math.max(1,parseInt($('#ubAncho').value,10)||1);
  if(!Number.isFinite(d)||!Number.isFinite(h)||h<d)return [];
  const out=[];for(let i=d;i<=h&&out.length<500;i++)out.push(pre+String(i).padStart(w,'0'));
  return out;
}
function ubicPreview(){
  const box=$('#ubPrev');if(!box)return;
  if($('#ubModo')&&$('#ubModo').value==='lote'){const n=ubicLoteNombres();box.textContent=n.length?`${n.length} estantería(s): ${n[0]} … ${n[n.length-1]}`:'Rango no válido.';}
  else{const e=($('#ubEstUna')&&$('#ubEstUna').value||'').trim();box.textContent=e?`1 estantería: ${e}`:'Escribe el nombre (o déjalo vacío para registrar solo el ámbito).';}
}
async function ubicCrear(){
  const amb=($('#ubAmb')&&$('#ubAmb').value||'').trim();if(!amb){toast('Indica el ámbito','warn');return;}
  let ests=[];
  if($('#ubModo').value==='lote'){ests=ubicLoteNombres();if(!ests.length){toast('Rango no válido','warn');return;}}
  else{const e=($('#ubEstUna').value||'').trim();if(e)ests=[e];}
...
async function ubicApi(path,body){
  try{const r=await api(path,{method:'POST',body:JSON.stringify(body)});
    if(!r.ok){toast(r.motivo||'error','bad');return r;}
    const det=r.modificados!=null?` · ${r.modificados} libro(s)`:r.liberados!=null?` · ${r.liberados} liberado(s)`:r.movidos!=null?` · ${r.movidos} movido(s)`:r.creadas!=null?` · ${r.creadas} creada(s)`:'';
...
async function ubicAccion(act,a,e){
  try{
    ...
    if(act==='est-mover'){const na=await ubicPedirTexto('Mover «'+e+'» a qué ámbito',a,ambitosNombres());if(!na||na===a)return;return void ubicApi('/ubicaciones/mover',{ambito:a,estanteria:e,nuevoAmbito:na});}
    if(act==='est-fus'){const de=await ubicPedirTexto('Fusionar «'+e+'» en qué estantería (mismo ámbito)','',estanteriasNombres(a).filter(x=>x!==e));if(!de||de===e)return;return void ubicApi('/ubicaciones/fusionar',{ambito:a,estanteria:e,destinoAmbito:a,destinoEstanteria:de});}
    if(act==='amb-explotar'){if(!await ubicConfirm('Explotar ámbito «'+a+'»','Todos sus libros quedarán SIN ubicación. ¿Seguir?'))return;return void ubicApi('/ubicaciones/explotar',{ambito:a});}
    if(act==='est-explotar'){if(!await ubicConfirm('Explotar estantería «'+e+'»','Sus libros quedarán SIN ubicación. ¿Seguir?'))return;return void ubicApi('/ubicaciones/explotar',{ambito:a,estanteria:e});}
...
async function ubicVerLibros(ambito,estanteria){
  ...
  const box=$('#ubicLibros');if(!box)return;
  box.innerHTML='<div class="muted" style="padding:14px">Cargando libros…</div>';
  const p=new URLSearchParams({ambito,page:'1'});if(estanteria)p.set('estanteria',estanteria);
  try{const r=await api('/catalogo?'+p.toString());
    const tit=esc(ambito)+(estanteria?' · '+esc(estanteria):'');
    box.innerHTML=`<div class="card"><div class="sec-h" style="margin-bottom:8px"><h3 style="margin:0">📚 ${tit}</h3><span class="muted">${r.total} libro(s)</span></div>`+
      (r.docs.length?`<div class="ubgrid">${r.docs.map(ubicCard).join('')}</div>${r.total>r.docs.length?`<div class="muted" style="margin-top:10px">Mostrando ${r.docs.length} de ${r.total}. <a class="rowlink" id="ubVerBusq">Ver todos en Búsqueda →</a></div>`:''}`:'<div class="muted">Sin libros en esta ubicación.</div>')+`</div>`;
    $$('#ubicLibros [data-doc]').forEach(el=>el.onclick=()=>verDoc(el.dataset.doc,{volver:'ubic',etiqueta:'Ubicaciones'}));
    const vb=$('#ubVerBusq');if(vb)vb.onclick=()=>{estadoBusqueda.extra={ambito,estanteria:estanteria||undefined,etiqueta:'📍 '+ambito+(estanteria?' · '+estanteria:'')};estadoBusqueda.page=1;go('search');};
...
function ubicPedirTexto(titulo,valor='',sugerencias=null){
  return new Promise(res=>{
    const dl=sugerencias?`<datalist id="ptDl">${sugerencias.map(s=>`<option value="${esc(s)}">`).join('')}</datalist>`:'';
    $('#cmpModal').innerHTML=`<div class="box card" style="max-width:420px"><h3 style="margin-top:0">${esc(titulo)}</h3>
    ...
    $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
    const fin=v=>{cerrarCmp();res(v);};
    $('#ptX').onclick=()=>fin(null);$('#cmpScrim').onclick=()=>fin(null);
    $('#ptOk').onclick=()=>fin(($('#ptVal').value||'').trim());
    const inp=$('#ptVal');if(inp)inp.onkeydown=ev=>{if(ev.key==='Enter')fin((inp.value||'').trim());};
    setTimeout(()=>{if(inp){inp.focus();inp.select();}},30);
  });
}
function ubicConfirm(titulo,txt){
  return new Promise(res=>{
    $('#cmpModal').innerHTML=`<div class="box card" style="max-width:420px"><h3 style="margin-top:0">${esc(titulo)}</h3><p class="muted">${esc(txt||'')}</p>
    ...
    $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
    const fin=v=>{cerrarCmp();res(v);};$('#pcX').onclick=()=>fin(false);$('#cmpScrim').onclick=()=>fin(false);$('#pcOk').onclick=()=>fin(true);
  });
}
...
async function grabarNFCUbic(ambito,estanteria){
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';
  let done=false;const ctrl=new AbortController();
  const cerrar=()=>{try{ctrl.abort();}catch(_){}cerrarCmp();};
  $('#nfcUX').onclick=cerrar;$('#cmpScrim').onclick=cerrar;
  try{
    const reader=new NDEFReader();await reader.scan({signal:ctrl.signal});
    reader.onreading=async ev=>{
      if(done)return;done=true;
      const uid=ev.serialNumber||'';
      let escrito=false;try{await reader.write({records},{signal:ctrl.signal});escrito=true;}catch(_){}
      try{await api('/ubicaciones/nfc',{method:'POST',body:JSON.stringify({ambito,estanteria:estanteria||null,uid})});}catch(_){}
      const m=$('#nfcUMsg');if(m){m.textContent=(escrito?'✅ Etiqueta grabada':'✅ UID registrado (no se pudo escribir el tag)')+(uid?(' · UID '+uid):'');m.style.color='var(--ok)';}
      toast('NFC de estantería '+(escrito?'grabada':'registrada'));setTimeout(()=>{cerrarCmp();loadUbic();},1100);
    };
    reader.onreadingerror=()=>{const m=$('#nfcUMsg');if(m)m.textContent='No se pudo leer la etiqueta.';};
  }catch(e){const m=$('#nfcUMsg');if(m){m.textContent='NFC: '+e.message;m.style.color='var(--bad)';}}
}
...
async function pickerUbic(){
  ...
  $('#cmpModal').innerHTML=`<div class="box card" style="max-width:460px"><h3 style="margin-top:0">📍 Asignar <b>${selDocs.size}</b> doc(s) a una estantería</h3>
  ...
  $('#cmpScrim').style.display='block';$('#cmpModal').style.display='grid';$('#puX').onclick=cerrarCmp;$('#cmpScrim').onclick=cerrarCmp;
  const refE=()=>llenarDatalist('puDlE',estanteriasDe(($('#puAmb').value)||''));refE();
  $('#puAmb').oninput=refE;
  $('#puOk').onclick=async()=>{
    const ambito=($('#puAmb').value||'').trim(),estanteria=($('#puEst').value||'').trim();
    if(!ambito){$('#puErr').textContent='Indica el ámbito';return;}
    try{const r=await api('/ubicaciones/asignar',{method:'POST',body:JSON.stringify({ids:[...selDocs],ambito,estanteria})});
      if(!r.ok){$('#puErr').textContent=r.motivo;return;}
      cerrarCmp();toast(`${r.n} doc(s) → ${r.ambito}${r.estanteria&&r.estanteria!=='Sin asignar'?' · '+r.estanteria:''}`);selDocs.clear();buscarCatalogo(estadoBusqueda.page||1);
    }catch(e){$('#puErr').textContent=e.message;}};
}
const loaders={dashboard:loadDashboard,activity:loadActivity,cuar:loadCuar,pap:loadPap,obras:loadObras,colecciones:loadColecciones,ubic:loadUbic,inbox:loadInbox,search:loadSearch};
...
let _deepDoc=null,_deepUbic=null,_deepOff=null,_compartido=false;
function abrirEnlaceProfundo(){
  try{history.replaceState(null,'',location.pathname);}catch(_){}
  if(_deepDoc){const id=_deepDoc;_deepDoc=null;_deepUbic=null;return verDoc(id,{volver:'search',etiqueta:'Búsqueda'});}
  // Etiqueta de estantería (?amb=&est=) → abrir sus libros en la Búsqueda (disponible para invitados).
  if(_deepUbic){const{amb,est}=_deepUbic;_deepUbic=null;
    estadoBusqueda.extra={ambito:amb,estanteria:est||undefined,etiqueta:'📍 '+amb+(est?' · '+est:'')};estadoBusqueda.page=1;go('search');}
}
// ¿Procesar lo compartido al instante? Interruptor persistente (localStorage). Por defecto SÍ.
let compartidosPendientes=[];
const autoCompartir=()=>{const sw=$('#inAutoCompartir');return sw?sw.checked:(localStorage.getItem('auto_compartir')!=='0');};
...
async function procesarCompartidos(){
  if(!_compartido)return;_compartido=false;
  if(ROL!=='admin'){toast('Solo un administrador puede dar de alta lo compartido','warn');return;}
  if(!('caches' in window))return;
  try{
    const cache=await caches.open('compartidos-v1');
    const idx=await cache.match('/__shared__/index.json');if(!idx){toast('No llegó nada que compartir','warn');return;}
    const metas=await idx.json();const files=[];
    for(const m of (metas||[])){const r=await cache.match('/__shared__/'+m.i);if(!r)continue;const b=await r.blob();files.push(new File([b],m.nombre||('compartido-'+m.i),{type:m.type||b.type}));}
    for(const k of await cache.keys())await cache.delete(k);   // vaciar tras leer
    if(!files.length)return;
    go('inbox');
    if(autoCompartir()){toast(`📥 Recibido(s) ${files.length} fichero(s) compartido(s)`);subirInbox(files);}
    else{compartidosPendientes=files;toast(`📥 ${files.length} fichero(s) en espera: añade datos y pulsa Procesar`);pintarCompartidosPendientes();}
  }catch(e){toast('No se pudo procesar lo compartido: '+e.message,'bad');}
}
...
function pintarCompartidosPendientes(){
  const out=$('#inboxResults');if(!out)return;
  if(!compartidosPendientes.length){out.innerHTML='';return;}
  ...
  $('#shProc').onclick=()=>{const f=compartidosPendientes.slice();compartidosPendientes=[];subirInbox(f);};
  $('#shDesc').onclick=()=>{compartidosPendientes=[];out.innerHTML='';toast('Descartado');};
}
...
async function cargarUsuariosLogin(){try{const r=await fetch('/api/usuarios').then(x=>x.json());const sel=$('#lgUser');if(!sel)return;
  sel.innerHTML=(r.usuarios||[]).map(u=>`<option value="${esc(u.user)}">${esc(u.user)}${u.rol==='admin'?' · admin':''}</option>`).join('')||'<option value="">—</option>';}catch{}}
const tokenDeCookie=()=>{const m=document.cookie.match(/(?:^|;\s*)panel_token=([^;]+)/);return m?decodeURIComponent(m[1]):'';};
const limpiarCookieToken=()=>{document.cookie='panel_token=; Max-Age=0; path=/';};
...
async function autoLoginURL(){
  ...
  const u=q.get('u')||q.get('user'), p=q.get('p')||q.get('pwd');
  const limpiar=()=>history.replaceState(null,'',location.pathname+location.hash);
  if(!u||!p)return false;
  try{
    const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({usuario:u,password:p})}).then(x=>x.json());
    limpiar();
    if(r&&r.ok){TOKEN=r.token;USER=r.usuario;ROL=r.rol;localStorage.setItem('panel_token',TOKEN);return true;}
  }catch{limpiar();}
  return false;
}
function mostrarLogin(){TOKEN='';ROL=null;localStorage.removeItem('panel_token');cargarUsuariosLogin();$('#login').classList.add('show');if(estadoTimer)clearInterval(estadoTimer);}
function aplicarRol(){document.body.classList.toggle('guest',ROL==='guest');$('#who').textContent=USER||'';$('#rol').textContent=ROL||'';$('#rol').className='rol '+(ROL||'');}
function bannerOffline(){let b=document.getElementById('offBanner');if(!b){b=document.createElement('div');b.id='offBanner';b.style.cssText='position:fixed;top:0;left:0;right:0;z-index:400;background:#7a5c00;color:#fff;text-align:center;font-size:13px;padding:6px 10px;box-shadow:0 1px 6px rgba(0,0,0,.4)';b.innerHTML='📴 Sin conexión — acerca una etiqueta NFC para ver su ficha offline';document.body.appendChild(b);}}
function arrancar(){$('#login').classList.remove('show');aplicarRol();iniciarHistorial();
  ...
  if(APP_OFFLINE){
    if(_deepOff){const off=_parseOffline(_b64uDec(_deepOff));if(off){mostrarOfflineNFC(off,null);return;}}
    pantallaEsperaNFC();return;
  }
  cargarUbicaciones();go('search');abrirEnlaceProfundo();procesarCompartidos();refrescarEstado();if(estadoTimer)clearInterval(estadoTimer);estadoTimer=setInterval(refrescarEstado,8000);}
$('#loginForm').onsubmit=async e=>{e.preventDefault();$('#lgErr').textContent='';
  try{const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({usuario:$('#lgUser').value.trim(),password:$('#lgPass').value})});
    const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.motivo||'usuario o contraseña incorrectos');
    TOKEN=j.token;USER=j.usuario;ROL=j.rol;localStorage.setItem('panel_token',TOKEN);_recordarSesion();$('#lgPass').value='';arrancar();
...
function _recordarSesion(){try{localStorage.setItem('panel_user',USER||'');localStorage.setItem('panel_rol',ROL||'');}catch(_){}}
$('#logout').onclick=async()=>{try{await api('/logout',{method:'POST'});}catch{};try{localStorage.removeItem('panel_user');localStorage.removeItem('panel_rol');}catch(_){}mostrarLogin();};
...
(async()=>{
  ...
  try{const sp=new URLSearchParams(location.search).get('s');if(sp){await vistaCompartida(sp);return;}}catch(_){}
  // 0b) Captura los enlaces profundos (?doc=<id> de un libro, ?amb=&est= de una estantería) ANTES de que
  //    autoLoginURL limpie la query.
  try{const qp=new URLSearchParams(location.search);_deepDoc=qp.get('doc')||null;_deepOff=qp.get('o')||null;_compartido=qp.get('compartido')==='1';
    const amb=qp.get('amb');if(amb)_deepUbic={amb,est:qp.get('est')||''};}catch(_){}
  ...
  if(ck){TOKEN=ck;localStorage.setItem('panel_token',TOKEN);limpiarCookieToken();}
  if(!TOKEN)return mostrarLogin();
  try{const me=await fetch('/api/yo',{headers:{'Authorization':'Bearer '+TOKEN}}).then(r=>r.json());
    if(me&&me.rol){USER=me.usuario;ROL=me.rol;_recordarSesion();arrancar();}else mostrarLogin();
  }catch{
    ...
    if(TOKEN&&rol){USER=localStorage.getItem('panel_user')||'';ROL=rol;APP_OFFLINE=true;arrancar();}else mostrarLogin();
  }})();
