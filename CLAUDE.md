...
- **illegible / structurally-damaged file (corrupt PDF/EPUB) or 0-byte ghost** → **Cuarentena/ilegibles** (`enviarAIlegibles`, deposit + sidecar): not a cataloguing problem but a *needs-a-healthy-copy* one — fixed from the panel's Cuarentena fix-it workflow (search-link a copy on Anna's Archive/Libgen/Gutenberg → upload it → `POST /api/saneamiento/reemplazar` re-ingests and recycles the deposit; `src/utils/saneamiento.js`). This **replaces the old `_ER Room`** (now legacy; `scripts/migrar-erroom-a-ilegibles.js` moves any existing `_ER Room` files into `Cuarentena/ilegibles` deposits).
